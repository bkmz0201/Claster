...
export class DesktopStateSynchronizer extends Service {
    constructor(workbenchService, electronApi, peekViewService) {
        super();
        ...
        this.startSync = () => {
            ...
            this.disposables.push(this.electronApi.events.ui.onTabAction(event => {
                if (event.type === 'open-in-split-view' &&
                ...
                if (event.type === 'activate-view' &&
                    event.payload.tabId === appInfo?.viewId) {
                    workbench.active(event.payload.viewIndex);
                }
            }));
            this.disposables.push(this.electronApi.events.ui.onCloseView(() => {
                (async () => {
            ...
            this.disposables.push(this.electronApi.events.ui.onToggleRightSidebar(tabId => {
                if (tabId === appInfo?.viewId) {
                    workbench.setSidebarOpen(!workbench.sidebarOpen$.value);
                }
            }));
            this.disposables.push(this.electronApi.events.ui.onTabGoToRequest(opts => {
            ...
            const activeViewIndexSub = workbench.activeViewIndex$.subscribe(activeViewIndex => {
                if (!appInfo?.viewId) {
                    return;
                }
                this.electronApi.handler.ui
            ...
            this.disposables.push(() => viewsSub.unsubscribe(), () => activeViewIndexSub.unsubscribe(), () => basenameSub.unsubscribe());
        };
        this.startSync();
    }
}
//# sourceMappingURL=desktop-state-synchronizer.js.map