...
export const skipOnboarding = async (context) => {
        ...
        await use({
            current: async () => {
                return await page.evaluate(async () => {
                    if (!globalThis.currentWorkspace) {
                        await new Promise((resolve, reject) => {
                            globalThis.addEventListener('affine:workspace:change', () => resolve(), {
                                once: true,
                            });
                            setTimeout(() => reject(new Error('timeout')), 5000);
                        });
                    }
                    const currentWorkspace = globalThis.currentWorkspace;
                    return {
    ...
    context: async ({ context }, use) => {
        ...
        if (enableCoverage) {
            ...
            await context.exposeFunction('collectIstanbulCoverage', (coverageJSON) => {
                if (coverageJSON)
                    fs.writeFileSync(path.join(istanbulTempDir, `playwright_coverage_${generateUUID()}.json`), coverageJSON);
            });
        }
        await use(context);
        if (enableCoverage) {
