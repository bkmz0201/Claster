...
export async function waitForLogMessage(page, log) {
...
export async function removeWithRetry(filePath, maxRetries = 5, delay = 500) {
...
export async function isContainedInBoundingBox(container, element, includeDescendant = false) {
    const containerBox = await container.boundingBox();
    if (!containerBox) {
        throw new Error('Container bounding box not found');
    }
    const { x: cx, y: cy, width: cw, height: ch } = containerBox;
    const inside = async (el) => {
