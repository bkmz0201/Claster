...
import { addBasicShapeElement, Shape, switchEditorMode, triggerComponentToolbarAction, } from '../utils/actions/edgeless.js';
import { enterPlaygroundRoom, initEmptyEdgelessState, } from '../utils/actions/misc.js';
import { parseStringToRgba } from '../utils/bs-alternative.js';
import { test } from '../utils/playwright.js';
async function setupWithColorPickerFunction(page) {
    await enterPlaygroundRoom(page);
    await initEmptyEdgelessState(page);
    await switchEditorMode(page);
}
function getColorPanelWithLabel(page, label) {
    return page.locator(`edgeless-color-panel[aria-label="${label}"]`);
}
function getCurrentColorUnitButton(locator) {
    return locator.locator('edgeless-color-button').locator('.color-unit').nth(0);
}
function getCurrentColor(locator) {
    return locator.evaluate(ele => getComputedStyle(ele.querySelector('svg')).getPropertyValue('fill'));
}
function getCustomButton(locator) {
    return locator.locator('edgeless-color-custom-button');
}
function getColorPickerPanel(locator) {
    return locator.locator('edgeless-color-picker');
}
function getPaletteControl(locator) {
    return locator.locator('.color-palette');
}
function getHueControl(locator) {
    return locator.locator('.color-slider-wrapper.hue .color-slider');
}
function getAlphaControl(locator) {
...
function getAlphaInput(locator) {
...
test.describe('basic functions', () => {
    ...
    test('should open color-picker panel when clicking on custom color button', async ({ page, }) => {
        await setupWithColorPickerFunction(page);
        const start0 = { x: 100, y: 100 };
        const end0 = { x: 150, y: 200 };
        await addBasicShapeElement(page, start0, end0, Shape.Square);
    ...
    test('should close color-picker panel when clicking on outside', async ({ page, }) => {
        await setupWithColorPickerFunction(page);
        const start0 = { x: 100, y: 100 };
        const end0 = { x: 150, y: 200 };
        await addBasicShapeElement(page, start0, end0, Shape.Square);
    ...
    test('should return to the palette panel when re-clicking the color button', async ({ page, }) => {
        await setupWithColorPickerFunction(page);
        const start0 = { x: 100, y: 100 };
        const end0 = { x: 150, y: 200 };
        await addBasicShapeElement(page, start0, end0, Shape.Square);
    ...
    test('should pick a color when clicking on the palette canvas', async ({ page, }) => {
        await setupWithColorPickerFunction(page);
        const start0 = { x: 100, y: 100 };
        const end0 = { x: 150, y: 200 };
        await addBasicShapeElement(page, start0, end0, Shape.Square);
    ...
    test('should pick a color when clicking on the hue control', async ({ page, }) => {
        await setupWithColorPickerFunction(page);
        const start0 = { x: 100, y: 100 };
        const end0 = { x: 150, y: 200 };
        await addBasicShapeElement(page, start0, end0, Shape.Square);
    ...
    test('should adjust alpha when clicking on the alpha control', async ({ page, }) => {
        await setupWithColorPickerFunction(page);
        const start0 = { x: 100, y: 100 };
        const end0 = { x: 150, y: 200 };
        await addBasicShapeElement(page, start0, end0, Shape.Square);
    ...
    test('should adjust alpha when changing the alpha input', async ({ page, }) => {
        await setupWithColorPickerFunction(page);
        const start0 = { x: 100, y: 100 };
        const end0 = { x: 150, y: 200 };
        await addBasicShapeElement(page, start0, end0, Shape.Square);
