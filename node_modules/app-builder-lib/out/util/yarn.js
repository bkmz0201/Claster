...
async function installOrRebuild(config, { appDir, projectDir }, options, forceInstall = false) {
...
async function installDependencies(config, { appDir, projectDir }, options) {
    const platform = options.platform || process.platform;
    const arch = options.arch || process.arch;
    const additionalArgs = options.additionalArgs;
    const pm = (0, node_module_collector_1.detectPackageManager)([projectDir]);
    builder_util_1.log.info({ pm, platform, arch, projectDir, appDir }, `installing production dependencies`);
    const execArgs = ["install"];
    if (pm === node_module_collector_1.PM.YARN_BERRY) {
...
/** @internal */
async function rebuild(config, { appDir, projectDir }, options) {
    const configuration = {
