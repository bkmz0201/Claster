...
vi.mock('@blocksuite/affine-components/context-menu', () => ({
    ...
    popFilterableSimpleMenu: (...args) => popFilterableSimpleMenu(...args),
    popupTargetFromElement: (el) => el,
}));
...
describe('data view helpers', () => {
    ...
    test('MobileKanbanViewUILogic.addRow triggers update', () => {
        ...
        const update = vi.fn();
        logic.ui$.value = { requestUpdate: update };
        if (!logic.ui$.value) {
    ...
    test('popCardMenu actions request update', () => {
        ...
        popCardMenu({}, 'g', 'c', kanbanViewLogic);
        const groups = popFilterableSimpleMenu.mock.calls[0][1];
        groups[2].items[0].select();
        expect(kanbanViewLogic.view.addCard).toHaveBeenCalledWith({ before: true, id: 'c' }, 'g');
        expect(update).toHaveBeenCalledTimes(1);
        groups[2].items[1].select();
        expect(kanbanViewLogic.view.addCard).toHaveBeenCalledWith({ before: false, id: 'c' }, 'g');
    ...
    test('popMobileRowMenu delete action requests update', () => {
        ...
        const view = { rowsDelete: vi.fn() };
        popMobileRowMenu({}, 'r1', tableViewLogic, view);
        const groups = popFilterableSimpleMenu.mock.calls.pop()[1];
        groups[1].items[0].select();
        expect(view.rowsDelete).toHaveBeenCalledWith(['r1']);
        expect(update).toHaveBeenCalled();
    });
});
//# sourceMappingURL=data-view-actions.spec.js.map