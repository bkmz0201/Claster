...
import { enterPlaygroundRoom, initEmptyCodeBlockState, } from './utils/actions/index.js';
...
test(scoped `create a paragraph-block while clicking zero-width of code-block which is the last block of page`, async ({ page }) => {
...
test(scoped `don't create a paragraph-block while clicking zero-width of code-block before a paragraph-block`, async ({ page }) => {
    await enterPlaygroundRoom(page);
    await page.evaluate(() => {
        const { doc } = window;
    ...
    await page.mouse.click(codeComponentrect.x + 20, codeComponentrect.y + codeComponentrect.height + 8);
    await assertBlockChildrenIds(page, '1', ['2', '3']);
});
test(scoped `create a paragraph-block while clicking between two non-paragraph-block`, async ({ page }) => {
    await enterPlaygroundRoom(page);
    await page.evaluate(async ({ bookMarkUrl, embedUrl }) => {
        const { doc } = window;
        const rootId = doc.addBlock('affine:page', {
            title: new window.$blocksuite.store.Text(),
        });
    ...
    await page.mouse.click(dividerComponentRect.x + 20, dividerComponentRect.y + dividerComponentRect.height + 8);
    ...
    await page.mouse.click(bookmarkComponentRect.x + 20, bookmarkComponentRect.y + bookmarkComponentRect.height + 8);
    ...
    await page.mouse.click(imageComponentRect.x + 20, imageComponentRect.y + imageComponentRect.height + 8);
    await assertBlockFlavour(page, '10', 'affine:paragraph');
});
//# sourceMappingURL=zero-width.spec.js.map