...
const getPageId = async (page) => {
    return page.evaluate(() => {
        return window.__appInfo?.viewId;
    });
};
const isActivePage = async (page) => {
    return page.evaluate(async () => {
        return await window.__apis?.ui.isActiveTab();
    });
};
const getActivePage = async (pages) => {
...
export const test = base.extend({
    ...
    electronApp: async ({}, use) => {
        try {
            ...
            const packageJson = await fs.readJSON(electronRoot.join('package.json').value);
