...
import { CELL_HEIGHT, DATE_FORMAT, MONTH_VIEW_HEIGHT, SCROLL_DOWN_TO_FOLD_THRESHOLD, WEEK_VIEW_HEIGHT, } from './constants';
...
export const ResizeViewport = ({ className, ...attrs }) => {
    ...
    const draggedHeight = Math.max(WEEK_VIEW_HEIGHT, Math.min(MONTH_VIEW_HEIGHT, staticHeight + dragOffset));
    const handleToggleModeWithAnimation = useCallback((targetMode, draggedDistance, isCancel = false) => {
        const targetDragOffset = isCancel
            ? 0
            : targetMode === 'week'
                ? -(MONTH_VIEW_HEIGHT - WEEK_VIEW_HEIGHT)
                : MONTH_VIEW_HEIGHT - WEEK_VIEW_HEIGHT;
        const dragOffsetProxy = new Proxy({ value: draggedDistance }, {
        ...
        return swipeHelper.init(draggableRef.current, {
            ...
            onSwipeEnd: ({ deltaY, speedY }) => {
                if (Math.abs(deltaY) > 2 * CELL_HEIGHT || speedY > 100) {
                    handleToggleModeWithAnimation(mode === 'week' ? 'month' : 'week', deltaY);
                }
                else {
    ...
    useGlobalEvent('scrollend', useCallback(() => {
        prevScrollYRef.current = window.scrollY;
    }, []));
    return (_jsxs("div", { className: clsx(styles.root, className), ...attrs, children: [_jsx(WeekHeader, { className: styles.weekRow }), _jsx("div", { ref: viewportRef, style: { height: draggedHeight }, children: mode === 'month' || isDragging || isAnimating ? (_jsx(MonthView, { viewportHeight: draggedHeight })) : (_jsx(WeekRowSwipe, { start: firstDayOfWeek })) }), _jsx("div", { className: styles.draggable, ref: draggableRef, children: _jsx("div", { className: styles.draggableHandle }) })] }));
};
