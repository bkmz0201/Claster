...
import { addDatabase, clickNewPageButton, getBlockSuiteEditorTitle, waitForEditorLoad, } from '@affine-test/kit/utils/page-logic';
...
test('link page is useable', async ({ page }) => {
    ...
    await page.waitForTimeout(500);
    await expect(page.locator('.doc-title-container:has-text("page1")')).toBeVisible();
});
test('append paragraph when click editor gap', async ({ page }) => {
    ...
    await page.locator('[data-testid=page-editor-blank]').click();
    expect(await paragraph.count(), 'click the gap again, should not append another paragraph').toBe(numParagraphs + 1);
    const editorContainer = locateEditorContainer(page);
    expect(await editorContainer.evaluate(el => el.contains(document.activeElement)), 'editor should should keep being focused').toBe(true);
});
