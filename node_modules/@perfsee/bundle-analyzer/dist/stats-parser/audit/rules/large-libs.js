...
exports.largeLibs = void 0;
const stats_1 = require("../../../stats");
const utils_1 = require("../../../utils");
const utils_2 = require("../utils");
const whiteList = new Set([stats_1.SOURCE_CODE_PATH, 'react-dom', 'vue', 'lodash-es', 'rxjs', 'antd']);
const largeLibs = ({ packages, chunks }) => {
    const largePackages = [];
    let packagesSize = (0, utils_1.getDefaultSize)();
    let largePackagesSize = (0, utils_1.getDefaultSize)();
    const initialAssets = new Set(chunks
        .filter((chunk) => !chunk.async)
        .map((chunk) => chunk.assets.map((asset) => asset.ref))
        .flat());
    packages.forEach((pkg) => {
        packagesSize = (0, utils_1.addSize)(packagesSize, pkg.size);
        packages;
        if (pkg.size.raw > /* 100KB */ 100000 &&
            !whiteList.has(pkg.name) &&
            pkg.assets.some((asset) => initialAssets.has(asset.ref))) {
            largePackages.push(pkg);
            largePackagesSize = (0, utils_1.addSize)(largePackagesSize, pkg.size);
        }
    });
    return {
        id: 'large-libraries',
...
Prefer smaller, functionally equivalent libraries to reduce your bundle size, or on-demand loading with dynamic import statement \`import('lib')\`.`,
        ...
        score: (0, utils_2.rangeScore)(largePackages.length, 0, 5),
        numericScore: {
...
exports.largeLibs = largeLibs;
//# sourceMappingURL=large-libs.js.map