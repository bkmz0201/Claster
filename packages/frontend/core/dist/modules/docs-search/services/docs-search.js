...
export class DocsSearchService extends Service {
    constructor(workspaceService, docsService) {
        super();
        this.workspaceService = workspaceService;
        this.docsService = docsService;
        this.indexerState$ = LiveData.from(this.indexer.state$, {
            indexing: 0,
            errorMessage: null,
        });
    }
            ...
            for (const bucket of buckets) {
                ...
                if (firstMatchFlavour === 'affine:page') {
                    ...
                    result.push({
                        docId: bucket.key,
                        title: title,
                        blockId: typeof matchedBlockId === 'string'
                            ? matchedBlockId
                            : matchedBlockId[0],
                        score: bucket.score,
    ...
    watchRefsFrom(ids) {
        ...
        return this.indexer
            ...
            return fromPromise(async () => {
                const refs = Array.from(new Map(nodes
                    .flatMap(node => {
    ...
    watchDatabasesTo(docId) {
        ...
        return this.indexer
            ...
            return nodes
                ...
                return {
                    docId: typeof node.fields.docId === 'string'
                        ? node.fields.docId
                        : node.fields.docId[0],
                    rowId: typeof node.fields.blockId === 'string'
                        ? node.fields.blockId
                        : node.fields.blockId[0],
                    databaseBlockId: typeof node.fields.parentBlockId === 'string'
