...
test.describe('AIChatWith/tags', () => {
    ...
    test.skip('should support chat with tag', async ({ loggedInPage: page, utils, }) => {
        const randomStr = Math.random().toString(36).substring(2, 6);
        await utils.editor.createTagAndDoc(page, 'Tag 1', `Tag${randomStr} is a cute cat`);
        await utils.chatPanel.chatWithTags(page, ['Tag 1']);
        await utils.chatPanel.makeChat(page, `What is Tag${randomStr}(Use English)`);
    ...
    test.skip('should support chat with multiple tags', async ({ loggedInPage: page, utils, }) => {
        const randomStr1 = Math.random().toString(36).substring(2, 6);
        const randomStr2 = Math.random().toString(36).substring(2, 6);
        await utils.editor.createTagAndDoc(page, 'Tag 2', `Tag${randomStr1} is a cute cat`);
        await utils.editor.createTagAndDoc(page, 'Tag 3', `Tag${randomStr2} is a cute dog`);
        await utils.chatPanel.chatWithTags(page, ['Tag 2', 'Tag 3']);
        await utils.chatPanel.makeChat(page, `What is Tag${randomStr1}? What is Tag${randomStr2}?(Use English)`);
        await utils.chatPanel.waitForHistory(page, [
            {
                role: 'user',
