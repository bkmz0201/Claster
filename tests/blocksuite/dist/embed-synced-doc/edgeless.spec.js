...
import { createNote, createShapeElement, getAllSortedIds, getIds, getSelectedBound, getSelectedIds, isIntersected, switchEditorMode, } from '../utils/actions/edgeless';
...
test.describe('Embed synced doc in edgeless mode', () => {
    ...
    test.fixme('drag embed synced doc to whiteboard should fit in height', async ({ page, }) => {
        ...
        const embedSyncedBlockInNote = page.locator('affine-embed-synced-doc-block');
        ...
        await page.mouse.move(embedSyncedBoxInNote.x - 10, embedSyncedBoxInNote.y - 100);
        await page.mouse.move(embedSyncedBoxInNote.x - 10, embedSyncedBoxInNote.y + 10);
        ...
        const EmbedSyncedDocBlockInCanvas = page.locator('affine-embed-edgeless-synced-doc-block');
        const EmbedSyncedDocBlockBoxInCanvas = await EmbedSyncedDocBlockInCanvas.boundingBox();
        const border = 1;
        if (!EmbedSyncedDocBlockBoxInCanvas) {
            throw new Error('EmbedSyncedDocBlockBoxInCanvas is not found');
        }
        expect(EmbedSyncedDocBlockBoxInCanvas.height).toBeCloseTo(height + 2 * border, 1);
    });
    test('new edgeless embed synced doc should fit in height', async ({ page, }) => {
        ...
        const createEmbedDocWithHeight = async (height) => {
            await page.evaluate(({ embedDocId, height }) => {
                const std = window.editor.std;
                const surface = std.store.getModelsByFlavour('affine:surface')[0];
                std.store.addBlock('affine:embed-synced-doc', {
                    pageId: embedDocId,
        ...
        const embedSyncedBlockInNote = page.locator('affine-embed-edgeless-synced-doc-block');
        {
    ...
    test.describe('edgeless element toolbar', () => {
        test.beforeEach(async ({ page }) => {
            ...
            await switchEditorMode(page);
            const edgelessEmbedSyncedBlock = page.locator('affine-embed-edgeless-synced-doc-block');
            await edgelessEmbedSyncedBlock.click();
        });
        const getDocIds = async (page) => {
            return page.evaluate(() => {
                return [...window.collection.docs.keys()];
            });
        };
        const locateToolbar = (page) => {
            return page.locator(
        ...
        test('should insert embed-synced-doc into page when click "Insert into page" button', async ({ page, }) => {
            ...
            await expect(edgelessEmbedSyncedBlock, 'the edgeless embed synced doc should be remained after click insert button').toBeVisible();
            await switchEditorMode(page);
            await expect(embedSyncedBlock).toBeVisible();
        });
        test('should render a reference node and all content of embed-synced-doc after click "Duplicate as note" button', async ({ page, }) => {
            // switch doc
            const switchDoc = async () => page.evaluate(() => {
            ...
            const refInfo = await reference.evaluate((reference) => {
        ...
        test('should be selected and not overlay with the embed-synced-doc after duplicating as note', async ({ page, }) => {
        ...
        test('edgeless note duplicated from embed-synced-doc should be above other elements', async ({ page, }) => {
...
//# sourceMappingURL=edgeless.spec.js.map