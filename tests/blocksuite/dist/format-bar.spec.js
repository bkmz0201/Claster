...
import { activeEmbed, captureHistory, dragBetweenCoords, dragBetweenIndices, enterPlaygroundRoom, focusRichText, focusRichTextEnd, focusTitle, getBoundingBox, getEditorHostLocator, getPageSnapshot, getSelectionRect, initEmptyParagraphState, initImageState, initThreeParagraphs, pasteByKeyboard, pressArrowDown, pressArrowLeft, pressArrowUp, pressEnter, pressEscape, pressTab, scrollToBottom, scrollToTop, selectAllBlocksByKeyboard, selectAllByKeyboard, setInlineRangeInInlineEditor, switchReadonly, type, undoByKeyboard, updateBlockType, waitNextFrame, withPressKey, } from './utils/actions/index.js';
import { assertAlmostEqual, assertBlockChildrenIds, assertLocatorVisible, assertRichImage, assertRichTextInlineRange, assertRichTexts, } from './utils/asserts.js';
...
test('should format quick bar show when clicking drag handle', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await initThreeParagraphs(page);
...
test('should format quick bar show when select text by keyboard', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    ...
    assertAlmostEqual(formatBarBox.y + formatBarBox.height - selectionRect.top, -10, 3);
    await page.keyboard.press('ArrowLeft');
    await expect(formatBar).not.toBeVisible();
    await withPressKey(page, 'Shift', async () => {
        let i = 10;
...
test('should format quick bar can only display one at a time', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await initThreeParagraphs(page);
...
test('should format quick bar be able to format text', async ({ page, }, testInfo) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await initThreeParagraphs(page);
    // drag only the `456` paragraph
    await dragBetweenIndices(page, [1, 0], [1, 3]);
    const { boldBtn, italicBtn, underlineBtn, strikeBtn, codeBtn } = getFormatBar(page);
    ...
    await expect(strikeBtn).toHaveAttribute('active', '');
    await expect(codeBtn).not.toHaveAttribute('active', '');
    expect(await getPageSnapshot(page, true)).toMatchSnapshot(`${testInfo.title}_finial.json`);
});
test('should format quick bar be able to change background color', async ({ page, }, testInfo) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await initThreeParagraphs(page);
    ...
    await highlight.highlightBtn.click();
    await highlight.redForegroundBtn.click();
    expect(await getPageSnapshot(page, true)).toMatchSnapshot(`${testInfo.title}_select_all.json`);
    await highlight.highlightBtn.click();
    await expect(highlight.defaultColorBtn).toBeVisible();
    await highlight.defaultColorBtn.click();
    expect(await getPageSnapshot(page, true)).toMatchSnapshot(`${testInfo.title}_default_color.json`);
});
test('should format quick bar be able to format text when select multiple line', async ({ page, }, testInfo) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await initThreeParagraphs(page);
    await dragBetweenIndices(page, [0, 0], [2, 3]);
...
test('should format quick bar be able to link text', async ({ page, }, testInfo) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await initThreeParagraphs(page);
    // drag only the `456` paragraph
...
test('should format quick bar be able to change to heading paragraph type', async ({ page, }, testInfo) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await initThreeParagraphs(page);
    // drag only the `456` paragraph
    ...
    expect(await getPageSnapshot(page, true)).toMatchSnapshot(`${testInfo.title}_bulleted.json`);
    await openParagraphMenu();
    await textBtn.click();
    expect(await getPageSnapshot(page, true)).toMatchSnapshot(`${testInfo.title}_finial.json`);
...
test('should format quick bar show when double click text', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await initThreeParagraphs(page);
...
test('should format quick bar position correct at the start of second line', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await page.evaluate(() => {
        const { doc } = window;
    ...
    await dragBetweenCoords(page, { x: textBox.x + textBox.width - 1, y: textBox.y + textBox.height - 1 }, { x: textBox.x, y: textBox.y + textBox.height - 1 });
    const { formatBar } = getFormatBar(page);
    await expect(formatBar).toBeVisible();
    await waitNextFrame(page);
...
test('should format quick bar action status updated while undo', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await focusRichText(page);
...
test('should format quick bar work in single block selection', async ({ page, }, testInfo) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await initThreeParagraphs(page);
    await dragBetweenIndices(page, [1, 0], [1, 3], { x: -26 - 24, y: -10 }, { x: 0, y: 0 });
    ...
    // await italicBtn.click();
    await expect(blockSelections).toHaveCount(1);
    expect(await getPageSnapshot(page, true)).toMatchSnapshot(`${testInfo.title}.json`);
    const noteEl = page.locator('affine-note');
    const { x, y, width, height } = await getBoundingBox(noteEl);
    await page.mouse.click(x + width / 2, y + height / 2);
    await expect(formatBar).not.toBeVisible();
});
test('should format quick bar work in multiple block selection', async ({ page, }, testInfo) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await initThreeParagraphs(page);
    await dragBetweenIndices(page, [2, 3], [0, 0], { x: 20, y: 20 }, { x: 0, y: 0 });
    const blockSelections = page
        .locator('affine-block-selection')
        .locator('visible=true');
...
test('should format quick bar with block selection works when update block type', async ({ page, }, testInfo) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await initThreeParagraphs(page);
    await dragBetweenIndices(page, [2, 3], [0, 0], { x: 20, y: 20 }, { x: 0, y: 0 });
...
test('should format quick bar show after convert to code block', async ({ page, }, testInfo) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await initThreeParagraphs(page);
    const formatBarController = getFormatBar(page);
...
test('buttons in format quick bar should have correct active styles', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await initThreeParagraphs(page);
    // `45`
...
test('should format quick bar show when double click button', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await initThreeParagraphs(page);
    await dragBetweenIndices(page, [0, 0], [2, 3]);
...
test('should the database action icon show correctly', async ({ page }) => {
    ...
    if (!codeBox)
        throw new Error('Missing code block box');
    ...
    await dragBetweenCoords(page, { x: position.startX + 10, y: position.startY - 10 }, { x: position.endX, y: position.endY }, { steps: 20 });
...
test('should show format-quick-bar and select all text of the block when triple clicking on text', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await focusRichText(page);
    await type(page, 'hello world');
...
test('should update the format quick bar state when there is a change in keyboard selection', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await page.evaluate(() => {
        const { doc } = window;
        const rootId = doc.addBlock('affine:page', {
...
test('create linked doc from block selection with format bar', async ({ page, }, testInfo) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await initThreeParagraphs(page);
    ...
    await page.mouse.dblclick(linkedDocBox.x + linkedDocBox.width / 2, linkedDocBox.y + linkedDocBox.height / 2);
    await waitNextFrame(page, 200);
    expect(await getPageSnapshot(page, true)).toMatchSnapshot(`${testInfo.title}.json`);
...
test('should not display format bar when just select embed node', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
...
//# sourceMappingURL=format-bar.spec.js.map