...
export class Viewport {
    ...
    getFitToScreenData(bounds, padding = [0, 0, 0, 0], maxZoom = ZOOM_MAX, fitToScreenPadding = 100) {
     ...
     */
    setCenter(centerX, centerY, forceUpdate = true) {
     ...
     * @param forceUpdate Whether to force complete any pending resize operations before setting the viewport.
     */
    setViewport(newZoom, newCenter = Vec.toVec(this.center), smooth = false, forceUpdate = true) {
     ...
     * @param forceUpdate whether to force complete any pending resize operations before setting the viewport
     */
    setViewportByBound(bound, padding = [0, 0, 0, 0], smooth = false, forceUpdate = true) {
     ...
     * @param forceUpdate Whether to force complete any pending resize operations before setting the viewport.
     */
    setZoom(zoom, focusPoint, wheel = false, forceUpdate = true) {
        if (forceUpdate && this._isResizing) {
            this._forceCompleteResize();
        }
    ...
    smoothTranslate(x, y, numSteps = 10) {
        const { center } = this;
    ...
    smoothZoom(zoom, focusPoint, numSteps = 10) {
