...
test.describe('expand mindmap node', () => {
    ...
    test('should expand the mindmap node', async ({ loggedInPage: page, utils, }) => {
        let id;
        const { expandMindMapNode } = await utils.editor.askAIWithEdgeless(page, async () => {
            id = await utils.editor.createMindmap(page);
        }, async () => {
            // Select the first child in the mindmap
            const { id: childId } = await utils.editor.getMindMapNode(page, id, [0, 0]);
...
//# sourceMappingURL=expand-mindmap-node.spec.js.map