...
export const MemberList = ({ isOwner, isAdmin, goToTeamBilling, }) => {
    ...
    return (_jsxs("div", { children: [pageMembers === undefined ? (isLoading ? (_jsx(MemberListFallback, { memberCount: memberCount
                    ? clamp(memberCount - pageNum * membersService.members.PAGE_SIZE, 1, membersService.members.PAGE_SIZE)
                    : 1 })) : (_jsx("span", { className: styles.errorStyle, children: error
                    ? UserFriendlyError.fromAny(error).message
                    : 'Failed to load members' }))) : (pageMembers?.map(member => (_jsx(MemberItem, { currentAccount: account, member: member, isOwner: isOwner, isAdmin: isAdmin, goToTeamBilling: goToTeamBilling }, member.id)))), memberCount !== undefined &&
                memberCount > membersService.members.PAGE_SIZE && (_jsx(Pagination, { totalCount: memberCount, countPerPage: membersService.members.PAGE_SIZE, pageNum: pageNum, onPageChange: handlePageChange }))] }));
};
const getShouldShow = ({ member, currentAccountId, isOwner, isAdmin, }) => {
    if (member.id === currentAccountId ||
...
const MemberItem = ({ member, isOwner, isAdmin, currentAccount, goToTeamBilling, }) => {
    const t = useI18n();
    ...
    const show = useMemo(() => getShouldShow({
        member,
    ...
    return (_jsxs("div", { className: styles.memberListItem, "data-testid": "member-item", children: [_jsx(Avatar, { size: 36, url: member.avatarUrl, name: (member.name ? member.name : member.email) }), _jsx("div", { className: styles.memberContainer, children: member.name ? (_jsxs(_Fragment, { children: [_jsx("div", { className: styles.memberName, children: member.name }), _jsx("div", { className: styles.memberEmail, children: member.email })] })) : (_jsx("div", { className: styles.memberName, children: member.email })) }), _jsx("div", { className: clsx(styles.roleOrStatus, {
                ...
                }), children: t.t(memberStatus) }), _jsx(Menu, { items: _jsx(MemberOptions, { member: member, openAssignModal: handleOpenAssignModal, isAdmin: isAdmin, isOwner: isOwner, goToTeamBilling: goToTeamBilling }), children: _jsx(IconButton, { disabled: !show, style: {
                        visibility: show ? 'visible' : 'hidden',
                        flexShrink: 0,
                    }, children: _jsx(MoreVerticalIcon, {}) }) }), _jsx(ConfirmAssignModal, { open: open, setOpen: setOpen, member: member, inputValue: inputValue, placeholder: workspaceName, setInputValue: setInputValue, isEquals: isEquals, onConfirm: confirmAssign })] }, member.id));
};
...
export const MemberListFallback = ({ memberCount, }) => {
    // prevent page jitter
