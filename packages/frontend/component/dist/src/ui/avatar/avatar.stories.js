import { jsx as _jsx, Fragment as _Fragment, jsxs as _jsxs } from "react/jsx-runtime";
import { CameraIcon } from '@blocksuite/icons/rc';
import { Avatar } from './avatar';
export default {
    title: 'UI/Avatar',
    component: Avatar,
    argTypes: {
        onClick: () => console.log('Click button'),
    },
};
const Template = args => _jsx(Avatar, { ...args });
export const DefaultAvatar = Template.bind(undefined);
DefaultAvatar.args = {
    name: 'AFFiNE',
    url: 'https://affine.pro/favicon-96.png',
    size: 50,
};
export const Fallback = Template.bind(undefined);
Fallback.args = {
    name: 'AFFiNE',
    size: 50,
};
export const ColorfulFallback = Template.bind(undefined);
ColorfulFallback.args = {
    size: 50,
    colorfulFallback: true,
    name: 'blocksuite',
};
export const ColorfulFallbackWithDifferentSize = args => (_jsxs(_Fragment, { children: [_jsx(Avatar, { ...args, size: 20, colorfulFallback: true, name: "AFFiNE" }), _jsx(Avatar, { ...args, size: 40, colorfulFallback: true, name: "AFFiNE" }), _jsx(Avatar, { ...args, size: 60, colorfulFallback: true, name: "AFFiNE" })] }));
export const WithHover = Template.bind(undefined);
WithHover.args = {
    size: 50,
    colorfulFallback: true,
    name: 'With Hover',
    hoverIcon: _jsx(CameraIcon, {}),
};
export const WithRemove = Template.bind(undefined);
WithRemove.args = {
    size: 50,
    colorfulFallback: true,
    name: 'With Hover',
    hoverIcon: _jsx(CameraIcon, {}),
    removeTooltipOptions: { content: 'This is remove tooltip' },
    avatarTooltipOptions: { content: 'This is avatar tooltip' },
    onRemove: e => {
        console.log('on remove', e);
    },
};
const img = 'data:image/png;base64,';
export const CustomizeBorderRadius = args => (_jsxs("div", { style: { display: 'flex', gap: 4 }, children: [_jsx(Avatar, { ...args, size: 60, colorfulFallback: true, name: "AFFiNE", rounded: 10 }), _jsx(Avatar, { ...args, size: 60, url: img, rounded: "10px" }), _jsx(Avatar, { ...args, size: 60, rounded: '5%', name: "C" })] }));
//# sourceMappingURL=avatar.stories.js.map