...
import { copyByKeyboard, pasteByKeyboard, selectAllByKeyboard, withCtrlOrMeta, writeTextToClipboard, } from '@affine-test/kit/utils/keyboard';
import { openHomePage } from '@affine-test/kit/utils/load-page';
import { clickNewPageButton, getBlockSuiteEditorTitle, waitForEditorLoad, } from '@affine-test/kit/utils/page-logic';
import { clickSideBarAllPageButton } from '@affine-test/kit/utils/sidebar';
import { expect } from '@playwright/test';
const openQuickSearchByShortcut = async (page, checkVisible = true) => {
    await withCtrlOrMeta(page, () => page.keyboard.press('k', { delay: 50 }));
    if (checkVisible) {
        await expect(page.getByTestId('cmdk-quick-search')).toBeVisible();
    }
};
const insertInputText = async (page, text) => {
    await page.locator('[cmdk-input]').fill(text);
    const actual = await page.locator('[cmdk-input]').inputValue();
    expect(actual).toBe(text);
};
const selectItem = async (page, label) => {
    const selectedEl = page
        .locator('[cmdk-item] [data-testid="cmdk-label"]')
        .filter({ hasText: label });
    await selectedEl.click();
};
const commandsIsVisible = async (page, label) => {
...
async function assertTitle(page, text) {
...
async function checkElementIsInView(page, locator) {
...
async function waitForScrollToFinish(page) {
...
async function assertResultList(page, texts) {
    await expect(async () => {
...
async function titleIsFocused(page) {
    const edgeless = page.locator('affine-edgeless-root');
    if (!edgeless) {
...
test('Click slider bar button', async ({ page }) => {
    ...
    const quickSearchButton = page.locator('[data-testid=slider-bar-quick-search-button]');
...
test('Navigate to the 404 page and try to open quick search', async ({ page, }) => {
...
test('assert the recent browse pages are on the recent list', async ({ page, }) => {
    await openHomePage(page);
    await waitForEditorLoad(page);
    // create first page
    await clickNewPageButton(page);
    ...
    await openQuickSearchByShortcut(page);
    {
        ...
        const quickSearchItems = page.locator('[cmdk-item] [data-testid="cmdk-label"]');
...
test('can use cmdk to search page content and scroll to it, then the block will be selected', async ({ page, }) => {
    await openHomePage(page);
    await waitForEditorLoad(page);
    await clickNewPageButton(page);
    await getBlockSuiteEditorTitle(page).click();
    await getBlockSuiteEditorTitle(page).fill('this is a new page to search for content');
    for (let i = 0; i < 30; i++) {
    ...
    const isVisitable = await checkElementIsInView(page, page.locator('[data-affine-editor-container]').getByText('123456'));
    expect(isVisitable).toBe(true);
    const selectionElement = page.locator('affine-scroll-anchoring-widget div.highlight');
    await expect(selectionElement).toBeVisible();
});
test('Create a new page with special characters in the title and search for this page', async ({ page, }) => {
...
test('disable quick search when the link-popup is visitable', async ({ page, }) => {
...
test('can use @ to open quick search to search for doc and insert into canvas', async ({ page, }) => {
    await openHomePage(page);
    await waitForEditorLoad(page);
    const url = page.url();
    ...
    await expect(page.locator('[cmdk-group-items] [cmdk-item][data-selected="true"]')).toContainText(docTitle);
    // press enter to insert the page to canvas
    await page.keyboard.press('Enter');
    await expect(page.locator('affine-embed-edgeless-synced-doc-block')).toBeVisible();
    await expect(page.getByTestId('edgeless-embed-synced-doc-title')).toContainText(docTitle);
...
test('can paste a doc link to create link reference', async ({ page }) => {
    ...
    await expect(page.locator('affine-reference:has-text("Getting Started")')).toBeVisible();
...
test('can use slash menu to insert an external link', async ({ page }) => {
    ...
    const insertLinkBtn = page.locator('[cmdk-item] [data-value="external-link:affine.pro"]');
    await expect(insertLinkBtn).toBeVisible();
    await insertLinkBtn.click();
    await expect(page.locator('affine-bookmark')).toBeVisible();
    await expect(page.locator('.affine-bookmark-content-url')).toContainText(link);
});
...
//# sourceMappingURL=quick-search.spec.js.map