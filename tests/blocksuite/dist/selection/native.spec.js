...
test('native range delete with indent', async ({ page }, testInfo) => {
    ...
    //   def
    //     ghi
    expect(await getPageSnapshot(page, true)).toMatchSnapshot(`${testInfo.title}_init.json`);
    ...
    //     ghi
    await pressBackspace(page);
    expect(await getPageSnapshot(page, true)).toMatchSnapshot(`${testInfo.title}_after_backspace.json`);
    await waitNextFrame(page);
    await undoByKeyboard(page);
    expect(await getPageSnapshot(page, true)).toMatchSnapshot(`${testInfo.title}_after_undo.json`);
    await redoByKeyboard(page);
    expect(await getPageSnapshot(page, true)).toMatchSnapshot(`${testInfo.title}_after_redo.json`);
});
...
test('select all text with dragging and delete by forwardDelete', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await initThreeParagraphs(page);
    await assertRichTexts(page, ['123', '456', '789']);
...
test('select text leaving a few words in the last line and delete', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await initThreeParagraphs(page);
    await assertRichTexts(page, ['123', '456', '789']);
...
test('select text leaving a few words in the last line and delete by forwardDelete', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await initThreeParagraphs(page);
    await assertRichTexts(page, ['123', '456', '789']);
...
test('select text in the same line with dragging leftward and move outside the affine-note', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await initThreeParagraphs(page);
    await assertRichTexts(page, ['123', '456', '789']);
    ...
    await dragBetweenIndices(page, [1, 3], [1, 0], { x: 0, y: 0 }, { x: 0, y: 0 }, {
        steps: 20,
        async beforeMouseUp() {
            await page.mouse.move(noteLeft - 1, blockRect.top + blockRect.height / 2);
        },
    });
    await pressBackspace(page);
    await type(page, 'abc');
...
test('select text in the same line with dragging leftward and move outside the affine-note by forwardDelete', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await initThreeParagraphs(page);
    await assertRichTexts(page, ['123', '456', '789']);
...
test('select text in the same line with dragging rightward and move outside the affine-note', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await initThreeParagraphs(page);
    await assertRichTexts(page, ['123', '456', '789']);
    ...
    await dragBetweenIndices(page, [1, 0], [1, 3], { x: 0, y: 0 }, { x: 0, y: 0 }, {
        steps: 20,
        async beforeMouseUp() {
            await page.mouse.move(noteRight + 1, blockRect.top + blockRect.height / 2);
        },
    });
    await pressBackspace(page);
    await type(page, 'abc');
...
test('select text in the same line with dragging rightward and move outside the affine-note by forwardDelete', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await initThreeParagraphs(page);
    await assertRichTexts(page, ['123', '456', '789']);
...
test('select text in the same line with dragging rightward and press enter create block', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await initThreeParagraphs(page);
    await assertRichTexts(page, ['123', '456', '789']);
...
test('Delete the second divider between two dividers by forwardDelete', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await focusRichText(page);
    await type(page, '--- ');
...
test('should delete line with content after divider not lose content', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await focusRichText(page);
    await type(page, '--- ');
...
test('the cursor should move to closest editor block when clicking outside container', async ({ page, }) => {
    test.info().annotations.push({
        type: 'issue',
...
test('should not crash when mouse over the left side of the list block prefix', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    ...
    await dragBetweenIndices(page, [1, 2], [1, 0], { x: 0, y: 0 }, { x: 0, y: 0 }, {
        beforeMouseUp: async () => {
...
test('should set the last block to end the range after when leaving the affine-note', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await initThreeParagraphs(page);
    await assertRichTexts(page, ['123', '456', '789']);
    ...
    await dragBetweenIndices(page, [0, 2], [2, 1], { x: 0, y: 0 }, { x: 0, y: 30 } // drag below the bottom of the last block
    );
    await copyByKeyboard(page);
    assertClipItems(page, 'text/plain', '3456789');
});
test('should set the first block to start the range before when leaving the affine-note-block-container', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await initThreeParagraphs(page);
    await assertRichTexts(page, ['123', '456', '789']);
    ...
    // blur
    await page.mouse.click(0, 0);
    await dragBetweenIndices(page, [2, 1], [0, 2], { x: 0, y: 0 }, { x: 0, y: -30 } // drag above the top of the first block
    );
    await copyByKeyboard(page);
...
test('should select texts on cross-note dragging', async ({ page }) => {
    ...
    await setInlineRangeInInlineEditor(page, {
        index: 3,
        length: 0,
    }, 3);
    // goto next note
    ...
    // blur
    await page.mouse.click(0, 0);
    await dragBetweenIndices(page, [0, 2], [3, 1], { x: 0, y: 0 }, { x: 0, y: 30 } // drag below the bottom of the last block
    );
    await copyByKeyboard(page);
    assertClipItems(page, 'text/plain', '3456789ABC');
});
test('should select full text of the first block when leaving the affine-note-block-container in edgeless mode', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    ...
    await dragBetweenIndices(page, [2, 1], [0, 2], { x: 0, y: 0 }, { x: 0, y: 0 }, // drag above the top of the first block
    {
        beforeMouseUp: async () => {
...
test('should add a new line when clicking the bottom of the last non-text block', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await initThreeParagraphs(page);
    await assertRichTexts(page, ['123', '456', '789']);
...
test('indent native multi-selection block', async ({ page }, testInfo) => {
    ...
    await assertTextSelection(page, from, to);
    expect(await getPageSnapshot(page, true)).toMatchSnapshot(`${testInfo.title}_after_tab.json`);
    await setSelection(page, 3, 1, 5, 1);
    await assertTextSelection(page, from, to);
    await waitNextFrame(page);
    await pressShiftTab(page);
    // should restore selection
    await assertTextSelection(page, from, to);
    expect(await getPageSnapshot(page, true)).toMatchSnapshot(`${testInfo.title}_after_shift_tab.json`);
});
test('should clear native selection before block selection', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await initThreeParagraphs(page);
    await assertRichTexts(page, ['123', '456', '789']);
    await dragBetweenIndices(page, [1, 3], [1, 0], { x: 0, y: 0 }, { x: 0, y: 0 }, { steps: 20 });
...
// ↑
test('should keep native range selection when scrolling backward with the scroll wheel', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await initThreeParagraphs(page);
    await assertRichTexts(page, ['123', '456', '789']);
    ...
    const blockHeight = await page.evaluate(() => {
        ...
        const container = viewport.querySelector('affine-note .affine-block-children-container');
        if (!container) {
            throw new Error();
        }
    ...
    await dragBetweenIndices(page, [14, 3], [14, 0], { x: 0, y: 0 }, { x: 0, y: 0 }, {
        // dont release mouse
        beforeMouseUp: async () => {
...
// ↓
test('should keep native range selection when scrolling forward with the scroll wheel', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await initThreeParagraphs(page);
    await assertRichTexts(page, ['123', '456', '789']);
    ...
    await page.mouse.move(0, 0);
    await dragBetweenIndices(page, [0, 0], [0, 3], { x: 0, y: 0 }, { x: 0, y: 0 }, {
        // dont release mouse
        beforeMouseUp: async () => {
...
test('should not show toolbar of image on native selection', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    ...
    await type(page, '123');
    await page.mouse.click(0, 0);
    await dragBetweenIndices(page, [0, 1], [0, 0], { x: 0, y: 0 }, { x: -40, y: 0 });
    await waitNextFrame(page);
    await copyByKeyboard(page);
    assertClipItems(page, 'text/plain', '123');
    await page.mouse.click(0, 0);
    await dragBetweenIndices(page, [0, 1], [0, 0], { x: 0, y: 0 }, { x: -40, y: -100 });
    await waitNextFrame(page);
    await copyByKeyboard(page);
    assertClipItems(page, 'text/plain', '123');
...
test('should collapse to end when press arrow-right on multi-line selection', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await initThreeParagraphs(page);
    await assertRichTexts(page, ['123', '456', '789']);
...
test('should collapse to start when press arrow-left on multi-line selection', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await initThreeParagraphs(page);
    await assertRichTexts(page, ['123', '456', '789']);
...
test('should select when clicking on blank area in edgeless mode', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    const ids = await initEmptyEdgelessState(page);
    await initThreeParagraphs(page);
    await assertRichTexts(page, ['123', '456', '789']);
...
test('press ArrowLeft in the start of first paragraph should not focus on title', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await focusRichText(page, 0);
    await type(page, '123');
...
test('scroll vertically when inputting long text in a block', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await focusRichText(page);
    for (let i = 0; i < 40; i++) {
...
test('auto-scroll when creating a new paragraph-block by pressing enter', async ({ page, }) => {
    test.info().annotations.push({
        type: 'issue',
        description: 'https://github.com/toeverything/blocksuite/issues/4547',
    });
...
test('unexpected scroll when clicking padding area', async ({ page }) => {
    ...
    await page.mouse.click(databaseRect.x + databaseRect.width + 10, databaseRect.y + 100);
