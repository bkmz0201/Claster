...
import { getFirstBlockCommand, getLastBlockCommand, getSelectedBlocksCommand, } from '@blocksuite/affine/shared/commands';
import { DocModeProvider, EditPropsStore, NotificationProvider, TelemetryProvider, } from '@blocksuite/affine/shared/services';
import {} from '@blocksuite/affine/std';
import { GfxControllerIdentifier } from '@blocksuite/affine/std/gfx';
import { BlockIcon, EdgelessIcon, InsertBleowIcon as InsertBelowIcon, LinkedPageIcon, PageIcon, } from '@blocksuite/icons/lit';
import { insertFromMarkdown } from '../../utils';
import { AIProvider } from '../provider';
import { reportResponse } from '../utils/action-reporter';
import { insertBelow } from '../utils/editor-actions';
export async function queryHistoryMessages(workspaceId, forkSessionId, docId) {
    // Get fork session messages
    const histories = await AIProvider.histories?.chats(workspaceId, forkSessionId, docId);
...
export function constructUserInfoWithMessages(messages, userInfo) {
...
export async function constructRootChatBlockMessages(doc, forkSessionId) {
    // Convert chat messages to AI chat block messages
    const userInfo = await AIProvider.userInfo;
    const forkMessages = (await queryHistoryMessages(doc.workspace.id, forkSessionId, doc.id));
    return constructUserInfoWithMessages(forkMessages, userInfo);
}
function getViewportCenter(mode, std) {
...
function addAIChatBlock(host, messages, sessionId, viewportCenter, index) {
    ...
    const aiChatBlockId = store.addBlock('affine:embed-ai-chat', {
 ...
 */
async function getInsertBelowBlock(host, currentSelections) {
    ...
    const [_, { selectedBlocks: blocks }] = host.command.exec(getSelectedBlocksCommand, {
 ...
 */
async function insertBelowBlock(host, content, block) {
    if (!block)
        return false;
...
export const PAGE_INSERT = {
    ...
    handler: async (host, content, currentSelections) => {
        ...
        if (isNothingSelected) {
            const [_, { firstBlock: noteBlock }] = host.command.exec(getFirstBlockCommand, {
                flavour: 'affine:note',
            });
...
export const EDGELESS_INSERT = {
    ...
    handler: async (host, content, currentSelections) => {
        ...
        if (isNothingSelected) {
            ...
            const isOnlyOneNoteSelected = selectedElements.length === 1 &&
                selectedElements[0] instanceof NoteBlockModel;
            if (isOnlyOneNoteSelected) {
                // Insert into selected note
                const [_, { lastBlock: lastBlockModel }] = host.command.exec(getLastBlockCommand, {
                    root: selectedElements[0],
                });
...
const SAVE_AS_BLOCK = {
    ...
    handler: async (host, _, __, chatSessionId, messageId) => {
        ...
        if (curMode !== 'edgeless') {
            ...
            docModeService.setEditorMode('edgeless');
            // Notify user to switch to edgeless mode
            notificationService?.notify({
                title: 'Save chat to a block',
                accent: 'info',
                message: 'This feature is not available in the page editor. Switch to edgeless mode.',
                onClose: function () { },
            });
        }
        try {
            ...
            const messages = await constructRootChatBlockMessages(host.store, newSessionId);
            // After switching to edgeless mode, the user can save the chat to a block
            const blockId = addAIChatBlock(host, messages, newSessionId, viewportCenter, newBlockIndex);
            if (!blockId) {
                return false;
            }
...
const ADD_TO_EDGELESS_AS_NOTE = {
    ...
    handler: async (host, content) => {
        ...
        if (elements.length > 0) {
            const bound = getCommonBoundWithRotation(elements.map(e => Bound.deserialize(e.xywh)));
...
export const SAVE_AS_DOC = {
    ...
    handler: (host, content) => {
        ...
        (function addContent() {
            ...
            complete = true;
            const { store } = newHost;
            insertFromMarkdown(newHost, content, store, noteId, 0).catch(console.error);
        })();
        return true;
    },
};
const CREATE_AS_LINKED_DOC = {
    ...
    handler: async (host, content) => {
        ...
        host.std.get(EdgelessCRUDIdentifier).addBlock('affine:embed-linked-doc', {
...
//# sourceMappingURL=chat-actions-handle.js.map