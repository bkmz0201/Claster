...
export const TabItem = ({ id, label, children, onClick }) => {
    const globalCache = useService(GlobalCacheService).globalCache;
    const activeTabId$ = useMemo(() => LiveData.from(globalCache.watch(cacheKey), 'home'), [globalCache]);
    ...
    return (_jsx("li", { className: tabItem, role: "tab", "aria-label": label, "data-active": isActive, onClick: handleClick, children: children }));
};
