...
import { pressArrowDown, pressArrowUp, pressBackspace, pressEnter, pressEscape, pressShiftTab, pressTab, redoByKeyboard, SHORT_KEY, type, undoByKeyboard, } from './utils/actions/keyboard.js';
import { captureHistory, enterPlaygroundRoom, focusRichText, getPageSnapshot, initEmptyEdgelessState, initEmptyParagraphState, resetHistory, waitNextFrame, } from './utils/actions/misc.js';
import { assertBlockChildrenIds, assertBlockCount, assertBlockFlavour, assertBlockSelections, assertKeyboardWorkInInput, assertParentBlockFlavour, assertRichImage, assertRichTextInlineRange, } from './utils/asserts.js';
import { test } from './utils/playwright.js';
const FILE_NAME = 'test-card-1.png';
const __dirname = path.dirname(fileURLToPath(import.meta.url));
const FILE_PATH = path.resolve(__dirname, `../../../blocksuite/playground/public/${FILE_NAME}`);
function getAttachment(page) {
    ...
    const getName = () => attachment.locator('.affine-attachment-content-title-text').innerText();
    return {
        // locators
        attachment,
        toolbar,
        ...
        getName,
        getSize: () => attachment.locator('.affine-attachment-content-info').innerText(),
        turnToEmbed: async () => {
...
test('can insert attachment from slash menu', async ({ page }, testInfo) => {
    ...
    const { insertAttachment, waitLoading, getName, getSize } = getAttachment(page);
...
test('should rename attachment works', async ({ page }) => {
    ...
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    const { attachment, renameBtn, renameInput, insertAttachment, waitLoading, getName, rename, } = getAttachment(page);
    await focusRichText(page);
    await insertAttachment();
    // Wait for the attachment to be uploaded
    await waitLoading();
...
test('should turn attachment to image works', async ({ page }, testInfo) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    const { attachment, insertAttachment, waitLoading, turnToEmbed, turnImageToCard, } = getAttachment(page);
    await focusRichText(page);
    await insertAttachment();
    // Wait for the attachment to be uploaded
    await waitLoading();
...
test(`support dragging attachment block directly`, async ({ page, }, testInfo) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    const { insertAttachment, waitLoading, getName, getSize } = getAttachment(page);
    await focusRichText(page);
...
test('press backspace after attachment block can select attachment block', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    const { insertAttachment, waitLoading } = getAttachment(page);
    await focusRichText(page);
...
test('attachment can be dragged from note to surface top level block', async ({ page, }) => {
    await enterPlaygroundRoom(page);
