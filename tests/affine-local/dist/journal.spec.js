...
function getJournalRow(page) {
    return page.locator('[data-testid="doc-property-row"][data-info-id="journal"]');
}
async function isJournalEditor(page, maybeDate) {
    // journal header
...
async function openPagePropertiesAndAddJournal(page) {
...
async function toggleJournal(row, value) {
...
async function createPageAndTurnIntoJournal(page) {
...
test('Should show duplicated tag when create journal on same day', async ({ page, }) => {
    await openHomePage(page);
    await createPageAndTurnIntoJournal(page);
