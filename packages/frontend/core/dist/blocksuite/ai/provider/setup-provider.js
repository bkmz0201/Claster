...
import { ContextCategories, } from '@affine/graphql';
import { z } from 'zod';
...
const filterStyleToPromptName = new Map(Object.entries({
...
const processTypeToPromptName = new Map(Object.entries({
    Clearer: 'Upscale image',
    'Remove background': 'Remove background',
    'Convert to sticker': 'Convert to sticker',
}));
export function setupAIProvider(client, globalDialogService, authService) {
    async function createSession({ promptName, workspaceId, docId, sessionId, retry, pinned, reuseLatestChat, }) {
        if (sessionId)
            return sessionId;
        if (retry)
    ...
    const accountSubscription = authService.session.account$.subscribe(account => {
...
Could you make a new website based on these notes and send back just the html file?`;
    ...
    AIProvider.provide('filterImage', async (options) => {
        // test to image
        const promptName = filterStyleToPromptName.get(options.style);
        if (!promptName) {
    ...
    AIProvider.provide('processImage', async (options) => {
        // test to image
        const promptName = processTypeToPromptName.get(options.type);
        if (!promptName) {
    ...
    AIProvider.provide('context', {
        ...
        removeContextTag: async (options) => {
        ...
        getContextDocsAndFiles: async (workspaceId, sessionId, contextId) => {
            return client.getContextDocsAndFiles(workspaceId, sessionId, contextId);
        },
        pollContextDocsAndFiles: async (workspaceId, sessionId, contextId, onPoll, abortSignal) => {
            const poll = async () => {
                const result = await client.getContextDocsAndFiles(workspaceId, sessionId, contextId);
                onPoll(result);
            };
            let attempts = 0;
            ...
            while (!abortSignal.aborted) {
                await poll();
                const interval = Math.min(MIN_INTERVAL * Math.pow(1.5, attempts), MAX_INTERVAL);
                attempts++;
                await new Promise(resolve => setTimeout(resolve, interval));
            }
        },
        pollEmbeddingStatus: async (workspaceId, onPoll, abortSignal) => {
            const poll = async () => {
        ...
        matchContext: async (content, contextId, workspaceId, limit, scopedThreshold, threshold) => {
            return client.matchContext(content, contextId, workspaceId, limit, scopedThreshold, threshold);
        },
        applyDocUpdates: async (workspaceId, docId, op, updates) => {
            return client.applyDocUpdates(workspaceId, docId, op, updates);
        },
        addContextBlob: async (options) => {
    ...
    AIProvider.provide('histories', {
        actions: async (workspaceId, docId) => {
            // @ts-expect-error - 'action' is missing in server impl
            return ((await client.getHistories(workspaceId, {}, docId, {
                action: true,
                withPrompt: true,
                withMessages: true,
            })) ?? []);
        },
        chats: async (workspaceId, sessionId, docId) => {
            // @ts-expect-error - 'action' is missing in server impl
            return ((await client.getHistories(workspaceId, {}, docId, {
                sessionId,
                withMessages: true,
        ...
        cleanup: async (workspaceId, docId, sessionIds) => {
            await client.cleanupSessions({ workspaceId, docId, sessionIds });
        },
        ids: async (workspaceId, docId, options) => {
            // @ts-expect-error - 'action' is missing in server impl
    ...
    AIProvider.provide('photoEngine', {
        async searchImages(options) {
            ...
            const result = await client.fetcher(url.toString()).then(res => res.json());
            if (!result.results)
                return [];
