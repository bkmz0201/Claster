...
import { enterPlaygroundRoom, initEmptyEdgelessState, initSixParagraphs, initThreeParagraphs, selectNoteInEdgeless, switchEditorMode, triggerComponentToolbarAction, } from '../../utils/actions/index.js';
...
test.describe('note slicer', () => {
    ...
    test('note slicer button should appears at right position', async ({ page, }) => {
        await enterPlaygroundRoom(page);
        const { noteId } = await initEmptyEdgelessState(page);
        ...
        await page.mouse.move(secondBlockRect.x + 1, secondBlockRect.y + secondBlockRect.height / 2);
        ...
        expect(buttonRectMiddle).toBeGreaterThan(firstBlockRect.y + firstBlockRect.height);
        ...
        await page.mouse.move(thirdBlockRect.x + 1, thirdBlockRect.y + thirdBlockRect.height / 2);
        ...
        expect(buttonRectMiddle).toBeGreaterThan(secondBlockRect.y + secondBlockRect.height);
        expect(buttonRectMiddle).toBeLessThan(thirdBlockRect.y);
    });
    test('note slicer button should appears at right position when editor is not located at left top corner', async ({ page, }) => {
        await enterPlaygroundRoom(page);
        const { noteId } = await initEmptyEdgelessState(page);
        await initThreeParagraphs(page);
        await assertRichTexts(page, ['123', '456', '789']);
