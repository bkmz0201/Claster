...
import { clickEdgelessModeButton, clickView, dblclickView, dragView, locateEditorContainer, locateToolbar, setEdgelessTool, } from '@affine-test/kit/utils/editor';
import { openHomePage } from '@affine-test/kit/utils/load-page';
import { clickNewPageButton, switchEdgelessTheme, waitForEditorLoad, } from '@affine-test/kit/utils/page-logic';
...
test('should correct text color on toolbar', async ({ page }) => {
    ...
    await expect(page.locator('edgeless-connector-label-editor rich-text')).toBeVisible();
    await page.keyboard.type('label');
    await page.keyboard.press('Escape');
    const toolbar = locateToolbar(page);
    const textColorContainer = toolbar.locator('edgeless-color-picker-button.text-color');
