...
import { ImageBlockModel, NoteBlockModel, NoteDisplayMode, } from '@blocksuite/affine/model';
import { isInsideEdgelessEditor, matchModels, } from '@blocksuite/affine/shared/utils';
import { GfxControllerIdentifier } from '@blocksuite/affine/std/gfx';
import { ThemeProvider } from '@blocksuite/affine-shared/services';
import { ChatWithAiIcon, DeleteIcon, InsertBleowIcon as InsertBelowIcon, InsertTopIcon, PageIcon, PenIcon, ReplaceIcon, ResetIcon, } from '@blocksuite/icons/lit';
import { insertFromMarkdown } from '../utils';
import { AIStarIconWithAnimation } from './_common/icons';
import { EXCLUDING_REPLACE_ACTIONS, INSERT_ABOVE_ACTIONS, } from './actions/consts';
import { pageResponseHandler, replaceWithMarkdown, } from './actions/page-response';
        ...
        handler: () => {
            ...
            doc.transact(() => {
                ...
                insertFromMarkdown(host, panel.answer, doc, noteBlockId)
                    ...
                    if (!newNote || !matchModels(newNote, [NoteBlockModel]))
                        return;
...
function buildPageResponseConfig(panel, id, ctx) {
    ...
    return [
        {
            ...
            items: [
                ...
                asCaption(host, id),
                {
                    ...
                    icon: ReplaceIcon(),
                    showWhen: () => !!panel.answer && !EXCLUDING_REPLACE_ACTIONS.includes(id),
                    handler: () => {
...
export function buildErrorResponseConfig(panel) {
    return [
        {
            name: '',
            items: [
...
export function buildFinishConfig(panel, id, ctx) {
    return {
        responses: buildPageResponseConfig(panel, id, ctx),
        actions: [],
    };
}
export function buildErrorConfig(panel) {
    return {
        upgrade: () => {
...
export function buildAIPanelConfig(panel, framework) {
