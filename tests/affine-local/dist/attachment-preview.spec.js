...
import { clickNewPageButton, getBlockSuiteEditorTitle, waitForEditorLoad, waitForEmptyEditor, } from '@affine-test/kit/utils/page-logic';
...
async function clickPeekViewControl(page, n = 0) {
    await page.getByTestId('peek-view-control').nth(n).click();
    await page.waitForTimeout(500);
}
async function enablePDFEmbedView(page) {
...
test('should sync name in pdf embed view', async ({ page }) => {
    ...
    const toolbar = locateToolbar(page);
    await expect(toolbar).toBeVisible();
    const attachmentTitle = attachment.locator('.affine-attachment-content-title-text');
...
test('should enable pointer event in pdf viewer', async ({ page }) => {
    ...
    const attachment = page.locator('affine-attachment');
    await attachment.click();
    const attachmentSelection = attachment.locator('.affine-attachment-container.focused');
    const toolbar = locateToolbar(page);
    // Switches to embed view
    await toolbar.getByLabel('Switch view').click();
    await toolbar.getByLabel('Embed view').click();
    ...
    await page.mouse.move(endPoint[0] + width / 2 + 10, endPoint[1] + height / 2 + 10);
...
test('should display status when an error is thrown in peek view', async ({ context, page, }) => {
    await openHomePage(page);
    await clickNewPageButton(page);
    await waitForEmptyEditor(page);
    const title = getBlockSuiteEditorTitle(page);
...
//# sourceMappingURL=attachment-preview.spec.js.map