...
import { pressEnter, selectAllByKeyboard, type, } from '../utils/actions/keyboard.js';
import { getBoundingBox, getEditorLocator, waitNextFrame, } from '../utils/actions/misc.js';
...
export async function initDatabaseColumn(page, title = '') {
...
export function getDatabaseCell(page, { rowIndex, columnType, columnIndex, }) {
...
export const getDatabaseColumnCells = (page, columnIndex) => {
    return page.locator(`dv-table-view-cell-container[data-column-index="${columnIndex}"]`);
};
export async function clickSelectOption(page, index = 0) {
    await page.getByTestId('option-more').nth(index).click();
}
export async function performSelectColumnTagAction(page, name, index = 0) {
    await clickSelectOption(page, index);
    await page
        .locator('.affine-menu-button', { hasText: new RegExp(name) })
        .click();
}
export async function assertSelectedStyle(page, key, value) {
    const style = await getElementStyle(page, '.select-selected', key);
    expect(style).toBe(value);
}
export async function clickDatabaseOutside(page) {
    const docTitle = page.locator('.doc-title-container');
    await docTitle.click();
}
export async function assertColumnWidth(locator, width) {
    const box = await getBoundingBox(locator);
    expect(box.width).toBe(width + 1);
    return box;
}
export async function assertDatabaseCellRichTexts(page, { rowIndex = 0, columnIndex = 1, text, }) {
    const cellContainer = page.locator(`dv-table-view-cell-container[data-row-index='${rowIndex}'][data-column-index='${columnIndex}']`);
    const cell = cellContainer.locator('affine-database-rich-text-cell');
    const actualTexts = await cell.evaluate(ele => {
        return ele.inlineEditor$.value?.yTextString;
    });
    expect(actualTexts).toEqual(text);
}
export async function assertDatabaseCellNumber(page, { rowIndex = 0, columnIndex = 1, text, }) {
...
export async function assertDatabaseCellLink(page, { rowIndex = 0, columnIndex = 1, text, }) {
    const actualTexts = await page.evaluate(({ rowIndex, columnIndex }) => {
        const rows = document.querySelector('.affine-database-block-rows');
        const row = rows?.querySelector(`.database-row:nth-child(${rowIndex + 1})`);
        const cell = row?.querySelector(`.database-cell:nth-child(${columnIndex + 1})`);
        const richText = cell?.querySelector('affine-database-link-cell');
        if (!richText)
            throw new Error('Missing database rich text cell');
        return richText.inlineEditor$.value.yText.toString();
    }, { rowIndex, columnIndex });
    expect(actualTexts).toEqual(text);
}
export async function assertDatabaseTitleText(page, text) {
    const dbTitle = page.locator('[data-block-is-database-title="true"]');
    expect(await dbTitle.inputValue()).toEqual(text);
}
export async function waitSearchTransitionEnd(page) {
    await waitNextFrame(page, 400);
}
export async function assertDatabaseSearching(page, isSearching) {
...
export async function focusDatabaseSearch(page) {
...
export async function blurDatabaseSearch(page) {
    const dbTitle = page.locator('[data-block-is-database-title="true"]');
    await dbTitle.click();
}
export async function focusDatabaseHeader(page, columnIndex = 0) {
...
export async function getDatabaseMouse(page) {
    const databaseRect = await page.getByTestId('dv-table-view').boundingBox();
    if (!databaseRect)
        throw new Error('Cannot find database rect');
    return {
        mouseOver: async () => {
...
export async function getDatabaseHeaderColumn(page, index = 0) {
...
export async function assertRowsSelection(page, rowIndexes) {
...
export async function assertCellsSelection(page, cellIndexes) {
...
export async function getElementStyle(page, selector, key) {
    const style = await page.evaluate(({ key, selector }) => {
        const el = document.querySelector(selector);
        if (!el)
            throw new Error(`Missing ${selector} tag`);
        // @ts-ignore
        return el.style[key];
    }, {
        key,
...
export async function focusKanbanCardHeader(page, index = 0) {
    const cardHeader = page.locator('data-view-header-area-text').nth(index);
    await cardHeader.click();
}
export async function clickKanbanCardHeader(page, index = 0) {
    const cardHeader = page.locator('data-view-header-area-text').nth(index);
    await cardHeader.click();
    await pressEnter(page);
}
export async function assertKanbanCardHeaderText(page, text, index = 0) {
    const cardHeader = page.locator('data-view-header-area-text').nth(index);
    await expect(cardHeader).toHaveText(text);
}
export async function assertKanbanCellSelected(page, { groupIndex, cardIndex, cellIndex, }) {
    const border = await page.evaluate(({ groupIndex, cardIndex, cellIndex }) => {
        const group = document.querySelector(`affine-data-view-kanban-group:nth-child(${groupIndex + 1})`);
        const card = group?.querySelector(`affine-data-view-kanban-card:nth-child(${cardIndex + 1})`);
        const cells = Array.from(card?.querySelectorAll(`affine-data-view-kanban-cell`) ??
            []);
        const cell = cells[cellIndex];
        if (!cell)
            throw new Error(`Missing cell tag`);
...
export async function assertKanbanCardSelected(page, { groupIndex, cardIndex, }) {
    const border = await page.evaluate(({ groupIndex, cardIndex }) => {
        const group = document.querySelector(`affine-data-view-kanban-group:nth-child(${groupIndex + 1})`);
        const card = group?.querySelector(`affine-data-view-kanban-card:nth-child(${cardIndex + 1})`);
        if (!card)
...
export function getKanbanCard(page, { groupIndex, cardIndex, }) {
...
export const moveToCenterOf = async (page, locator) => {
    const box = (await locator.boundingBox());
    expect(box).toBeDefined();
    await page.mouse.move(box.x + box.width / 2, box.y + box.height / 2);
};
export const changeColumnType = async (page, column, name) => {
    await waitNextFrame(page);
...
export const pressKey = async (page, key, count = 1) => {
