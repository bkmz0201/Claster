...
test.describe('embed-synced-doc render', () => {
    ...
    test('nested embed synced doc should be rendered as card when depth >=1', async ({ page, }) => {
    ...
    test('synced doc should be readonly', async ({ page }) => {
        ...
        await page.evaluate(async (embedDocId) => {
            ...
            const databaseId = doc2.addBlock('affine:database', {
                title: new window.$blocksuite.store.Text('Database 1'),
            }, noteId);
            const model = doc2.getModelById(databaseId);
        ...
        const databaseFirstCell = page.locator('.affine-database-column-header.database-row');
