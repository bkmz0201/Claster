...
import { buildShowcaseWorkspace, createFirstAppData, } from '@affine/core/utils/first-app-data';
 ...
 */
export const Component = ({ defaultIndexRoute = 'all', children, fallback, }) => {
    // navigating and creating may be slow, to avoid flickering, we show workspace fallback
    const [navigating, setNavigating] = useState(true);
    const [creating, setCreating] = useState(false);
    const authService = useService(AuthService);
    const defaultServerService = useService(DefaultServerService);
    const loggedIn = useLiveData(authService.session.status$.map(s => s === 'authenticated'));
    ...
    useLayoutEffect(() => {
        ...
        if (searchParams.get('initCloud') === 'true') {
            if (loggedIn) {
                ...
                const openWorkspace = list.find(w => w.flavour === 'affine-cloud') ?? list[0];
    ...
    if (navigating || creating) {
        return fallback ?? _jsx(AppContainer, { fallback: true });
    }
    // TODO(@eyhn): We need a no workspace page
    return (children ?? (_jsx("div", { style: {
            position: 'fixed',
            left: 'calc(50% - 150px)',
            top: '50%',
        }, children: _jsx(WorkspaceNavigator, { open: true, menuContentOptions: {
                forceMount: true,
            } }) })));
};
