...
const createPointerEventState = (button = MouseButton.MIDDLE) => ({
    raw: {
        button,
        preventDefault: vi.fn(),
    },
});
const createPanToolHarness = (toolName = 'default', options) => {
    ...
    const originalToolType = { toolName };
    ...
    globalThis.document ??= {};
    const panTool = new PanTool(gfx);
    let pointerDownHandler = null;
    panTool.eventTarget = {
        addHook: (_eventName, handler) => {
...
describe('PanTool middle mouse behavior', () => {
    ...
    test('temporarily switches to pan tool and restores original selection snapshot', () => {
        ...
        const { pointerDownHandler, selection, selectionEntry, originalToolType, originalToolOptions, setTool, } = createPanToolHarness('default', originalOptions);
        ...
        const middlePointerUpHandler = pointerUpHandlers[0];
        middlePointerUpHandler({ button: MouseButton.MIDDLE });
        expect(selection.set).toHaveBeenCalledWith([selectionEntry]);
        expect(setTool).toHaveBeenNthCalledWith(2, originalToolType, originalToolOptions);
    ...
    test('restores frame navigator with restoredAfterPan flag', () => {
        ...
        const { pointerDownHandler, navigatorSettingUpdated, setTool, originalToolType, } = createPanToolHarness('frameNavigator', frameOptions);
        pointerDownHandler(createPointerEventState());
        expect(navigatorSettingUpdated.next).toHaveBeenCalledWith({
            blackBackground: false,
        });
        const frameNavigatorPointerUpHandler = pointerUpHandlers[0];
    ...
    test('ignores middle button when PanTool is already active', () => {
        const { pointerDownHandler, setTool } = createPanToolHarness(PanTool.toolName);
        const pointerState = createPointerEventState();
        const result = pointerDownHandler(pointerState);
...
//# sourceMappingURL=pan-tool-middle-mouse.spec.js.map