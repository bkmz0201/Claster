...
import { androidpublisher_v3, auth as google_auth, } from '@googleapis/androidpublisher';
export async function fetchVersionCode(applicationId) {
    ...
    if (process.env.GITHUB_OUTPUT) {
        execSync(`echo "EDIT_ID=${appEdit.data.id}" >> ${process.env.GITHUB_OUTPUT}`, {
            stdio: 'inherit',
        });
    }
    return versionCode;
...
const gradlePath = new Package('@affine/android').join('App/app/build.gradle').value;
...
fs.writeFileSync(gradlePath, gradleFile.replace(versionCodeRegexPattern, `$1 ${versionCode}`));
