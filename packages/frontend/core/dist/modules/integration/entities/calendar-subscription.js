...
export class CalendarSubscription extends Entity {
    constructor(store) {
        super();
        this.store = store;
        this.config$ = LiveData.from(this.store.watchSubscription(this.props.url), {});
        this.content$ = LiveData.from(this.store.watchSubscriptionCache(this.props.url), '');
        this.name$ = LiveData.computed(get => {
            ...
            try {
                ...
                return vCalendar.getFirstPropertyValue('x-wr-calname') || '';
            }
            catch {
                return '';
            }
        ...
        this.update = effect(throttleTime(30 * 1000), switchMap(() => fromPromise(async () => {
            ...
            return EMPTY;
        }), catchErrorInto(this.error$), onStart(() => this.loading$.setValue(true)), onComplete(() => this.loading$.setValue(false)))));
    }
    dispose() {
        super.dispose();
        this.update.reset();
