...
async function getLastMatchingTag(inputTag, tagFilter, tagTemplate) {
  ...
  const isVersion = semver__default.valid(inputVersionString) !== null;
  const isPrerelease2 = semver__default.prerelease(inputVersionString) !== null;
  ...
  if (!isPrerelease2 && isVersion) {
    tag = filteredTags.find((tag2) => {
      ...
      return versionString !== inputVersionString && semver__default.valid(versionString) !== null && semver__default.prerelease(versionString) === null;
    });
  }
...
async function sendRelease(options, content) {
  ...
  let method = "POST";
  try {
    const exists = await ofetch.$fetch(`https://${options.baseUrlApi}/repos/${options.releaseRepo}/releases/tags/${options.to}`, {
      headers
    });
    if (exists.url) {
      url = exists.url;
  ...
  console.log(
    ansis.cyan(method === "POST" ? "Creating release notes..." : "Updating release notes...")
  );
  ...
  console.log(ansis.green(`Released on ${res.html_url}`));
}
...
async function resolveAuthorInfo(options, info) {
  ...
  try {
    ...
    const data = await ofetch.$fetch(`https://${options.baseUrlApi}/search/users?q=${q}`, {
...
function parseCommits(commits, config) {
  return commits.map((commit) => changelogen.parseGitCommit(commit, config)).filter(notNullish);
}
...
exports.getSafeTagTemplate = getSafeTagTemplate;
