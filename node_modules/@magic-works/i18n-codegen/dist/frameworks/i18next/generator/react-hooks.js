...
import { createPropertyName, NUMBER_TYPE, createReadonlyType, printer, isIdent, castStatement, castExpression, stdlib, pureAnnotate, } from '../../../utils/typescript.js';
export function i18NextReactHooksGenerator(gen) {
    ...
    // hooks return type
    {
        ...
        for (const [key, detail] of items) {
            ...
            if (detail.interpolations.size) {
                params.push(factory.createParameterDeclaration(undefined, undefined, 'options', undefined, createInterpolationType(detail.interpolations)));
            }
            ...
            // key(...params): type
            const node = factory.createMethodSignature(undefined, createPropertyName(key), undefined, undefined, params, returnType);
    ...
    const hooksCode = [
        ...
        factory.createVariableStatement(undefined, factory.createVariableDeclarationList([
            factory.createVariableDeclaration(factory.createObjectBindingPattern([
                factory.createBindingElement(undefined, undefined, factory.createIdentifier('t'), undefined),
            ]), undefined, undefined, factory.createCallExpression(factory.createIdentifier('useTranslation'), undefined, gen.generatorOptions?.namespace
                ? [factory.createStringLiteral(gen.generatorOptions.namespace)]
                : [])),
        ], ts.NodeFlags.Const)),
        factory.createReturnStatement(gen.generatorOptions?.es6Proxy === false
    ...
    statements.push(factory.createFunctionDeclaration([factory.createModifier(ts.SyntaxKind.ExportKeyword)], undefined, factory.createIdentifier(gen.generatorOptions?.hooks || 'useTypedTranslation'), undefined, [], hooksReturnType, factory.createBlock(hooksCode, false)));
    ...
    if (htmlTags.size) {
        ...
        statements.push(...statements.splice(2, statements.length - 2, castStatement `
        ...
        for (const [key, detail] of htmlTags) {
            if (detail.type !== 'key')
                continue; // TODO
            const node = factory.createPropertySignature(undefined, createPropertyName(key), undefined, createTagType(createInterpolationType(detail.interpolations), [...detail.tags.keys()], detail.relatedContexts));
            const comment = getCommentForKey(key);
            comment && ts.addSyntheticLeadingComment(node, ts.SyntaxKind.MultiLineCommentTrivia, comment, true);
            elements.push(node);
        }
        statements.push(factory.createVariableStatement([factory.createModifier(ts.SyntaxKind.ExportKeyword)], factory.createVariableDeclarationList([
            factory.createVariableDeclaration(factory.createIdentifier(gen.generatorOptions?.trans || 'TypedTrans'), undefined, factory.createTypeLiteralNode(elements), gen.generatorOptions?.es6Proxy === false
                ? factory.createObjectLiteralExpression([...htmlTags]
                    .map(([k, r]) => {
                    if (r.type !== 'key')
                        return null;
                    return factory.createPropertyAssignment(createPropertyName(k), pureAnnotate(castExpression `createComponent(${JSON.stringify(k)})`));
                })
                    .filter(Boolean), true)
    ...
    const printedSourceFile = [
        ...
        .filter(Boolean)
        .join('\n');
    if (gen.generatorOptions?.emitTS)
        return new Map([[`${gen.outputBase}.ts`, printedSourceFile]]);
    const options = {
        declaration: true,
        strict: true,
        skipLibCheck: true,
    ...
    const host = {
        ...
        getSourceFile: (fileName, option) => {
            if (fileName.endsWith('.d.ts'))
                return stdlib(fileName);
            if (fs[fileName])
                return ts.createSourceFile(fileName, fs[fileName], option);
    ...
    if (!js || !dts) {
        console.log(ts.formatDiagnostics(result.diagnostics, {
            getCanonicalFileName: (x) => x,
            getCurrentDirectory: () => '/',
            getNewLine: () => '\n',
        }));
...
function createTagType(props, tagNames, context) {
    return factory.createTypeReferenceNode(factory.createIdentifier('ComponentType'), [
        factory.createTypeReferenceNode(factory.createIdentifier('TypedTransProps'), [
            props,
            factory.createTypeLiteralNode(tagNames.map((tag) => factory.createPropertySignature(undefined, createPropertyName(tag), undefined, factory.createTypeReferenceNode(factory.createQualifiedName(factory.createIdentifier('JSX'), factory.createIdentifier('Element')), undefined)))),
            context?.length
                ? factory.createUnionTypeNode(context.map((context) => factory.createLiteralTypeNode(factory.createStringLiteral(context))))
                : null,
        ].filter(Boolean)),
    ]);
}
function createInterpolationType(interpolations) {
    return createReadonlyType(factory.createTypeLiteralNode([...interpolations].map(([key, [, type, required]]) => factory.createPropertySignature(undefined, key, required ? undefined : factory.createToken(ts.SyntaxKind.QuestionToken), type))));
}
function getPropertyKeyTextFromText(text) {
    if (isIdent(text))
        return text;
    return `[${JSON.stringify(text)}]`;
}
function generateUseTKeys([k, r]) {
    if (r.type !== 'key')
        return null;
    if (r.tags.size)
        return null;
    ...
    if (r.interpolations.size)
        return `${prop}: x => t(${key}, x)`;
    return `${prop}: () => t(${key})`;
}
function getTopLevelKeys(x) {
    if (x.root.type !== 'object')
        throw new Error();
    const realNodes = x.root.items;
    const nodes = new Map(realNodes);
    const comments = new Map();
    function appendComment(key, content) {
        if (!comments.has(key))
            comments.set(key, []);
        ...
        for (const [_, detail] of details) {
            ...
            if (baseNode.type === 'key') {
                ...
                detail.interpolations.forEach((t, k) => {
                    if (k === 'count')
                        t[1] = NUMBER_TYPE;
                    if (!baseNode.interpolations.has(k))
                        baseNode.interpolations.set(k, t);
                    else {
    ...
    for (const [contextBase, contextNodes] of x.contexts) {
        ...
        if (baseNode.type === 'key') {
            ...
            baseNode.interpolations.set('context', [
                [null, null],
                factory.createUnionTypeNode(baseNode.relatedContexts.map((string) => factory.createLiteralTypeNode(factory.createStringLiteral(string)))),
                false,
            ]);
        }
    }
