...
import { EmbedFigmaBlockComponent, EmbedGithubBlockComponent, EmbedIframeBlockComponent, EmbedLoomBlockComponent, EmbedYoutubeBlockComponent, getDocContentWithMaxLength, } from '@blocksuite/affine/blocks/embed';
import { EmbedLinkedDocBlockComponent, EmbedSyncedDocBlockComponent, } from '@blocksuite/affine/blocks/embed-doc';
...
import { AffineReference, toggleReferencePopup, } from '@blocksuite/affine/inlines/reference';
import { BookmarkBlockModel, EmbedIframeBlockModel, EmbedLinkedDocModel, EmbedSyncedDocModel, SurfaceRefBlockSchema, } from '@blocksuite/affine/model';
import { getSelectedModelsCommand } from '@blocksuite/affine/shared/commands';
import { ImageSelection } from '@blocksuite/affine/shared/selection';
import { ActionPlacement, CitationProvider, GenerateDocUrlProvider, isRemovedUserInfo, OpenDocExtensionIdentifier, ToolbarModuleExtension, UserProvider, } from '@blocksuite/affine/shared/services';
import { matchModels } from '@blocksuite/affine/shared/utils';
import { BlockFlavourIdentifier, BlockSelection, TextSelection, } from '@blocksuite/affine/std';
import { GfxBlockElementModel, GfxPrimitiveElementModel, } from '@blocksuite/affine/std/gfx';
import { CopyAsImgaeIcon, CopyIcon, EditIcon, LinkIcon, OpenInNewIcon, } from '@blocksuite/icons/lit';
    ...
    return {
        configure: (groups) => {
            ...
            if (clipboardGroup) {
                ...
                clipboardGroup.items.splice(copyIndex + 1, 0, createCopyAsPngMenuItem(framework));
            }
            return groups;
        },
    };
...
function createCopyLinkToBlockMenuItem(framework, item = {
    ...
    return {
        ...
        action: async (ctx) => {
            ...
            const options = { workspaceId, pageId, mode };
            let type = '';
            if (mode === 'page') {
            ...
            const success = await copyLinkToBlockStdScopeClipboard(str, ctx.std.clipboard);
            if (success) {
...
function createToolbarMoreMenuConfigV2(baseUrl) {
    return {
        actions: [
            {
                ...
                actions: [
                    {
                        ...
                        when: ({ isEdgelessMode, gfx, flags }) => !flags.isHovering() &&
                            isEdgelessMode &&
                    ...
                    },
                    {
                        ...
                        when: ({ isPageMode, selection, gfx, flags }) => {
                            ...
                            const items = selection
                                .getGroup('note')
                                .filter(item => [TextSelection, BlockSelection, ImageSelection].some(t => item.is(t)));
                        ...
                        run({ isPageMode, std, store, gfx, workspace, editorMode }) {
                            ...
                            let type = '';
                            if (isPageMode) {
                                const [ok, { selectedModels = [] }] = std.command.exec(getSelectedModelsCommand);
                                if (!ok || !selectedModels.length)
                                    return;
                ...
                actions: [
                    {
                        ...
                        when: ctx => {
                            ...
                            const hasBlockSelection = ctx.selection.filter(BlockSelection).length === 1;
                        ...
                        content: ctx => {
                            ...
                            if (!('props' in model))
                                return null;
                            const createdAt = 'meta:createdAt';
                            if (!(createdAt in model.props))
                                return null;
                            const createdBy = 'meta:createdBy';
                            if (!(createdBy in model.props))
                                return null;
                            const createdByUserId = model.props[createdBy];
                            const createdAtTimestamp = model.props[createdAt];
...
function createExternalLinkableToolbarConfig(klass) {
    return {
        actions: [
            {
                ...
                actions: [
                    ...
                    },
                    {
                        ...
                        run(ctx) {
                            ...
                            toggleEmbedCardEditModal(ctx.host, model, 'card', undefined, undefined, (_std, _component, props) => {
...
function createOpenDocActions(ctx, target, isSameDoc, actions = openDocActions.map(({ type: mode, label, icon, enabled: when, shortcut }, i) => ({
    mode,
    id: `${i}.${mode}`,
    label,
    icon,
    ...
    return actions
        ...
        return {
            ...
            generate(ctx) {
                ...
                const when = ctx.std.get(OpenDocExtensionIdentifier).isAllowed(openMode) &&
                    (shouldOpenInCenterPeek ? isPeekable(target) : true);
                const run = shouldOpenInCenterPeek
                    ? (_ctx) => peek(target)
                    : (_ctx) => target.open({ openMode });
...
function createOpenDocActionGroup(klass, settings) {
    return {
        ...
        content(ctx) {
            ...
            if (!block)
                return null;
            return renderOpenDocMenu(settings, ctx, block, block.model.props.pageId === ctx.store.id);
        },
    };
}
function createEdgelessOpenDocActionGroup(klass) {
    return {
        placement: ActionPlacement.More,
        id: 'Z.c.open-doc',
        generate(ctx) {
            ...
            if (!block)
                return null;
            const actions = createOpenDocActions(ctx, block, block.model.props.pageId === ctx.store.id).map(action => ({ ...action, ...action.generate(ctx) }));
            return { actions };
        },
    };
}
function createSurfaceRefToolbarConfig(baseUrl) {
    return {
        actions: [
            {
                ...
                content: ctx => {
                    ...
                    return html `${keyed(surfaceRefBlock, html `<editor-menu-button
              aria-label="Open"
              .contentPadding=${'8px'}
...
function renderOpenDocMenu(settings, ctx, target, isSameDoc) {
    ...
    if (!actions.length)
        return null;
    const openDocMode = computed(() => settings.settingSignal.value.openDocMode ?? 'open-in-active-view');
    const updateOpenDocMode = (mode) => settings.openDocMode.set(mode);
    return html `${keyed(target, html `
            ...
            actions: [
                ...
                },
                {
                    ...
                    run(ctx) {
                        ...
                        const abortController = new AbortController();
                        abortController.signal.onabort = () => ctx.show();
                        toggleEmbedCardEditModal(ctx.host, model, 'card', doc
                            ? {
            ...
            actions: [
                ...
                },
                {
                    ...
                    run(ctx) {
                        ...
                        // Clears
                        ctx.reset();
                        const { inlineEditor, selfInlineRange, docTitle, referenceInfo } = target;
                        if (!inlineEditor || !selfInlineRange)
                            return;
                        const abortController = new AbortController();
                        const popover = toggleReferencePopup(ctx.std, docTitle, referenceInfo, inlineEditor, selfInlineRange, abortController);
            ...
            actions: [
                {
                    ...
                    icon: CopyIcon(),
                    run(ctx) {
                        const model = ctx.getCurrentBlockByType(EmbedIframeBlockComponent)?.model;
                        if (!model)
                            return;
                        const { url } = model.props;
                        navigator.clipboard.writeText(url).catch(console.error);
...
export const createCustomToolbarExtension = (settings, baseUrl) => {
    return [
                ...
                actions: [
                    {
                        ...
                        content(ctx) {
                            ...
                            if (!(target instanceof AffineReference))
                                return null;
                            return renderOpenDocMenu(settings, ctx, target, target.referenceInfo.pageId === ctx.store.id);
                        },
                    },
        ...
        ToolbarModuleExtension({
            id: BlockFlavourIdentifier(`custom:${SurfaceRefBlockSchema.model.flavour}`),
