...
const TrashHeader = () => {
    const t = useI18n();
    return (_jsx(Header, { left: _jsxs("div", { className: styles.trashTitle, children: [_jsx(DeleteIcon, { className: styles.trashIcon }), t['com.affine.workspaceSubPath.trash']()] }) }));
};
export const TrashPage = () => {
    ...
    const handleMultiRestore = useCallback((ids) => {
        ids.forEach(id => {
            restoreFromTrash(id);
        });
    ...
    const handleMultiDelete = useCallback((ids) => {
        ids.forEach(pageId => {
            permanentlyDeletePage(pageId);
        });
        toast(t['com.affine.toastMessage.permanentlyDeleted']());
    }, [permanentlyDeletePage, t]);
    const onConfirmPermanentlyDelete = useCallback((ids, callbacks) => {
        if (ids.length === 0) {
            return;
        }
        openConfirmModal({
    ...
    return (_jsxs(DocExplorerContext.Provider, { value: explorerContextValue, children: [_jsx(ViewTitle, { title: t['Trash']() }), _jsx(ViewIcon, { icon: 'trash' }), _jsx(ViewHeader, { children: _jsx(TrashHeader, {}) }), _jsx(ViewBody, { children: _jsx("div", { className: styles.body, children: isEmpty ? (_jsx(EmptyPageList, { type: "trash" })) : (_jsx(DocsExplorer, { disableMultiDelete: !isAdmin && !isOwner, onRestore: isAdmin || isOwner ? handleMultiRestore : undefined, onDelete: isAdmin || isOwner ? onConfirmPermanentlyDelete : undefined })) }) })] }));
};
