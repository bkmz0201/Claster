...
async function computeNodeModuleFileSets(platformPackager, mainMatcher) {
    ...
    const pm = (0, node_module_collector_1.detectPackageManager)(appDir === projectDir ? [appDir] : [appDir, projectDir]);
    let deps = await (0, node_module_collector_1.getNodeModules)(pm, appDir, platformPackager.info.tempDirManager);
    if (projectDir !== appDir && deps.length === 0) {
        const packageJson = require(path.join(appDir, "package.json"));
        if (Object.keys(packageJson.dependencies || {}).length > 0) {
            builder_util_1.log.debug({ projectDir, appDir }, "no node_modules in app dir, trying to find in project dir");
            deps = await (0, node_module_collector_1.getNodeModules)(pm, projectDir, platformPackager.info.tempDirManager);
        }
    }
