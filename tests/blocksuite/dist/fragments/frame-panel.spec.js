...
import { addBasicShapeElement, addNote, createNote, createShapeElement, dragBetweenViewCoords, edgelessCommonSetup, enterPresentationMode, getZoomLevel, setEdgelessTool, Shape, switchEditorMode, toggleFramePanel, } from '../utils/actions/edgeless.js';
import { waitNextFrame } from '../utils/actions/index.js';
import { assertEdgelessNonSelectedRect, assertEdgelessSelectedRect, assertZoomLevel, } from '../utils/asserts.js';
import { test } from '../utils/playwright.js';
async function dragFrameCard(page, fromCard, toCard, direction = 'down') {
    ...
    await page.mouse.move(toRect.x + center.x + offset.x, toRect.y + center.y + offset.y, { steps: 10 });
    await page.mouse.up();
}
test.describe('frame panel', () => {
    ...
    test('should display frame cards when there are frames', async ({ page }) => {
        await edgelessCommonSetup(page);
        await toggleFramePanel(page);
        await addBasicShapeElement(page, { x: 300, y: 300 }, { x: 350, y: 350 }, Shape.Square);
    ...
    test('should render edgeless note correctly in frame preview', async ({ page, }) => {
        await edgelessCommonSetup(page);
        await toggleFramePanel(page);
        await addNote(page, 'hello', 150, 500);
        await page.mouse.click(0, 0);
    ...
    test.describe('frame panel behavior after mode switch', () => {
        async function setupFrameTest(page) {
            await edgelessCommonSetup(page);
            await toggleFramePanel(page);
            await addNote(page, 'hello', 150, 500);
            await page.mouse.click(0, 0);
            await waitNextFrame(page, 100);
            await setEdgelessTool(page, 'frame');
            await dragBetweenCoords(page, { x: 100, y: 440 }, { x: 640, y: 600 }, { steps: 10 });
            await waitNextFrame(page, 100);
            const edgelessNote = page.locator('affine-frame-card affine-edgeless-note');
            expect(await edgelessNote.count()).toBe(1);
            return edgelessNote;
        }
        test('should render edgeless note correctly after mode switch', async ({ page, }) => {
            ...
            const { width: noteWidth, height: noteHeight, x: noteX, y: noteY, } = initialNoteRect;
        ...
        test('should update frame preview when note is moved', async ({ page }) => {
            ...
            await switchEditorMode(page);
            await switchEditorMode(page);
            async function moveNoteAndCheck(start, end, comparison) {
            ...
            await moveNoteAndCheck({ x: 150, y: 500 }, { x: 200, y: 550 }, 'greaterThan');
            // Move the note back to the left
            await moveNoteAndCheck({ x: 200, y: 550 }, { x: 100, y: 450 }, 'lessThan');
            // Move the note diagonally
            await moveNoteAndCheck({ x: 100, y: 450 }, { x: 250, y: 600 }, 'greaterThan');
        });
    });
    ...
    test('should fit the viewport to the frame when double click frame card', async ({ page, }) => {
        await edgelessCommonSetup(page);
        await toggleFramePanel(page);
    ...
    test('should reorder frames when drag and drop frame card', async ({ page, }) => {
        await edgelessCommonSetup(page);
        ...
        const edgelessNote = page.locator('affine-edgeless-root affine-edgeless-note');
        await expect(edgelessNote).toBeVisible();
    });
});
//# sourceMappingURL=frame-panel.spec.js.map