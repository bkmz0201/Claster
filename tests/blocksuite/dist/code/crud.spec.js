...
import { createCodeBlock, pressArrowLeft, pressArrowUp, pressBackspace, pressEnter, pressEscape, pressShiftTab, pressTab, redoByKeyboard, type, undoByKeyboard, } from '../utils/actions/keyboard.js';
import { enterPlaygroundRoom, focusRichText, focusRichTextEnd, getPageSnapshot, initEmptyCodeBlockState, initEmptyParagraphState, setSelection, waitNextFrame, } from '../utils/actions/misc.js';
import { assertBlockCount, assertRichTexts, assertTitle, } from '../utils/asserts.js';
...
test('use markdown syntax can create code block', async ({ page, }, testInfo) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await focusRichText(page);
    await type(page, 'aaa');
    ...
    //     ccc
    await type(page, '``` ');
    expect(await getPageSnapshot(page, true)).toMatchSnapshot(`${testInfo.title}_markdown_syntax.json`);
});
test('use markdown syntax with trailing characters can create code block', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await focusRichText(page);
...
test('use more than three backticks can not create code block', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await focusRichText(page);
    await type(page, '`````');
...
test('change code language can work', async ({ page }, testInfo) => {
    ...
    await page.click('.affine-filterable-list > .items-container > icon-button:nth-child(1)');
...
test('toggle code block wrap can work', async ({ page }, testInfo) => {
    ...
    expect(await getPageSnapshot(page, true)).toMatchSnapshot(`${testInfo.title}_3.json`);
});
...
test('code block toolbar widget can appear and disappear during mousemove', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyCodeBlockState(page);
    await focusRichText(page);
    const position = await page.locator('affine-code').boundingBox();
    if (!position)
        throw new Error('Failed to get affine code position');
    await page.mouse.move(position.x, position.y);
    const locator = page.locator('.code-toolbar-container');
    const toolbarPosition = await locator.boundingBox();
    if (!toolbarPosition)
        throw new Error('Failed to get option position');
...
test('should open more menu and do not close after selecting', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyCodeBlockState(page);
    await focusRichText(page);
    const codeBlockController = getCodeBlock(page);
...
test('language selection list should not close when hovering out of code block', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyCodeBlockState(page, { language: 'javascript' });
    const codeBlockController = getCodeBlock(page);
    await codeBlockController.codeBlock.hover();
...
test('language selection list should not change when hovering over its elements', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyCodeBlockState(page);
    const codeBlockController = getCodeBlock(page);
    await codeBlockController.codeBlock.hover();
...
test('format text in code block', async ({ page }, testInfo) => {
    ...
    await boldBtn.click();
    expect(await line.innerText()).toBe('const aaa = 1000;');
    expect(await getPageSnapshot(page, true)).toMatchSnapshot(`${testInfo.title}_format.json`);
    ...
    await expect(linkLocator.nth(0)).toHaveAttribute('href', 'https://www.baidu.com');
    await expect(linkLocator.nth(1)).toHaveAttribute('href', 'https://www.baidu.com');
    await expect(linkLocator.nth(2)).toHaveAttribute('href', 'https://www.baidu.com');
    expect(await getPageSnapshot(page, true)).toMatchSnapshot(`${testInfo.title}_link.json`);
});
