...
export const AISubscribe = ({ displayedFrequency = 'yearly', ...btnProps }) => {
    ...
    if (!price || !price.yearlyAmount) {
        return (_jsx(Skeleton, { className: btnProps.className, width: 160, height: 36, style: {
                borderRadius: 18,
                ...btnProps.style,
            } }));
    }
    const priceReadable = `$${(price.yearlyAmount /
        100 /
        (displayedFrequency === 'yearly' ? 1 : 12)).toFixed(2)}`;
    const priceFrequency = displayedFrequency === 'yearly'
        ? t['com.affine.payment.billing-setting.year']()
        : t['com.affine.payment.billing-setting.month']();
    return (_jsx(CheckoutSlot, { onBeforeCheckout: onBeforeCheckout, checkoutOptions: checkoutOptions, renderer: props => (_jsxs(Button, { variant: "primary", ...props, ...btnProps, "data-testid": "ai-subscribe-button", children: [btnProps.children ?? `${priceReadable} / ${priceFrequency}`, displayedFrequency === 'monthly' ? (_jsx("span", { style: {
                        fontSize: 10,
                        opacity: 0.75,
                        letterSpacing: -0.2,
                        paddingLeft: 4,
                    }, children: t['com.affine.payment.ai.subscribe.billed-annually']() })) : null] })) }));
};
