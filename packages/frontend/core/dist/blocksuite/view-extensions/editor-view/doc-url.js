...
import { GenerateDocUrlExtension, ParseDocUrlExtension, } from '@blocksuite/affine/shared/services';
function patchParseDocUrlExtension(framework) {
    ...
    const ParseDocUrl = ParseDocUrlExtension({
        parseDocUrl(url) {
            ...
            if (!info || info.workspaceId !== workspaceService.workspace.id)
                return;
...
function patchGenerateDocUrlExtension(framework) {
    const workspaceService = framework.get(WorkspaceService);
    const workspaceServerService = framework.get(WorkspaceServerService);
    const baseUrl = workspaceServerService.server?.baseUrl ?? location.origin;
...
export function patchDocUrlExtensions(framework) {
    return [
