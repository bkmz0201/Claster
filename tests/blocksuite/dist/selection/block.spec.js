...
import { assertAlmostEqual, assertBlockCount, assertRichTexts, } from '../utils/asserts.js';
...
test('select all should work for multiple notes in doc mode', async ({ page, }) => {
    const n = 4;
    await enterPlaygroundRoom(page);
...
async function clickListIcon(page, i = 0) {
...
test('click the list icon can select and delete by forwardDelete', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await initThreeLists(page);
    await assertRichTexts(page, ['123', '456', '789']);
...
// ↑
test('should keep selection state when scrolling backward', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await initThreeParagraphs(page);
    await assertRichTexts(page, ['123', '456', '789']);
    ...
    const [, container, distance] = await page.evaluate(() => {
        ...
        const container = viewport.querySelector('.affine-block-children-container');
        if (!container) {
            throw new Error();
        }
        return [
...
// ↑
test('should keep selection state when scrolling backward with the scroll wheel', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await initThreeParagraphs(page);
    await assertRichTexts(page, ['123', '456', '789']);
    ...
    const [last, distance] = await page.evaluate(() => {
        ...
        return [last.getBoundingClientRect(), distance];
    });
...
// ↓
test('should keep selection state when scrolling forward with the scroll wheel', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await initThreeParagraphs(page);
    await assertRichTexts(page, ['123', '456', '789']);
    ...
    const [first, distance] = await page.evaluate(() => {
        ...
        return [first.getBoundingClientRect(), distance];
    });
    await page.mouse.move(0, 0);
    await dragBetweenCoords(page, {
        x: first.left - 10,
...
test('should not clear selected rects when clicking on scrollbar', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await initThreeParagraphs(page);
    await assertRichTexts(page, ['123', '456', '789']);
...
test('should not clear selected rects when scrolling the wheel', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await initThreeParagraphs(page);
    await assertRichTexts(page, ['123', '456', '789']);
...
test('should refresh selected rects when resizing the window/viewport', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await initThreeParagraphs(page);
    await assertRichTexts(page, ['123', '456', '789']);
...
test('should clear block selection before native selection', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await initThreeParagraphs(page);
    await assertRichTexts(page, ['123', '456', '789']);
...
test('should not be misaligned when the editor container has padding or margin', async ({ page, }) => {
...
test('undo should clear block selection', async ({ page }) => {
    ...
    await dragBetweenCoords(page, { x: rect.x - 5, y: rect.y - 5 }, { x: rect.x + 5, y: rect.y + rect.height });
...
test('should not draw rect for sub selected blocks when entering tab key', async ({ page, }, testInfo) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await initThreeParagraphs(page);
    await assertRichTexts(page, ['123', '456', '789']);
    ...
    await dragBetweenCoords(page, { x: coord.x - 60, y: coord.y + 10 }, { x: coord.x + 100, y: coord.y + 30 }, { steps: 50 });
    await pressTab(page);
    expect(await getPageSnapshot(page, true)).toMatchSnapshot(`${testInfo.title}.json`);
});
test('should blur rich-text first on starting block selection', async ({ page, }) => {
    ...
    const coord = await getIndexCoordinate(page, [1, 2]);
    await dragBetweenCoords(page, { x: coord.x - 30, y: coord.y - 10 }, { x: coord.x + 20, y: coord.y + 50 });
...
test('should show toolbar of image on block selection', async ({ page }) => {
    ...
    await expect(page.locator('affine-block-selection').locator('visible=true')).toHaveCount(1);
});
test('click bottom of page and if the last is embed block, editor should insert a new editable block', async ({ page, }, testInfo) => {
    await enterPlaygroundRoom(page);
    await initImageState(page);
    await activeEmbed(page);
...
test('should un-select blocks when pressing escape', async ({ page }) => {
    ...
    await expect(page.locator('affine-block-selection').locator('visible=true')).toHaveCount(1);
    await pressEscape(page);
    await expect(page.locator('affine-block-selection').locator('visible=true')).toHaveCount(0);
    await focusRichText(page, 2);
    await pressEnter(page);
    await type(page, '-');
...
test('should scroll page properly by wheel after inserting a new block and selecting it', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
...
test('should not select parent block when dragging area only intersects with child', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await initThreeParagraphs(page);
    await assertRichTexts(page, ['123', '456', '789']);
...
test('scroll should update dragging area and select blocks when dragging', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
