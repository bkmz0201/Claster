...
import { DatabaseBlockModel, EmbedLinkedDocModel, EmbedSyncedDocModel, ImageBlockModel, NoteBlockModel, NoteDisplayMode, } from '@blocksuite/affine/model';
import { embedSyncedDocMiddleware, MarkdownAdapter, titleMiddleware, } from '@blocksuite/affine/shared/adapters';
import { getImageSelectionsCommand, getSelectedBlocksCommand, } from '@blocksuite/affine/shared/commands';
import { DocModeProvider } from '@blocksuite/affine/shared/services';
import { getBlockProps, isInsideEdgelessEditor, matchModels, } from '@blocksuite/affine/shared/utils';
import { BlockStdScope } from '@blocksuite/affine/std';
import { GfxControllerIdentifier, GfxPrimitiveElementModel, } from '@blocksuite/affine/std/gfx';
...
import { getSelectedAttachments, getSelectedImagesAsBlobs, getSelectedTextContent, selectedToCanvas, } from './selection-utils';
export async function extractSelectedContent(host) {
...
async function extractEdgelessSelected(host) {
    ...
    if (selectedElements.length) {
        ...
        const markdownAdapter = new MarkdownAdapter(transformer, host.store.provider);
        ...
        try {
            ...
            for (const element of selectedElements) {
                ...
                else if (element instanceof EmbedSyncedDocModel ||
...
async function extractPageSelected(host) {
    ...
    if (hasText && !hasImages) {
        ...
        const markdown = (await getSelectedTextContent(host, 'markdown')).trim() || '';
        return {
            quote: text,
            markdown,
            images,
...
export async function extractMarkdownFromDoc(doc) {
    ...
    const textModels = blockModels.filter(model => !matchModels(model, [ImageBlockModel, DatabaseBlockModel]));
...
function getNoteBlockModels(doc) {
    const notes = doc
        .getBlocksByFlavour('affine:note')
        .filter(note => note.model.props.displayMode !==
...
async function getTransformer(doc) {
