import { addProperty, copyCellsByProperty, databaseBlockProperties, deleteColumn, getCell, getProperty, updateCell, } from '@blocksuite/affine-block-database';
import { DatabaseBlockSchemaExtension, NoteBlockSchemaExtension, ParagraphBlockSchemaExtension, RootBlockSchemaExtension, } from '@blocksuite/affine-model';
...
describe('DatabaseManager', () => {
    ...
    beforeEach(() => {
        ...
        const databaseModel = doc.getModelById(databaseBlockId);
        db = databaseModel;
        col1 = addProperty(db, 'end', databaseBlockProperties.numberColumnConfig.create('Number'));
        col2 = addProperty(db, 'end', propertyModelPresets.selectPropertyModelConfig.create('Single Select', {
            options: selection,
        }));
        col3 = addProperty(db, 'end', databaseBlockProperties.richTextColumnConfig.create('Rich Text'));
    ...
    test('addColumn', () => {
        const column = databaseBlockProperties.numberColumnConfig.create('Test Column');
    ...
    test('copyCellsByColumn', () => {
        const newColId = addProperty(db, 'end', propertyModelPresets.selectPropertyModelConfig.create('Copied Select', {
            options: selection,
        }));
