 ...
 */
export async function applyPatchToDoc(doc, patch) {
    // Get all note blocks
    ...
    for (const op of patch) {
        if (op.op === 'delete') {
            ...
            if (index === -1)
                continue;
            await replaceFromMarkdown(undefined, op.content, doc, parentId, index, op.id);
        }
        else if (op.op === 'insert') {
            // Insert new block
            const parentId = note.id;
            const index = op.index;
            await insertFromMarkdown(undefined, op.block.content, doc, parentId, index, op.block.id);
        }
    }
}
//# sourceMappingURL=apply-patch-to-doc.js.map