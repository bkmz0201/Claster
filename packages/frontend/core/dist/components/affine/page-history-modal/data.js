...
import { applyUpdate, Doc as YDoc, encodeStateAsUpdate, encodeStateVector, UndoManager, } from 'yjs';
import { useMutateQueryResource, useMutation, } from '../../../components/hooks/use-mutation';
import { useQueryInfinite } from '../../../components/hooks/use-query';
const logger = new DebugLogger('page-history');
export const useDocSnapshotList = (workspaceId, pageDocId) => {
    const pageSize = 10;
    ...
    return [histories, shouldLoadMore ? loadMore : false, !!loadingMore];
};
const snapshotFetcher = async ([fetchService, workspaceId, pageDocId, ts] // timestamp is the key to the history/snapshot
) => {
    if (!ts) {
        return null;
    }
    const res = await fetchService.fetch(`/api/workspaces/${workspaceId}/docs/${pageDocId}/histories/${ts}`);
    if (!res.ok) {
...
const getOrCreateShellWorkspace = (workspaceId, flavourProvider) => {
...
export const usePageHistory = (workspaceId, pageDocId, ts) => {
    const fetchService = useService(FetchService);
    // snapshot should be immutable. so we use swr immutable to disable revalidation
    const { data } = useSWRImmutable([fetchService, workspaceId, pageDocId, ts], {
...
export const useSnapshotPage = (docCollection, pageDocId, ts) => {
    ...
    const page = useMemo(() => {
        ...
        const historyShellWorkspace = getOrCreateShellWorkspace(docCollection.id, workspacesService.getWorkspaceFlavourProvider(affineWorkspace.meta));
...
export const historyListGroupByDay = (histories) => {
    const map = new Map();
...
export function revertUpdate(doc, snapshotUpdate, getMetadata) {
    ...
    const changesSinceSnapshotUpdate = encodeStateAsUpdate(doc, snapshotStateVector);
    ...
    undoManager.undo();
    const revertChangesSinceSnapshotUpdate = encodeStateAsUpdate(snapshotDoc, currentStateVector);
    applyUpdate(doc, revertChangesSinceSnapshotUpdate);
}
export const useRestorePage = (docCollection, pageId) => {
    ...
    const onRestore = useMemo(() => {
        return async (version, update) => {
            ...
            await mutateQueryResource(listHistoryQuery, vars => {
                return (vars.pageDocId === pageDocId && vars.workspaceId === docCollection.id);
            });
