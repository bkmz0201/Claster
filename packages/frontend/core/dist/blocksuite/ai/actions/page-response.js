...
import { getSurfaceBlock, SurfaceBlockModel, } from '@blocksuite/affine/blocks/surface';
...
import { GfxBlockElementModel, LayerManager, } from '@blocksuite/affine/std/gfx';
...
const PADDING = 100;
export async function pageResponseHandler(id, host, ctx, place = 'after') {
    switch (id) {
...
function responseToBrainstormMindmap(host, ctx, place) {
    ...
    host.store.transact(() => {
        ...
        setTimeout(() => {
            ...
            const targetBound = [x, y + h / 2 + PADDING - 15, w, h];
...
function responseToMakeItReal(host, ctx, place) {
    const surface = getSurfaceBlock(host.store);
    const aiPanel = getAIPanelWidget(host);
    if (!aiPanel.answer || !surface)
        return;
...
async function responseToCreateSlides(host, ctx, place) {
...
export function responseToCreateImage(host, place) {
    const aiPanel = getAIPanelWidget(host);
...
export async function replaceWithMarkdown(host) {
    const aiPanel = getAIPanelWidget(host);
    const { answer } = aiPanel;
...
async function insertMarkdownBelow(host) {
    const aiPanel = getAIPanelWidget(host);
    const { answer } = aiPanel;
    const selection = getSelection(host);
    if (!answer || !selection)
...
async function insertMarkdownAbove(host) {
    const aiPanel = getAIPanelWidget(host);
    const { answer } = aiPanel;
    const selection = getSelection(host);
    if (!answer || !selection)
...
function addSurfaceRefBlock(host, bound, place) {
    const surface = getSurfaceBlock(host.store);
    if (!surface)
        return;
    const frame = host.store.addBlock('affine:frame', {
...
function getTargetModel(host, place) {
...
function addSiblingBlocks(host, props, place) {
    const targetModel = getTargetModel(host, place);
    if (!targetModel)
        return;
    return host.store.addSiblingBlocks(targetModel, props, place);
}
function findFrameObject(obj) {
