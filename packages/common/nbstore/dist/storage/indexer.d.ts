...
type HighlightAbleField<T extends keyof IndexerSchema> = {
    [K in keyof IndexerSchema[T]]: IndexerSchema[T][K] extends 'FullText' ? K : never;
...
export type SearchResult<T extends keyof IndexerSchema, O extends SearchOptions<T>> = {
    ...
    nodes: ({
        ...
        fields: {
            [key in O['fields'][number]]: string | string[];
        };
    } : unknown) & (O['highlights'] extends any[] ? {
        highlights: {
            [key in O['highlights'][number]['field']]: string[];
        };
    } : unknown))[];
};
...
export type AggregateResult<T extends keyof IndexerSchema, O extends AggregateOptions<T>> = {
    pagination: ResultPagination;
    buckets: ({
        key: string;
        score: number;
...
export declare abstract class IndexerStorageBase implements IndexerStorage {
    readonly storageType = "indexer";
    readonly recommendRefreshInterval: number;
    abstract readonly connection: Connection;
    abstract readonly isReadonly: boolean;
    abstract search<T extends keyof IndexerSchema, const O extends SearchOptions<T>>(table: T, query: Query<T>, options?: O): Promise<SearchResult<T, O>>;
    abstract aggregate<T extends keyof IndexerSchema, const O extends AggregateOptions<T>>(table: T, query: Query<T>, field: keyof IndexerSchema[T], options?: O): Promise<AggregateResult<T, O>>;
    abstract search$<T extends keyof IndexerSchema, const O extends SearchOptions<T>>(table: T, query: Query<T>, options?: O): Observable<SearchResult<T, O>>;
    abstract aggregate$<T extends keyof IndexerSchema, const O extends AggregateOptions<T>>(table: T, query: Query<T>, field: keyof IndexerSchema[T], options?: O): Observable<AggregateResult<T, O>>;
    abstract deleteByQuery<T extends keyof IndexerSchema>(table: T, query: Query<T>): Promise<void>;
    abstract insert<T extends keyof IndexerSchema>(table: T, document: IndexerDocument<T>): Promise<void>;
    abstract delete<T extends keyof IndexerSchema>(table: T, id: string): Promise<void>;
    abstract update<T extends keyof IndexerSchema>(table: T, document: IndexerDocument<T>): Promise<void>;
