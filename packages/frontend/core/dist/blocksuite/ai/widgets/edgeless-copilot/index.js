...
import { autoPlacement, autoUpdate, computePosition, limitShift, offset, shift, size, } from '@floating-ui/dom';
  ...
  `; }
        ...
        _showCopilotInput() {
            requestConnectedFrame(() => {
                ...
                if (!rootBlockId)
                    return;
                const input = this.host.view.getWidget(AFFINE_AI_PANEL_WIDGET, rootBlockId);
                if (input instanceof AffineAIPanelWidget) {
                    ...
                    if (aiPanel.config && !aiPanel.config.generateAnswer) {
                        ...
                        aiPanel.config.inputCallback = text => {
                            const panel = this.shadowRoot?.querySelector('edgeless-copilot-panel');
        ...
        _updateCopilotPanel(referenceElement) {
        ...
        connectedCallback() {
            ...
            this._disposables.add(CopilotSelectionTool.draggingAreaUpdated.subscribe(shouldShowPanel => {
                this._visible = true;
...
export const edgelessCopilotWidget = WidgetViewExtension('affine:page', AFFINE_EDGELESS_COPILOT_WIDGET, literal `${unsafeStatic(AFFINE_EDGELESS_COPILOT_WIDGET)}`);
