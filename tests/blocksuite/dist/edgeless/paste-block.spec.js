...
import { click, copyByKeyboard, enterPlaygroundRoom, focusRichText, getAllEdgelessNoteIds, getAllEdgelessTextIds, getNoteBoundBoxInEdgeless, initEmptyEdgelessState, pasteByKeyboard, pasteTestImage, pressEnter, pressEnterWithShortkey, pressEscape, selectAllByKeyboard, setEdgelessTool, switchEditorMode, type, } from '../utils/actions/index.js';
import { test } from '../utils/playwright.js';
test.describe('pasting blocks', () => {
    const initContent = async (page) => {
        // Text
    ...
    test('pasting a note block', async ({ page }) => {
        ...
        const newNote = page.locator(`affine-edgeless-note[data-block-id="${newNoteId}"]`);
    ...
    test('pasting a edgeless block', async ({ page }) => {
        ...
        const newText = page.locator(`affine-edgeless-text[data-block-id="${newTextId}"]`);
...
//# sourceMappingURL=paste-block.spec.js.map