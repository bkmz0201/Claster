...
export const getOpenUrlInDesktopAppLink = (url, newTab = true, scheme = channelToScheme[BUILD_CONFIG.appBuildType]) => {
    try {
        if (!scheme) {
            return null;
        }
        ...
        return new URL(`${scheme}://${urlObject.host}${urlObject.pathname}?${params.toString()}#${urlObject.hash}`).toString();
    }
    catch (e) {
