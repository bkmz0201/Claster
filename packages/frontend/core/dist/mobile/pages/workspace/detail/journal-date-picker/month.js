...
import { CELL_HEIGHT, DATE_FORMAT, HORIZONTAL_SWIPE_THRESHOLD, MONTH_VIEW_HEIGHT, ROWS_GAP, WEEK_VIEW_HEIGHT, } from './constants';
...
export const MonthView = ({ viewportHeight }) => {
    const rootRef = useRef(null);
    const swipeRef = useRef(null);
    const { width, selected, onSelect, setCursor } = useContext(JournalDatePickerContext);
    const [swiping, setSwiping] = useState(false);
    const [animating, setAnimating] = useState(false);
    const [swipingDeltaX, setSwipingDeltaX] = useState(0);
    const weeks = useMemo(() => getWeeks(selected ?? dayjs().format(DATE_FORMAT)), [selected]);
    ...
    return (_jsx("div", { className: styles.monthViewClip, ref: rootRef, children: _jsxs("div", { ref: swipeRef, className: styles.monthsSwipe, style: { transform: `translateX(${translateX}px)` }, children: [_jsx(MonthGrid, { hidden: !swiping && !animating, date: getFirstDayOfMonth(selected, -1) }), _jsx(MonthGrid, { style: { transform: `translateY(${translateY}px)` }, date: selected ?? '' }), _jsx(MonthGrid, { hidden: !swiping && !animating, date: getFirstDayOfMonth(selected, 1) })] }) }));
};
const MonthGrid = ({ date, className, hidden, ...props }) => {
    const weeks = useMemo(() => getWeeks(date ?? dayjs().format(DATE_FORMAT)), [date]);
    return (_jsx("div", { className: clsx(styles.monthView, className), ...props, children: hidden ? null : weeks.map(week => _jsx(WeekRow, { start: week }, week)) }));
};
