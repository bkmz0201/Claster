...
export const useEnableCloud = () => {
    ...
    const enableCloud = useCallback(async (ws, options) => {
        try {
            if (!ws)
                return;
            if (!account)
                return;
            const { id: newId } = await workspacesService.transformLocalToCloud(ws, account.id, 'affine-cloud');
    ...
    const openSignIn = useCallback(() => globalDialogService.open('sign-in', {
        step: 'signIn',
    }), [globalDialogService]);
    const signInOrEnableCloud = useCallback(async (...args) => {
    ...
    const confirmEnableCloud = useCallback((ws, options) => {
        ...
        openConfirmModal({
            ...
            confirmText: loginStatus === 'authenticated'
                ? t['Enable']()
            ...
            onConfirm: async () => await signInOrEnableCloud(ws, {
                ...options,
...
//# sourceMappingURL=use-enable-cloud.js.map