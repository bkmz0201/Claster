...
export const useDetailPageHeaderResponsive = (availableWidth) => {
    const mode = useLiveData(useService(EditorService).editor.mode$);
    const workbench = useService(WorkbenchService).workbench;
    const viewPosition = useViewPosition();
    const workbenchViewsCount = useLiveData(workbench.views$.map(views => views.length));
    ...
    const showDivider = !BUILD_CONFIG.isElectron &&
        viewPosition.isLast &&
        !rightSidebarOpen &&
...
//# sourceMappingURL=use-header-responsive.js.map