...
import { assertTitle, clickEdgelessModeButton, clickView, createEdgelessNoteBlock, getEdgelessSelectedIds, getPageMode, getSelectedXYWH, isDocTitleFocused, locateEditorContainer, locateModeSwitchButton, locateToolbar, moveToView, resizeElementByHandle, toViewCoord, } from '@affine-test/kit/utils/editor';
import { pasteByKeyboard, pressBackspace, pressEnter, pressEscape, selectAllByKeyboard, undoByKeyboard, } from '@affine-test/kit/utils/keyboard';
...
test.describe('edgeless page block', () => {
    const locateHeaderToolbar = (page) => page.getByTestId('edgeless-page-block-header');
    test('only first note block has header toolbar and its element toolbar', async ({ page, }) => {
    ...
    test('should shrink note block when clicking on the toggle button', async ({ page, }) => {
        const toolbar = locateHeaderToolbar(page);
    ...
    test('page title in toolbar should be displayed when page block is collapsed and hidden when page block is not collapsed', async ({ page, }) => {
        const toolbar = locateHeaderToolbar(page);
    ...
    test('should switch to page mode when expand button is clicked', async ({ page, }) => {
        const toolbar = locateHeaderToolbar(page);
        const viewInPageButton = toolbar.getByTestId('edgeless-note-view-in-page-button');
        await viewInPageButton.click();
        expect(await getPageMode(page)).toBe('page');
    });
    test('should open doc properties dialog when info button is clicked', async ({ page, }) => {
        const toolbar = locateHeaderToolbar(page);
    ...
    test('should copy note edgeless link to clipboard when link button is clicked', async ({ page, }) => {
        const toolbar = locateHeaderToolbar(page);
...
test.describe('edgeless note element toolbar', () => {
    test('the toolbar of page block should not contains auto-height button and display in page button', async ({ page, }) => {
        await selectAllByKeyboard(page);
    ...
    test('the toolbar of note block should contains auto-height button and display in page button', async ({ page, }) => {
        await createEdgelessNoteBlock(page, [100, 100]);
    ...
    test('display in page button', async ({ page }) => {
        ...
        await expect(undoButton, 'the toast should be hidden immediately when undo by keyboard').toBeHidden();
        await displayInPage.click();
        await viewTocButton.click();
        const toc = page.locator('affine-outline-panel');
        await toc.waitFor({ state: 'visible' });
        const highlightNoteCards = toc.locator('affine-outline-note-card > [data-status="selected"]');
        await expect(highlightNoteCards).toHaveCount(1);
    });
    test('note edgeless styles', async ({ page }) => {
        const getNoteEdgelessProps = async (page, noteId) => {
            const container = locateEditorContainer(page);
            return await container.evaluate((container, noteId) => {
                const root = container.querySelector('affine-edgeless-root');
                const note = root.gfx.getElementById(noteId);
        ...
        const borderStylePanel = noteStylePanel.getByTestId('affine-note-border-style-panel');
        ...
        const cornerPanel = noteStylePanel.getByTestId('affine-note-corner-radius-panel');
        ...
        const toggleButton = headerToolbar.getByTestId('edgeless-note-toggle-button');
        await toggleButton.click();
        expect(await getNoteEdgelessProps(page, noteId)).toEqual({
            collapse: true,
            collapsedHeight: 48,
...
test.describe('note block rendering', () => {
    test('collapsed content rendering', async ({ page }) => {
        ...
        const center = [xywh[0] + xywh[2] / 2, xywh[1] + xywh[3] / 2];
        const note = page
            .locator('affine-edgeless-note')
            .getByTestId('edgeless-note-clip-container')
            .nth(1);
        await expect(note, 'should hide collapsed content').toHaveCSS('overflow-y', 'clip');
        await moveToView(page, center);
        await expect(note, 'should show collapsed content when hover').toHaveCSS('overflow-y', 'visible');
        ...
        await expect(note, 'should hide collapsed content during dragging').toHaveCSS('overflow-y', 'clip');
        await page.mouse.move(x3, y3, { steps: 10 });
        await page.mouse.up();
        await page.mouse.move(x3, y3);
        await expect(note, 'should show collapsed content when dragging is finished').toHaveCSS('overflow-y', 'visible');
    });
    test('cursor should not jump to page block title from note block', async ({ page, }) => {
        await createEdgelessNoteBlock(page, [50, 50]);
        await pressBackspace(page);
        expect(await isDocTitleFocused(page)).toBeFalsy();
    });
});
test('should convert note block to linked doc when clicking turn into linked doc button', async ({ page, }) => {
    await createEdgelessNoteBlock(page, [100, 100]);
