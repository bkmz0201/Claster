...
import { AuthService, SubscriptionService, WorkspaceSubscriptionService, } from '../../../../../modules/cloud';
import { ConfirmLoadingModal, DowngradeModal, DowngradeTeamModal, } from './modals';
/**
 * Cancel action with modal & request
 * @param param0
 * @returns
 */
export const CancelAction = ({ children, open, onOpenChange, }) => {
    ...
    const downgrade = useAsyncCallback(async () => {
        try {
            ...
            if (account && prevRecurring) {
                downgradeNotify(getDowngradeQuestionnaireLink({
                    email: account.email ?? '',
                    id: account.id,
    ...
    return (_jsxs(_Fragment, { children: [children, _jsx(DowngradeModal, { open: open, onCancel: downgrade, onOpenChange: onOpenChange, loading: isMutating })] }));
};
export const CancelTeamAction = ({ children, open, onOpenChange, }) => {
    const [idempotencyKey, setIdempotencyKey] = useState(nanoid());
    const [isMutating, setIsMutating] = useState(false);
    ...
    const downgrade = useAsyncCallback(async () => {
        try {
            ...
            setIsMutating(true);
            await subscription.cancelSubscription(idempotencyKey, SubscriptionPlan.Team);
            await subscription.waitForRevalidation();
            // refresh idempotency key
            setIdempotencyKey(nanoid());
            onOpenChange(false);
    ...
    return (_jsxs(_Fragment, { children: [children, _jsx(DowngradeTeamModal, { open: open, onCancel: downgrade, onOpenChange: onOpenChange, loading: isMutating })] }));
};
/**
 ...
 * @returns
 */
export const ResumeAction = ({ children, open, onOpenChange, }) => {
    ...
    return (_jsxs(_Fragment, { children: [children, _jsx(ConfirmLoadingModal, { type: 'resume', open: open, onConfirm: resume, onOpenChange: onOpenChange, loading: isMutating })] }));
};
export const TeamResumeAction = ({ children, open, onOpenChange, }) => {
    // allow replay request on network error until component unmount or success
    const [idempotencyKey, setIdempotencyKey] = useState(nanoid());
    const [isMutating, setIsMutating] = useState(false);
    const subscription = useService(WorkspaceSubscriptionService).subscription;
