...
import type { AggregateResult, BlobRecord, DocClock, DocClocks, DocDiff, DocRecord, DocUpdate, ListedBlobRecord, Query, SearchResult, StorageType } from '../storage';
...
export interface StoreInitOptions {
    local: {
        [key in StorageType]?: StorageInitOptions;
    };
...
export type WorkerOps = UnionToIntersection<Values<Values<{
    [k in keyof GroupedWorkerOps]: {
        [k2 in keyof GroupedWorkerOps[k]]: k2 extends string ? Record<`${k}.${k2}`, GroupedWorkerOps[k][k2]> : never;
    };
}>>>;
