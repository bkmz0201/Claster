...
export function getLinkedDocPopover(page) {
    ...
    const assertExistRefText = async (text) => {
        await expect(refNode).toBeVisible();
    ...
    const createDoc = async (pageType, pageName) => {
        await type(page, '@');
    ...
    const assertActivePageIdx = async (idx) => {
        if (idx !== 0) {
    ...
    return {
        ...
        findRefNode,
        assertExistRefText,
        createLinkedDoc: async (pageName) => createDoc('LinkedPage', pageName),
        /**
         * @deprecated
         */
        createSubpage: async (pageName) => createDoc('Subpage', pageName),
        assertActivePageIdx,
    };
}
//# sourceMappingURL=linked-doc.js.map