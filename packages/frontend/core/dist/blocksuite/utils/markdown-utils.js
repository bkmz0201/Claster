...
import { defaultImageProxyMiddleware, embedSyncedDocMiddleware, MarkdownAdapter, pasteMiddleware, PlainTextAdapter, titleMiddleware, } from '@blocksuite/affine/shared/adapters';
...
let markdownWorkspace = null;
const getMarkdownWorkspace = (featureFlagService) => {
...
export const markdownToSnapshot = async (markdown, store, host) => {
    ...
    if (page) {
        ...
        if (pageSnapshot) {
            const snapshot = {
                type: 'slice',
                content: [
                    pageSnapshot.blocks.children.find(b => b.flavour === 'affine:note'),
                ],
...
export async function insertFromMarkdown(host, markdown, doc, parent, index, id) {
    const { snapshot, transformer } = await markdownToSnapshot(markdown, doc, host);
    ...
    for (let i = 0; i < snapshots.length; i++) {
        ...
        const model = await transformer.snapshotToBlock(blockSnapshot, doc, parent, (index ?? 0) + i);
        if (model) {
            models.push(model);
        }
    }
...
export async function replaceFromMarkdown(host, markdown, doc, parent, index, id) {
    doc.deleteBlock(id);
    const { snapshot, transformer } = await markdownToSnapshot(markdown, doc, host);
    const snapshots = snapshot?.content.flatMap(x => x.children) ?? [];
...
export async function markDownToDoc(answer, middlewares, affineFeatureFlagService) {
