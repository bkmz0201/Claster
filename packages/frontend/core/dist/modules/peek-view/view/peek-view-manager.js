...
import { GenericImagePreviewModalWithClose, ImagePreviewPeekView, } from './image-preview';
import { PeekViewModalContainer, } from './modal-container';
import { AttachmentPeekViewControls, DefaultPeekViewControls, DocPeekViewControls, } from './peek-view-controls';
function renderPeekView({ info }, animating) {
    if (info.type === 'template') {
        return toReactNode(info.template);
    }
    if (info.type === 'doc') {
        return _jsx(DocPeekPreview, { docRef: info.docRef, animating: animating });
    }
    if (info.type === 'attachment' && info.docRef.blockIds?.[0]) {
        return (_jsx(AttachmentPreviewPeekView, { docId: info.docRef.docId, blockId: info.docRef.blockIds?.[0] }));
    }
    if (info.type === 'image' && info.docRef.blockIds?.[0]) {
        return (_jsx(ImagePreviewPeekView, { docId: info.docRef.docId, blockId: info.docRef.blockIds?.[0] }));
    }
    if (info.type === 'image-list') {
        return _jsx(GenericImagePreviewModalWithClose, { ...info.data });
    }
    if (info.type === 'ai-chat-block') {
        return _jsx(AIChatBlockPeekView, { model: info.model, host: info.host });
    }
    return null; // unreachable
}
const renderControls = ({ info }) => {
    if (info.type === 'doc') {
        return _jsx(DocPeekViewControls, { docRef: info.docRef });
    }
    if (info.type === 'attachment') {
        return _jsx(AttachmentPeekViewControls, { docRef: info.docRef });
    }
...
const getRendererProps = (activePeekView, animating) => {
    if (!activePeekView) {
        return;
    }
    ...
    return {
        children: preview,
        controls,
        target: activePeekView?.target.element instanceof HTMLElement
...
export const PeekViewManagerModal = () => {
    ...
    return (_jsx(PeekViewModalContainer, { ...renderProps, animation: show?.animation ? renderProps?.animation : 'none', open: !!show?.value && !!renderProps, onOpenChange: open => {
            if (!open) {
                peekViewEntity.close();
            }
        }, onAnimationStart: onAnimationStart, onAnimationEnd: onAnimationEnd, children: renderProps?.children }));
};
//# sourceMappingURL=peek-view-manager.js.map