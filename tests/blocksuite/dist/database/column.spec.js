...
import { assertDatabaseColumnOrder, dragBetweenCoords, enterPlaygroundRoom, getBoundingBox, initDatabaseDynamicRowWithData, initEmptyDatabaseState, pressArrowRight, pressArrowUp, pressArrowUpWithShiftKey, pressBackspace, pressEnter, pressEscape, redoByClick, selectAllByKeyboard, type, undoByClick, waitNextFrame, } from '../utils/actions/index.js';
import { test } from '../utils/playwright.js';
import { assertDatabaseCellNumber, assertDatabaseCellRichTexts, assertSelectedStyle, changeColumnType, clickDatabaseOutside, clickSelectOption, getDatabaseCell, getDatabaseHeaderColumn, initDatabaseColumn, performColumnAction, performSelectColumnTagAction, switchColumnType, } from './actions.js';
...
test.describe('switch column type', () => {
    ...
    test('switch to progress', async ({ page }) => {
        ...
        const { x: progressBgX, y: progressBgY, width: progressBgWidth, } = await getBoundingBox(progressBg);
        ...
        const { x: dragX, y: dragY, width, height, } = await getBoundingBox(dragHandle);
        ...
        await dragBetweenCoords(page, { x: dragCenterX, y: dragCenterY }, { x: endX, y: dragCenterY });
...
test.describe('select column tag action', () => {
    ...
    test('should support modifying select tag color', async ({ page }) => {
        ...
        await pressEscape(page);
        await assertSelectedStyle(page, 'backgroundColor', 'var(--affine-v2-chip-label-red)');
    });
});
test.describe('drag-to-fill', () => {
    ...
    test('should not show in multi (row or column) selection', async ({ page, }) => {
        await enterPlaygroundRoom(page);
        await initEmptyDatabaseState(page);
        await initDatabaseColumn(page);
        await initDatabaseDynamicRowWithData(page, '', true);
    ...
    test('should fill columns with data', async ({ page }) => {
        ...
        await dragBetweenCoords(page, { x: bbox.x + bbox.width / 2, y: bbox.y + bbox.height / 2 }, { x: bbox.x, y: bbox.y + 200 });
        expect(await cells.nth(0).innerText()).toBe('thing');
