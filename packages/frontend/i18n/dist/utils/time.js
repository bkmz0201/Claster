 ...
 * ```
 */
export function i18nTime(time, options = {}) {
    ...
    if (relativeOption) {
        ...
        const timeUnitProcessor = {
            second: () => {
                ...
                if (relativeOption.max[1] === 'second' &&
            ...
            minute: () => {
                ...
                if (relativeOption.max[1] === 'minute' &&
        ...
        const processors = Object.entries(timeUnitProcessor).sort((a, b) => timeUnitCode[a[0]] - timeUnitCode[b[0]]);
    ...
    const formatter = new Intl.DateTimeFormat(i18n.language, {
        year: !absoluteOption.noYear && !absoluteOption.noDate ? 'numeric' : undefined,
        month: !absoluteOption.noDate &&
            timeUnitCode[absoluteOption.accuracy] <= timeUnitCode['month']
            ? 'short'
            : undefined,
        day: !absoluteOption.noDate &&
            timeUnitCode[absoluteOption.accuracy] <= timeUnitCode['day']
            ? 'numeric'
            : undefined,
        hour: timeUnitCode[absoluteOption.accuracy] <= timeUnitCode['hour']
            ? 'numeric'
            : undefined,
        minute: timeUnitCode[absoluteOption.accuracy] <= timeUnitCode['minute']
            ? 'numeric'
            : undefined,
        second: timeUnitCode[absoluteOption.accuracy] <= timeUnitCode['second']
            ? 'numeric'
            : undefined,
    });
