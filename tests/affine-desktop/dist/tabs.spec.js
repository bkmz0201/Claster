...
import { closeTab, expectActiveTab, expectTabCount, expectTabTitle, } from '@affine-test/kit/utils/app-tabs';
import { clickNewPageButton, createLinkedPage, dragTo, getPageByTitle, } from '@affine-test/kit/utils/page-logic';
...
test('drag a page from "All pages" list to tabs header', async ({ page }) => {
    ...
    await dragTo(page, getPageByTitle(page, title), page.getByTestId('add-tab-view-button'));
    await expectTabCount(page, 2);
...
test('reorder tabs', async ({ page }) => {
    ...
    await dragTo(page, page.getByTestId('workbench-tab').nth(0), page.getByTestId('workbench-tab').nth(1), 'right');
