...
const TeamItem = memo(({ workspaces, badgeText }) => {
    const t = useI18n();
    ...
    if (workspaces.length === 0)
        return null;
    const displayName = workspaces.length > 1
        ? t['com.affine.workspace.cloud.account.team.multi']()
        : workspaces[0].profile.profile$.value?.name || 'Team';
    const tooltipContent = workspaces.length > 1
        ? t['com.affine.workspace.cloud.account.team.tips-2']()
        : t['com.affine.workspace.cloud.account.team.tips-1']();
    return (_jsx(Tooltip, { content: tooltipContent, children: _jsxs("div", { className: styles.teamWorkspace, onClick: handleClick, children: [_jsx("div", { className: styles.teamAvatarStack, children: workspaces.map((workspace, index) => (_jsx(WorkspaceAvatar, { meta: workspace.meta, rounded: 3, size: 16, name: workspace.profile.profile$.value?.name, colorfulFallback: true, className: clsx(styles.workspaceAvatar, {
                            ['multi-avatar']: index !== 0,
                        }) }, workspace.profile.id))) }), _jsx("span", { className: styles.teamName, children: displayName }), _jsx("span", { className: styles.teamBadge, children: badgeText })] }) }));
});
...
export const TeamList = memo(() => {
    ...
    const teamWorkspaces = useMemo(() => {
        ...
        for (const profile of workspaceProfiles) {
            if (!profile.profile$.value?.isTeam)
                continue;
    ...
    if (teamWorkspaces.ownerWorkspaces.length === 0 &&
        teamWorkspaces.memberWorkspaces.length === 0) {
        return null;
    }
    return (_jsxs(_Fragment, { children: [teamWorkspaces.ownerWorkspaces.length > 0 && (_jsx(TeamItem, { workspaces: teamWorkspaces.ownerWorkspaces, badgeText: t['com.affine.workspace.cloud.account.team.owner']() })), teamWorkspaces.memberWorkspaces.length > 0 && (_jsx(TeamItem, { workspaces: teamWorkspaces.memberWorkspaces, badgeText: t['com.affine.workspace.cloud.account.team.member']() })), _jsx(Divider, {})] }));
});
