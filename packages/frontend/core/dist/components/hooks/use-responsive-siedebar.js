...
let OBSERVED = false;
export const useResponsiveSidebar = (hideThreshold = 540, floatThreshold = 768) => {
    ...
    const handleFloatSidebar = useCallback((float) => {
        appSidebarService.sidebar.setSmallScreenMode(float);
    }, [appSidebarService.sidebar]);
    useEffect(() => {
        if (OBSERVED) {
            console.warn(`"${useResponsiveSidebar.name}" already observed, do not call it multiple times`);
            return;
        }
        OBSERVED = true;
...
//# sourceMappingURL=use-responsive-siedebar.js.map