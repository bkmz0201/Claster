...
import { addColumn, addRows, initDatabaseByOneStep, pasteString, selectCell, verifyCellContents, } from './utils';
test.describe('Database Clipboard Operations', () => {
    test('paste tab-separated data from Excel into database', async ({ page, }) => {
    ...
    test('handle empty cells when pasting tab-separated data', async ({ page, }) => {
        // Open the home page and wait for the editor to load
        await initDatabaseByOneStep(page);
        await addColumn(page, 'multi-select');
        // Create a database block with two rows
