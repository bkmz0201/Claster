...
import { activeNoteInEdgeless, clickView, copyByKeyboard, createNote, dragBlockToPoint, edgelessCommonSetup, enterPlaygroundRoom, expectConsoleMessage, focusRichText, getPageSnapshot, initEmptyEdgelessState, initEmptyParagraphState, pasteByKeyboard, pasteContent, pressArrowDown, pressArrowRight, pressArrowUp, pressBackspace, pressEnter, pressShiftTab, pressTab, selectAllByKeyboard, setInlineRangeInSelectedRichText, SHORT_KEY, switchEditorMode, type, waitForInlineEditorStateUpdated, waitNextFrame, } from './utils/actions/index.js';
import { assertAlmostEqual, assertBlockChildrenIds, assertBlockCount, assertBlockFlavour, assertBlockSelections, assertParentBlockFlavour, assertRichTextInlineRange, } from './utils/asserts.js';
...
test.beforeEach(async ({ page }) => {
    await page.route('https://affine-worker.toeverything.workers.dev/api/worker/link-preview', async (route) => {
        await route.fulfill({
            json: {},
        });
    });
});
const createBookmarkBlockBySlashMenu = async (page, url = LOCAL_HOST_URL) => {
...
test(scoped `copy url to create bookmark in page mode`, async ({ page }, testInfo) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await focusRichText(page);
...
test(scoped `copy url to create bookmark in edgeless mode`, async ({ page }, testInfo) => {
    await enterPlaygroundRoom(page);
...
test.fixme(scoped `support dragging bookmark block directly`, async ({ page }, testInfo) => {
    await createBookmarkBlockBySlashMenu(page);
    ...
    await page.waitForTimeout(200);
    await page.waitForTimeout(200);
    expect(await getPageSnapshot(page, true)).toMatchSnapshot(`${testInfo.title}_after_add_paragraph.json`);
    // drag bookmark block
    ...
    expect(await getPageSnapshot(page, true)).toMatchSnapshot(`${testInfo.title}_after_drag.json`);
});
test('press backspace after bookmark block can select bookmark block', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await focusRichText(page);
    await pressEnter(page);
...
test.describe('embed card toolbar', () => {
    async function showEmbedCardToolbar(page) {
        await createBookmarkBlockBySlashMenu(page);
        const bookmark = page.locator('affine-bookmark');
        await bookmark.click();
        await page.waitForTimeout(100);
    ...
    test('change card style', async ({ page }) => {
        ...
        const { cardStyleHorizontalButton, cardStyleListButton } = getEmbedCardToolbar(page);
...
test('bookmark can be dragged from note to surface top level block', async ({ page, }) => {
    await enterPlaygroundRoom(page);
...
test.describe('embed youtube card', () => {
    ...
    test(scoped `change youtube card style`, async ({ page }) => {
        ...
        const snapshot = (await getPageSnapshot(page));
        expect(ignoreSnapshotId(snapshot)).toMatchSnapshot('horizontal-youtube.json');
...
test.describe('embed github card', () => {
    test('github embed card should show banner in edgeless mode', async ({ page, }) => {
        await enterPlaygroundRoom(page);
        await initEmptyEdgelessState(page);
        await switchEditorMode(page);
...
test('drag a card from canvas to note should not change the style of the card', async ({ page, }) => {
    ...
    const style = await noteBookmark.evaluate((el) => el.model.props.style);
