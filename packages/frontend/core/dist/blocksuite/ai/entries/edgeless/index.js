...
export function edgelessToolbarAIEntryConfig() {
    return {
        actions: [
            {
                ...
                content: ({ host, chain }) => {
                    const filteredGroups = edgelessAIGroups.reduce((pre, group) => {
                        const filtered = group.items.filter(item => item.showWhen?.(chain, 'edgeless', host));
                    ...
                    const handler = () => {
                        ...
                        if (aiPanel.config) {
                            ...
                            aiPanel.config.inputCallback = text => {
                                ...
                                const panel = copilotWidget.shadowRoot?.querySelector('edgeless-copilot-panel');
