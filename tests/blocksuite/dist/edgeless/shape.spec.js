...
import { assertEdgelessShapeType, assertEdgelessTool, changeShapeFillColor, changeShapeFillColorToTransparent, changeShapeStrokeColor, changeShapeStrokeStyle, changeShapeStrokeWidth, clickComponentToolbarMoreMenuButton, dragBetweenViewCoords, getEdgelessSelectedRect, getSelectedBoundCount, locatorComponentToolbar, locatorEdgelessToolButton, locatorShapeStrokeStyleButton, openComponentToolbarMoreMenu, pickColorAtPoints, resizeElementByHandle, setEdgelessTool, switchEditorMode, triggerComponentToolbarAction, zoomResetByKeyboard, } from '../utils/actions/edgeless.js';
import { addBasicBrushElement, addBasicRectShapeElement, clickView, copyByKeyboard, dblclickView, dragBetweenCoords, enterPlaygroundRoom, focusRichText, initEmptyEdgelessState, pasteByKeyboard, pressBackspace, pressEscape, selectAllBlocksByKeyboard, type, waitNextFrame, } from '../utils/actions/index.js';
import { assertEdgelessCanvasText, assertEdgelessColorSameWithHexColor, assertEdgelessNonSelectedRect, assertEdgelessSelectedRect, assertRichTexts, assertSelectedBound, } from '../utils/asserts.js';
...
test.skip('change shape fill color', async ({ page }) => {
    ...
    await assertEdgelessColorSameWithHexColor(page, lightThemeV2['edgeless/palette/medium/greyMedium'], picked);
});
test('change shape stroke color', async ({ page }) => {
    ...
    await assertEdgelessColorSameWithHexColor(page, lightThemeV2['edgeless/palette/heavy/yellow'], picked);
});
test('the tooltip of shape tool button should be hidden when the shape menu is shown', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyEdgelessState(page);
    await switchEditorMode(page);
...
test('edgeless toolbar shape menu shows up and close normally', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyEdgelessState(page);
    await switchEditorMode(page);
...
test('hovering on shape should not have effect on underlying block', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyEdgelessState(page);
    ...
    if (blockBox === null)
        throw new Error('Unexpected box value: box is null');
    const { x, y } = blockBox;
    await setEdgelessTool(page, 'shape');
...
test('shape element should not move when the selected state is inactive', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyEdgelessState(page);
    await switchEditorMode(page);
    await setEdgelessTool(page, 'shape');
    await dragBetweenCoords(page, { x: 100, y: 100 }, { x: 200, y: 200 });
    await setEdgelessTool(page, 'default');
    await dragBetweenCoords(page, { x: 50, y: 50 }, { x: 110, y: 110 }, { steps: 2 });
    await assertEdgelessSelectedRect(page, [100, 100, 100, 100]);
});
...
test('should show selected rect after exiting editing by pressing Escape', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyEdgelessState(page);
    await switchEditorMode(page);
    await zoomResetByKeyboard(page);
...
test('change shape style', async ({ page }) => {
    ...
    await assertEdgelessColorSameWithHexColor(page, lightThemeV2['edgeless/palette/light/purpleLight'], picked);
});
...
test.describe('shape hit test', () => {
    async function addTransparentRect(page, start, end) {
        const rect = {
            start,
            end,
        };
    ...
    test.fixme('using text tool to add text in shape hollow area', async ({ page, }) => {
        await addTransparentRect(page, rect.start, rect.end);
        await page.mouse.click(rect.start.x - 20, rect.start.y - 20);
        await assertEdgelessNonSelectedRect(page);
        await assertEdgelessTool(page, 'default');
    ...
    test('should enter edit mode when double-clicking a text area in a shape with a transparent background', async ({ page, }) => {
        await addTransparentRect(page, rect.start, rect.end);
        await page.mouse.click(rect.start.x - 20, rect.start.y - 20);
        await assertEdgelessNonSelectedRect(page);
        await assertEdgelessTool(page, 'default');
        ...
        await pressEscape(page);
        await waitNextFrame(page);
        const alignmentMenu = locatorComponentToolbar(page).getByLabel('alignment-menu');
...
test('should create a shape when press s and click on canvas', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyEdgelessState(page);
    await switchEditorMode(page);
...
test('shape tool should not be changed after adding new shape', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyEdgelessState(page);
    await switchEditorMode(page);
    await setEdgelessTool(page, 'shape');
