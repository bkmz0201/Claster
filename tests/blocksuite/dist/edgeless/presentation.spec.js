...
import { assertEdgelessTool, createFrame, createNote, createShapeElement, dragBetweenViewCoords, edgelessCommonSetup, enterPresentationMode, getSelectedBound, locatorPresentationToolbarButton, resizeElementByHandle, selectElementInEdgeless, selectNoteInEdgeless, setEdgelessTool, Shape, switchEditorMode, toggleFramePanel, } from '../utils/actions/edgeless.js';
import { copyByKeyboard, pasteByKeyboard, pressEscape, selectAllBlocksByKeyboard, } from '../utils/actions/keyboard.js';
import { enterPlaygroundRoom, initEmptyEdgelessState, waitNextFrame, } from '../utils/actions/misc.js';
...
test.describe('presentation', () => {
    ...
    test('should exit presentation mode when press escape', async ({ page }) => {
        ...
        const navigatorBlackBackground = page.locator('.edgeless-navigator-black-background');
    ...
    test('should be able to adjust order of presentation in toolbar', async ({ page, }) => {
        await edgelessCommonSetup(page);
        ...
        const frameItems = page.locator('edgeless-frame-order-menu .item.draggable');
        const dragIndicators = page.locator('edgeless-frame-order-menu .drag-indicator');
    ...
    test('should be able to adjust order of presentation in frame panel', async ({ page, }) => {
        await edgelessCommonSetup(page);
        ...
        const drag = async (from, to) => {
            ...
            if (endBBox === null)
                return;
            await page.mouse.move(startBBox.x + startBBox.width / 2, startBBox.y + startBBox.height / 2);
    ...
    test('duplicate frames should keep the presentation orders', async ({ page, }) => {
        await edgelessCommonSetup(page);
        await createFrame(page, [100, 100], [100, 200]);
        await createFrame(page, [200, 100], [300, 200]);
        await createFrame(page, [300, 100], [400, 200]);
    ...
    test('note should hide the collapse button when enter presentation mode', async ({ page, }) => {
    ...
    test('note should be visible when enter presentation mode', async ({ page, }) => {
        await enterPlaygroundRoom(page);
        const { noteId } = await initEmptyEdgelessState(page);
        await switchEditorMode(page);
        await selectNoteInEdgeless(page, noteId);
        ...
        const frame1 = await createFrame(page, [noteBound[0] - 10, noteBound[1] - 10], [noteBound[0] + noteBound[2] + 10, noteBound[1] + noteBound[3] + 10]);
        await selectElementInEdgeless(page, [frame1]);
        const frame1Bound = await getSelectedBound(page);
        await pressEscape(page);
        await createFrame(page, [frame1Bound[0] + frame1Bound[2] + 10, frame1Bound[1]], [frame1Bound[0] + 2 * frame1Bound[2], frame1Bound[1] + frame1Bound[3]]);
        await enterPresentationMode(page);
        const nextButton = locatorPresentationToolbarButton(page, 'next');
        const prevButton = locatorPresentationToolbarButton(page, 'previous');
    ...
    test('should disable black background when space+drag in presentation mode', async ({ page, }) => {
        await edgelessCommonSetup(page);
        await createNote(page, [300, 100], 'hello');
        await setEdgelessTool(page, 'frame');
        await dragBetweenViewCoords(page, [240, 0], [800, 200]);
...
//# sourceMappingURL=presentation.spec.js.map