...
import { Workspace, yarnList, } from '@affine-tools/utils/workspace';
...
export class InitCommand extends Command {
    constructor() {
        super(...arguments);
        this.genOxlintConfig = () => {
            const json = JSON.parse(readFileSync(this.workspace.join('oxlint.json').value, 'utf-8'));
            const ignoreList = readFileSync(this.workspace.join('.prettierignore').value, 'utf-8')
                .split('\n')
        ...
        this.genProjectTsConfig = (prev) => {
            return applyEdits(prev, modify(prev, ['references'], this.workspace.packages
                .filter(p => p.isTsProject)
                .map(p => ({ path: p.path.relativePath })), {}));
        };
        this.genPackageTsConfig = (pkg, prev) => {
            // TODO(@forehalo):
            ...
            return applyEdits(prev, modify(prev, ['references'], pkg.deps
                .filter(p => p.isTsProject)
                .map(d => ({ path: pkg.path.relative(d.path.value) })), {}));
        };
    }
    static { this.paths = [['init'], ['i'], ['codegen']]; }
    async execute() {
    ...
    format(content, parser) {
        const config = JSON.parse(readFileSync(this.workspace.join('.prettierrc').value, 'utf-8'));
