import { useMutateQueryResource, useMutation, } from '@affine/admin/use-mutation';
import { useAsyncCallback } from '@affine/core/components/hooks/affine-async-hooks';
import { createChangePasswordUrlMutation, createUserMutation, deleteUserMutation, disableUserMutation, enableUserMutation, importUsersMutation, listUsersQuery, updateAccountFeaturesMutation, updateAccountMutation, } from '@affine/graphql';
import { useCallback, useMemo, useState } from 'react';
import { toast } from 'sonner';
export const useCreateUser = () => {
    const { trigger: createAccount, isMutating: creating, error, } = useMutation({
    ...
    const create = useAsyncCallback(async ({ name, email, password, features }) => {
        try {
...
export const useUpdateUser = () => {
    const { trigger: updateAccount, isMutating: updating, error, } = useMutation({
    ...
    const revalidate = useMutateQueryResource();
    const update = useAsyncCallback(async ({ userId, name, email, features, }) => {
        try {
            await updateAccount({
                id: userId,
                input: {
...
export const useResetUserPassword = () => {
    ...
    const onResetPassword = useCallback(async (id, callback) => {
...
export const useDeleteUser = () => {
    ...
    const revalidate = useMutateQueryResource();
    const deleteById = useAsyncCallback(async (id, callback) => {
...
export const useEnableUser = () => {
    ...
    const revalidate = useMutateQueryResource();
    const enableById = useAsyncCallback(async (id, callback) => {
...
export const useDisableUser = () => {
    ...
    const revalidate = useMutateQueryResource();
    const disableById = useAsyncCallback(async (id, callback) => {
...
export const useImportUsers = () => {
    ...
    const revalidate = useMutateQueryResource();
    const handleImportUsers = useCallback(async (input, callback) => {
...
export const useExportUsers = () => {
    const exportCSV = useCallback(async (users, fields, callback) => {
        ...
        const headers = selectedFields.map(fieldId => fields.find(field => field.id === fieldId)?.label || fieldId);
    ...
    const copyToClipboard = useCallback(async (users, fields, callback) => {
        const selectedFields = fields
            .filter(field => field.checked)
            .map(field => field.id);
        const dataToCopy = [];
...
//# sourceMappingURL=use-user-management.js.map