...
import { addBasicConnectorElement, assertEdgelessConnectorToolMode, changeConnectorStrokeColor, changeConnectorStrokeStyle, changeConnectorStrokeWidth, ConnectorMode, createConnectorElement, createShapeElement, dragBetweenViewCoords, edgelessCommonSetup as commonSetup, getConnectorPath, getConnectorPathWithInOut, locatorComponentToolbar, pickColorAtPoints, rotateElementByHandle, selectElementInEdgeless, setEdgelessTool, Shape, toModelCoord, toViewCoord, triggerComponentToolbarAction, triggerShapeSwitch, } from '../../utils/actions/edgeless.js';
import { clickView, pressBackspace, waitNextFrame, } from '../../utils/actions/index.js';
import { assertConnectorPath, assertEdgelessNonSelectedRect, assertEdgelessSelectedRect, getSelectedRect, } from '../../utils/asserts.js';
import { test } from '../../utils/playwright.js';
test('path #1, the upper line is parallel with the lower line of antoher, and anchor from top to bottom of another', async ({ page, }) => {
...
test('path #2, the top-right point is overlapped with the bottom-left point of another, and anchor from top to bottom of another', async ({ page, }) => {
    await commonSetup(page);
    await createShapeElement(page, [0, 0], [100, 100], Shape.Square);
...
test('path #3, the two shape are parallel in x axis, the anchor from the right to right', async ({ page, }) => {
    await commonSetup(page);
    await createShapeElement(page, [0, 0], [100, 100], Shape.Square);
...
test('when element is removed, connector should be deleted too', async ({ page, }) => {
    await commonSetup(page);
    await createShapeElement(page, [0, 0], [100, 100], Shape.Square);
...
test.describe('quick connect', () => {
    test('should create a connector when clicking on button', async ({ page, }) => {
        await commonSetup(page);
        await createShapeElement(page, [0, 0], [100, 100], Shape.Square);
    ...
    test('should be uncreated if the target is not found after clicking', async ({ page, }) => {
        await commonSetup(page);
        await createShapeElement(page, [0, 0], [100, 100], Shape.Square);
        const [x, y] = await toViewCoord(page, [50, 50]);
        await page.mouse.click(x, y);
    ...
    test('should be uncreated if the target is not found after pressing ESC', async ({ page, }) => {
        await commonSetup(page);
        await createShapeElement(page, [0, 0], [100, 100], Shape.Square);
        // select shape
        const [x, y] = await toViewCoord(page, [50, 50]);
    ...
    test('should follow the mouse to automatically select the starting point', async ({ page, }) => {
        await commonSetup(page);
        await createShapeElement(page, [0, 0], [100, 100], Shape.Square);
    ...
    test('the triangle connectors should remain the same when switch to other shape', async ({ page, }) => {
        await commonSetup(page);
        const shape1Id = await createShapeElement(page, [0, 0], [100, 100], Shape.Triangle);
        const shape2Id = await createShapeElement(page, [200, 0], [300, 100], Shape.Triangle);
        await setEdgelessTool(page, 'connector');
    ...
    test('connector can not be moved directly if the source or target is not selected', async ({ page, }) => {
        await commonSetup(page);
        const normalConnectorId = await createConnectorElement(page, [0, 0], [100, 100]);
        ...
        const shape1 = await createShapeElement(page, [150, 150], [200, 200], Shape.Square);
        const shape2 = await createShapeElement(page, [250, 250], [300, 300], Shape.Square);
        const connectorWithShapes = await createConnectorElement(page, [190, 175], [260, 275]);
