...
export const NavigationPanelDocNode = ({ docId, onDrop, location, reorderable, isLinked, canDrop, operations: additionalOperations, dropEffect, parentPath, }) => {
    const t = useI18n();
    const { docsSearchService, workspaceService, docsService, globalContextService, docDisplayMetaService, featureFlagService, guardService, } = useServices({
        WorkspaceService,
        DocsSearchService,
        DocsService,
    ...
    const Icon = useCallback(({ className }) => {
        return _jsx(DocIcon, { className: className });
    }, [DocIcon]);
    const children = useLiveData(useMemo(() => LiveData.from(!isCollapsed ? docsSearchService.watchRefsFrom(docId) : NEVER, null), [docsSearchService, docId, isCollapsed]));
    ...
    const operations = useNavigationPanelDocNodeOperations(docId, useMemo(() => ({
    ...
    return (_jsx(NavigationPanelTreeNode, { icon: Icon, name: t.t(docTitle), dndData: dndData, onDrop: handleDropOnDoc, renameable: true, extractEmojiAsIcon: enableEmojiIcon, collapsed: isCollapsed, setCollapsed: setCollapsed, collapsible: !!appSettings.showLinkedDocInSidebar, canDrop: handleCanDrop, to: `/${docId}`, onClick: () => {
            track.$.navigationPanel.docs.openDoc();
        }, active: active, postfix: referencesLoading &&
            !isCollapsed && (_jsx(Tooltip, { content: t['com.affine.rootAppSidebar.docs.references-loading'](), children: _jsx("div", { className: styles.loadingIcon, children: _jsx(Loading, {}) }) })), reorderable: reorderable, renameableGuard: {
            docId,
            action: 'Doc_Update',
        }, onRename: handleRename, childrenPlaceholder: searching ? null : (_jsx(Empty, { onDrop: handleDropOnPlaceholder, noAccessible: !!children && children.length > 0 })), operations: finalOperations, dropEffect: handleDropEffectOnDoc, "data-testid": `navigation-panel-doc-${docId}`, explorerIconConfig: {
            where: 'doc',
            id: docId,
        }, children: appSettings.showLinkedDocInSidebar ? (_jsx(Guard, { docId: docId, permission: "Doc_Read", children: canRead => canRead
                ? children?.map((child, index) => (_jsx(NavigationPanelDocNode, { docId: child.docId, reorderable: false, location: {
                        at: 'navigation-panel:doc:linked-docs',
                        docId,
                    }, parentPath: path, isLinked: true }, `${child.docId}-${index}`)))
                : null })) : null }));
};
