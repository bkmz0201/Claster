...
export class WorkspaceTransformService extends Service {
    constructor(factory, destroy) {
         ...
         */
        this.transformLocalToCloud = async (local, accountId, flavour) => {
            if (local.flavour !== 'local') {
                throw new Error('Only local workspace can be transformed to cloud workspace');
            }
            const localDocStorage = local.engine.doc.storage;
            const localDocList = Array.from(local.docCollection.docs.keys());
            const newMetadata = await this.factory.create(flavour, async (docCollection, blobStorage, docStorage) => {
                const rootDocBinary = (await localDocStorage.getDoc(local.docCollection.doc.guid))?.bin;
                if (rootDocBinary) {
                ...
                // transform db
                await transformWorkspaceDBLocalToCloud(local.id, docCollection.id, localDocStorage, docStorage, accountId);
