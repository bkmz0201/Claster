...
export const EditorChatPanel = forwardRef(function EditorChatPanel({ editor, onLoad }, ref) {
    ...
    useEffect(() => {
        if (onLoad && chatPanelRef.current) {
            chatPanelRef.current.updateComplete
                .then(() => {
                if (ref) {
    ...
    const { docDisplayConfig, searchMenuConfig, networkSearchConfig, reasoningConfig, playgroundConfig, } = useAIChatConfig();
    const confirmModal = useConfirmModal();
    const specs = useAISpecs();
    const handleAISubscribe = useAISubscribe();
    useEffect(() => {
        if (!editor || !editor.host)
            return;
        if (!chatPanelRef.current) {
            ...
            chatPanelRef.current.affineWorkspaceDialogService = framework.get(WorkspaceDialogService);
            ...
            chatPanelRef.current.notificationService = new NotificationServiceImpl(confirmModal.closeConfirmModal, confirmModal.openConfirmModal);
    ...
    useEffect(() => {
        ...
        if (autoResized)
            return;
        const subscription = AIProvider.slots.previewPanelOpenChange.subscribe(open => {
            if (!open)
                return;
    ...
    return _jsx("div", { className: styles.root, ref: containerRef });
});
