...
import { createHTMLTargetConfig, createNodeTargetConfig, createWorkerTargetConfig, } from './webpack';
function getBaseWorkerConfigs(pkg) {
    const core = new Package('@affine/core');
    return [
        createWorkerTargetConfig(pkg, core.srcPath.join('modules/workspace-engine/impls/workspace-profile.worker.ts').value),
        createWorkerTargetConfig(pkg, core.srcPath.join('modules/pdf/renderer/pdf.worker.ts').value),
        createWorkerTargetConfig(pkg, core.srcPath.join('blocksuite/view-extensions/turbo-renderer/turbo-painter.worker.ts').value),
    ];
}
function getBundleConfigs(pkg) {
    switch (pkg.name) {
        case '@affine/admin': {
            return [
        ...
        case '@affine/android': {
            ...
            workerConfigs.push(createWorkerTargetConfig(pkg, pkg.srcPath.join('nbstore.worker.ts').value));
            return [
                createHTMLTargetConfig(pkg, pkg.srcPath.join('index.tsx').value, {}, workerConfigs.map(config => config.name)),
                ...workerConfigs,
            ];
        }
...
export class BundleCommand extends PackageCommand {
    constructor() {
        ...
        this._deps = false;
        this.waitDeps = false;
        this.dev = Option.Boolean('--dev,-d', false, {
            description: 'Run in Development mode',
        });
    }
    static { this.paths = [['bundle'], ['webpack'], ['pack'], ['bun']]; }
    async execute() {
    ...
    static async dev(pkg, devServerConfig) {
        ...
        const devServer = new WebpackDevServer(merge({}, defaultDevServerConfig, devServerConfig), compiler);
