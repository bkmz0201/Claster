...
import { getNoteBoundBoxInEdgeless, setEdgelessTool, switchEditorMode, } from '../../utils/actions/edgeless.js';
import { addBasicBrushElement, addBasicRectShapeElement, click, clickInCenter, clickView, dragBetweenCoords, enterPlaygroundRoom, getBoundingRect, initEmptyEdgelessState, initThreeParagraphs, pressEnter, pressEscape, undoByKeyboard, waitNextFrame, } from '../../utils/actions/index.js';
import { assertBlockCount, assertEdgelessRemoteSelectedModelRect, assertEdgelessRemoteSelectedRect, assertEdgelessSelectedModelRect, assertEdgelessSelectedRect, assertRectExist, assertSelectionInNote, } from '../../utils/asserts.js';
import { test } from '../../utils/playwright.js';
test('should update rect of selection when resizing viewport', async ({ page, }) => {
...
test('should update react of remote selection when resizing viewport', async ({ context, page: pageA, }) => {
    ...
    await actions.createShapeElement(pageA, [0, 0], [100, 100], actions.Shape.Square);
    ...
    await actions.decreaseZoomLevel(pageB);
    await waitNextFrame(pageB);
    const selectedRectInZoom = await getBoundingRect(pageB, '.affine-edgeless-selected-rect');
...
test('selection box of shape element sync on fast dragging', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyEdgelessState(page);
    await switchEditorMode(page);
    await actions.zoomResetByKeyboard(page);
    ...
    await dragBetweenCoords(page, { x: 110, y: 110 }, { x: 660, y: 460 }, { click: true });
    await assertEdgelessSelectedRect(page, [650, 450, 100, 100]);
});
test('when the selection is always a note, it should remain in an active state', async ({ page, }) => {
    await enterPlaygroundRoom(page);
...
test('should auto panning when selection rectangle reaches viewport edges', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyEdgelessState(page);
    await switchEditorMode(page);
    await actions.zoomResetByKeyboard(page);
...
test('should also update dragging area when viewport changes', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyEdgelessState(page);
    await switchEditorMode(page);
    await actions.zoomResetByKeyboard(page);
...
test('should the selected rect be below the edgeless element toolbar', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyEdgelessState(page);
    await switchEditorMode(page);
    ...
    expect(brHandleRect.x > toolbarRect.x &&
        brHandleRect.x < toolbarRect.x + toolbarRect.width &&
        brHandleRect.y > toolbarRect.y &&
        brHandleRect.y < toolbarRect.y + toolbarRect.height).toBe(true);
    ...
    const topElement = await page.evaluate(brHandleCenter => {
        const element = document.elementFromPoint(brHandleCenter[0], brHandleCenter[1]);
        if (!element) {
...
test('should the block selectable after undo drag a block from canvas to note', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyEdgelessState(page);
    await switchEditorMode(page);
    ...
    await page.dragAndDrop('.affine-drag-handle-grabber.dots', 'affine-edgeless-note[data-block-id="2"]');
    await waitNextFrame(page);
