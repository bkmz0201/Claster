...
export function patchForEdgelessNoteConfig(framework, reactToLit, insidePeekView) {
    return NoteConfigExtension({
        edgelessNoteHeader: ({ note }) => reactToLit(_jsx(EdgelessNoteHeader, { note: note })),
        pageBlockTitle: ({ note }) => {
            const journalService = framework.get(JournalService);
            const isJournal = !!journalService.journalDate$(note.store.id).value;
            if (isJournal) {
                return reactToLit(_jsx(BlocksuiteEditorJournalDocTitle, { page: note.store }));
            }
            else {
        ...
        pageBlockViewportFitAnimation: insidePeekView
            ...
            : ({ std, note }) => {
                ...
                const leftPadding = parseInt(window
                ...
                let editorWidth = parseInt(window
                    .getComputedStyle(rootView)
                ...
                const leftMargin = containerWidth > editorWidth
                    ? (containerWidth - editorWidth) / 2
                    : 0;
                const pageTitleAnchor = gfx.viewport.toModelCoord(leftPadding + leftMargin, 0);
...
export function patchForEmbedSyncedDocConfig(reactToLit) {
    return EmbedSyncedDocConfigExtension({
        edgelessHeader: ({ model, std }) => reactToLit(_jsx(EdgelessEmbedSyncedDocHeader, { model: model, std: std })),
    });
}
