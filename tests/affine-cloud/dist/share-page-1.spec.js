...
import { getBlockSuiteEditorTitle, waitForEditorLoad, } from '@affine-test/kit/utils/page-logic';
...
test('can enable share page', async ({ page, browser }) => {
    ...
    // check share page is accessible
    {
        ...
        await expect(page2.locator('affine-paragraph').first()).toContainText('TEST CONTENT');
    }
});
test('share page should have toc', async ({ page, browser }) => {
    ...
    // check share page is accessible
    {
        ...
        await page2.goto(url);
        await waitForEditorLoad(page2);
        const tocIndicators = page2.locator('affine-outline-viewer .outline-viewer-indicator');
        ...
        const toggleButton = viewer.locator('[data-testid="toggle-outline-panel-button"]');
        await expect(toggleButton).toHaveCount(0);
    }
});
test('append paragraph should be disabled in shared mode', async ({ page, browser, }) => {
    await page.reload();
    await waitForEditorLoad(page);
    await createLocalWorkspace({
        name: 'test',
...
//# sourceMappingURL=share-page-1.spec.js.map