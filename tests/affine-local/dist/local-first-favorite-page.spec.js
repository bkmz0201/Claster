...
import { clickNewPageButton, clickPageMoreActions, getBlockSuiteEditorTitle, getPageByTitle, waitForEditorLoad, } from '@affine-test/kit/utils/page-logic';
import { expect } from '@playwright/test';
test('New a page and open it, then favorite it', async ({ page, workspace, }) => {
...
test('Cancel favorite', async ({ page, workspace }) => {
    ...
    await expect(favorites.getByText('this is a new page to favorite')).toBeVisible();
    // cancel favorite
    await page.getByTestId('all-pages').click();
    ...
    await expect(favorites.getByTestId('slider-bar-favorites-empty-message')).toBeVisible();
    const currentWorkspace = await workspace.current();
    expect(currentWorkspace.meta.flavour).toContain('local');
});
//# sourceMappingURL=local-first-favorite-page.spec.js.map