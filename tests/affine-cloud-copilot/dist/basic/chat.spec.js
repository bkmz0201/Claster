...
test.describe('AIBasic/Chat', () => {
    ...
    test('should display empty state when no messages', async ({ loggedInPage: page, }) => {
    ...
    `, async ({ loggedInPage: page, utils }) => {
        ...
        await utils.chatPanel.makeChat(page, 'Introduce AFFiNE to me. Answer in 500 words.');
    ...
    test('should support stop generating', async ({ loggedInPage: page, utils, }) => {
        await utils.chatPanel.makeChat(page, 'Introduce AFFiNE to me. Answer in 5000 words.');
        // AI Generating
        await utils.chatPanel.waitForHistory(page, [
            {
                role: 'user',
    ...
    test('should render ai actions inline if the answer is the last one in the list, otherwise, nest them under the "More" menu', async ({ loggedInPage: page, utils, }) => {
        await utils.chatPanel.makeChat(page, 'Hello, how can you help me? Answer in 50 words.');
        await utils.chatPanel.waitForHistory(page, [
            {
                role: 'user',
        ...
        await utils.chatPanel.makeChat(page, 'Nice to meet you. Answer in 50 words.');
        await utils.chatPanel.waitForHistory(page, [
            {
                role: 'user',
                content: 'Hello, how can you help me? Answer in 50 words.',
    ...
    test('should show error when request failed', async ({ loggedInPage: page, utils, }) => {
    ...
    test('should support retrying failed messages', async ({ loggedInPage: page, utils, }) => {
        // Simulate network error by disconnecting
        await page.route('**/graphql', route => route.abort('failed'));
        // Send a message that will fail
        await utils.chatPanel.makeChat(page, 'Hello. Answer in 50 words.');
    ...
    test('should support retrying question', async ({ loggedInPage: page, utils, }) => {
        await utils.chatPanel.makeChat(page, 'Introduce Large Language Model. Answer in 50 words.');
        await utils.chatPanel.waitForHistory(page, [
            {
                role: 'user',
    ...
    test('should support sending message with button', async ({ loggedInPage: page, utils, }) => {
        await utils.chatPanel.openChatPanel(page);
    ...
    test('should support copying answer', async ({ loggedInPage: page, utils, }) => {
        await utils.chatPanel.openChatPanel(page);
        await utils.chatPanel.makeChat(page, 'Hello. Answer in 50 words.');
        await utils.chatPanel.waitForHistory(page, [
            {
    ...
    test('should support copying selected answer content', async ({ loggedInPage: page, utils, }) => {
        await utils.chatPanel.openChatPanel(page);
        await utils.chatPanel.makeChat(page, 'Help me write a two-line love poem. Answer in 50 words.');
        await utils.chatPanel.waitForHistory(page, [
            {
                role: 'user',
        ...
        await expect(async () => {
            ...
            if (firstParagraph && lastParagraph) {
                ...
                await page.mouse.move(lastParagraph.x + lastParagraph.width, lastParagraph.y + lastParagraph.height);
                await page.mouse.up();
            }
    ...
    test('chat with ask ai input in page mode', async ({ loggedInPage: page, utils, }) => {
        await utils.chatPanel.closeChatPanel(page);
        await utils.editor.askAIWithText(page, 'AFFiNE is an open source all in one workspace.');
        ...
        await utils.chatPanel.waitForHistory(page, [
            {
                role: 'user',
                content: 'AFFiNE is an open source all in one workspace.\nTranslate to chinese.',
            },
            {
                role: 'assistant',
                status: 'success',
    ...
    test('chat with ask ai input in edgeless mode', async ({ loggedInPage: page, utils, }) => {
        await utils.chatPanel.closeChatPanel(page);
    ...
    test('should support chat with ask ai input in edgeless mode when nothing selected', async ({ loggedInPage: page, utils, }) => {
        await utils.chatPanel.closeChatPanel(page);
    ...
    test('should support create a new chat after ask ai', async ({ loggedInPage: page, utils, }) => {
        await utils.chatPanel.closeChatPanel(page);
        await utils.editor.askAIWithText(page, 'AFFiNE is an open source all in one workspace.');
        await page.keyboard.type('Translate to chinese.');
        const sendButton = await page.getByTestId('ai-panel-input-send');
    ...
    test('should support pin chat', async ({ loggedInPage: page, utils }) => {
        ...
        // pinned
        await expect(page.getByTestId('ai-panel-pin-chat')).toHaveAttribute('data-pinned', 'false');
        await page.getByTestId('ai-panel-pin-chat').click();
        await expect(page.getByTestId('ai-panel-pin-chat')).toHaveAttribute('data-pinned', 'true');
        // create new doc
        await utils.editor.createDoc(page, 'Doc 1', 'doc1');
        await utils.chatPanel.expectToHaveHistory(page, [
            {
