 ...
 */
export const routeModulePaths = ['all', 'collection', 'tag', 'trash'];
const isRouteModulePath = (path) => routeModulePaths.includes(path);
export const resolveLinkToDoc = (href, baseUrl) => {
    const meta = resolveRouteLinkMeta(href, baseUrl);
    if (!meta || meta.moduleName !== 'doc')
        return null;
...
export function toDocSearchParams(params) {
