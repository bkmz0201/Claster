...
export class StringInvertedIndex {
    constructor(table, fieldKey) {
        this.table = table;
        this.fieldKey = fieldKey;
    }
    async match(trx, term) {
    ...
    async all(trx) {
        const objs = await trx
            .objectStore('invertedIndex')
            .index('key')
            .getAll(IDBKeyRange.bound([this.table, InvertedIndexKey.forPrefix(this.fieldKey).buffer()], [
            this.table,
    ...
    async insert(trx, id, terms) {
        const uniqueTerms = new Set(terms);
        await Promise.all(Array.from(uniqueTerms).map(term => trx.objectStore('invertedIndex').put({
            table: this.table,
...
export class IntegerInvertedIndex {
    constructor(table, fieldKey) {
        this.table = table;
        this.fieldKey = fieldKey;
    }
...
export class BooleanInvertedIndex {
    constructor(table, fieldKey) {
        this.table = table;
        this.fieldKey = fieldKey;
    }
    ...
    async insert(trx, id, terms) {
        ...
        await Promise.all(Array.from(uniqueTerms).map(term => trx.objectStore('invertedIndex').put({
            table: this.table,
            key: InvertedIndexKey.forBoolean(this.fieldKey, term === 'true').buffer(),
            nid: id,
        })));
    }
}
export class FullTextInvertedIndex {
    constructor(table, fieldKey) {
        this.table = table;
        this.fieldKey = fieldKey;
    }
    async match(trx, term) {
        ...
        const avgFieldLength = (await trx
            .objectStore('indexerMetadata')
            .get(`full-text:avg-field-length:${this.table}:${this.fieldKey}`))?.value ?? 0;
        for (const token of queryTokens) {
            ...
            const objs = [
                ...
                ...(await trx
                    .objectStore('invertedIndex')
                    .index('key')
                    .getAll(IDBKeyRange.bound([this.table, key.buffer()], [this.table, key.add1().buffer()], true, true), 5000 // get maximum 5000 items for prefix match
                )),
            ];
            const submatched = [];
            for (const obj of objs) {
                ...
                for (const position of positions) {
                    ...
                    const score = bm25(termFreq, 1, totalCount, fieldLength, avgFieldLength) *
    ...
    async insert(trx, id, terms) {
        ...
        for (const [term, positions] of globalTokenMap) {
            promises.push(trx.objectStore('invertedIndex').put({
                table: this.table,
                key: InvertedIndexKey.forString(this.fieldKey, term).buffer(),
                nid: id,
                pos: positions,
        ...
        const newAvgFieldLength = (avgFieldLength * totalCount + totalTermLength) / newTotalCount;
        promises.push(indexerMetadataStore.put({
            key: countKey,
            value: newTotalCount,
        }));
        promises.push(indexerMetadataStore.put({
...
export class InvertedIndexKey {
    constructor(field, value, gap = new Uint8Array([58])) {
        this.field = field;
        this.value = value;
        this.gap = gap;
    }
    ...
    asInt64() {
        return new DataView(this.value.buffer).getBigInt64(0, false); /* big-endian */
    }
    add1() {
        if (this.value.byteLength > 0) {
            ...
            return new InvertedIndexKey(this.field, new Uint8Array(0), new Uint8Array([59]));
        }
    }
    static forPrefix(field) {
        return new InvertedIndexKey(new TextEncoder().encode(field), new Uint8Array(0));
    }
    static forString(field, value) {
        return new InvertedIndexKey(new TextEncoder().encode(field), new TextEncoder().encode(value));
    }
    static forBoolean(field, value) {
        const bytes = new Uint8Array(1);
        bytes.set([value ? 1 : 0]);
        return new InvertedIndexKey(new TextEncoder().encode(field), bytes);
    }
    static forInt64(field, value) {
        const bytes = new Uint8Array(8);
        new DataView(bytes.buffer).setBigInt64(0, value, false); /* big-endian */
        return new InvertedIndexKey(new TextEncoder().encode(field), bytes);
    }
    buffer() {
        const tmp = new Uint8Array(this.field.byteLength + (this.value?.byteLength ?? 0) + 1);
