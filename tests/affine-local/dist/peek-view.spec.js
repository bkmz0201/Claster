...
import { clickEdgelessModeButton, getSelectedXYWH, getViewportBound, } from '@affine-test/kit/utils/editor';
import { pressEnter, pressEscape } from '@affine-test/kit/utils/keyboard';
import { openHomePage } from '@affine-test/kit/utils/load-page';
import { clickNewPageButton, createLinkedPage, createSyncedPageInEdgeless, type, waitForEmptyEditor, } from '@affine-test/kit/utils/page-logic';
...
test('can open peek view via db+click link card', async ({ page }) => {
    ...
    await expect(page.locator('affine-embed-linked-doc-block:has-text("Test Page")')).toBeVisible();
    ...
    await expect(page
        ...
        .locator('doc-title:has-text("Test Page")')).toBeVisible();
});
test('should not open peek view when content is covered by canvas element', async ({ page, }) => {
    await page.keyboard.press('Enter');
    await clickEdgelessModeButton(page);
    await createSyncedPageInEdgeless(page, 'Test Page');
    const syncedDocBlock = await page.locator('affine-embed-edgeless-synced-doc-block');
    ...
    await page.mouse.move(syncedDocRect.x + syncedDocRect.width, syncedDocRect.y + syncedDocRect.height, {
        steps: 10,
    });
    await page.mouse.up();
    await page.waitForTimeout(100);
...
test('can open peek view for embedded frames', async ({ page }) => {
    const frameInViewport = async () => {
        ...
        return (frameXYWH[0] >= viewportBound[0] &&
    ...
    await page
        ...
        .click();
    await page
        .locator('affine-toolbar-widget editor-menu-action[aria-label="Open in center peek"]')
        .click();
...
test.skip('can open peek view for fav link', async ({ page }) => {
    ...
    // verify peek view is opened
    await expect(page.getByTestId('peek-view-modal')).toBeVisible();
    await expect(page.getByTestId('peek-view-modal')).toContainText('test page title');
    // can close modal when navigate
    await openHomePage(page);
    await expect(page.getByTestId('peek-view-modal')).not.toBeVisible();
});
//# sourceMappingURL=peek-view.spec.js.map