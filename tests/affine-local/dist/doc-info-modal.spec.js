...
import { clickNewPageButton, clickPageMoreActions, getBlockSuiteEditorTitle, getPageByTitle, getPageOperationButton, waitForEmptyEditor, } from '@affine-test/kit/utils/page-logic';
import { addCustomProperty, closeTagsEditor, ensurePagePropertiesVisible, expectTagsVisible, filterTags, removeSelectedTag, } from '@affine-test/kit/utils/properties';
import { getCurrentDocIdFromUrl } from '@affine-test/kit/utils/url';
import { expect } from '@playwright/test';
const searchAndCreateTag = async (page, name) => {
...
test('New a page and open it ,then open info modal in the title bar', async ({ page, }) => {
...
test('New a page and open it ,then open info modal in the title bar more action button', async ({ page, }) => {
...
test('New a page and add to favourites, then open info modal from sidebar', async ({ page, }) => {
    const newPageId = getCurrentDocIdFromUrl(page);
    await clickPageMoreActions(page);
    await page.getByTestId('editor-option-menu-favorite').click();
    await page.getByTestId('all-pages').click();
    const favoriteListItemInSidebar = page.locator(`[data-testid="navigation-panel-favorites"] [data-testid="navigation-panel-doc-${newPageId}"]`);
    expect(await favoriteListItemInSidebar.textContent()).toBe('this is a new page');
...
//# sourceMappingURL=doc-info-modal.spec.js.map