...
import { lazy as reactLazy, Suspense, useEffect, useMemo, useRef, useState, } from 'react';
 ...
 **/
const MobileRouteContainer = ({ route }) => {
    return (_jsx(AffineErrorBoundary, { children: _jsx(Suspense, { children: _jsx(route.Component, {}) }) }));
};
const warpedRoutes = workbenchRoutes.map((originalRoute) => {
    ...
    return {
        path,
        Component: () => {
            return _jsx(MobileRouteContainer, { route: route });
        },
...
export const Component = () => {
    ...
    if (workspaceNotFound) {
        if (BUILD_CONFIG.isMobileWeb /* only browser has share page */ &&
            detailDocRoute) {
    ...
    return (_jsx(WorkspaceLayout, { meta: meta, children: _jsx(MobileWorkbenchRoot, { routes: warpedRoutes }) }));
};
