...
function extractPageIcon(doc) {
    ...
    if (pageHeader) {
        console.log('Found .page-header-icon:', pageHeader.outerHTML.substring(0, 300) + '...');
    }
    ...
    if (emojiIcon && emojiIcon.textContent) {
        console.log('Found emoji icon (.page-header-icon .notion-emoji):', emojiIcon.textContent);
        return {
            type: 'emoji',
    ...
    if (altEmojiIcon &&
        ...
        console.log('Found emoji icon ([role="img"][aria-label]):', altEmojiIcon.textContent);
        return {
            type: 'emoji',
    ...
    for (const span of iconSpans) {
        if (span.textContent && /\p{Emoji}/u.test(span.textContent.trim())) {
            ...
            console.log('Found emoji in span.icon:', span.textContent, 'parent classes:', parent?.className);
            // Check if this is in a page header context
            if (parent &&
                (parent.classList.contains('page-header-icon') ||
                    parent.closest('.page-header-icon'))) {
                console.log('Using emoji from span.icon in page header:', span.textContent);
                return {
                    type: 'emoji',
 ...
 */
async function importNotionZip({ collection, schema, imported, extensions, }) {
