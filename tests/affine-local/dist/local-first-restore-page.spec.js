...
import { clickNewPageButton, getBlockSuiteEditorTitle, getPageByTitle, getPageOperationButton, waitForEditorLoad, } from '@affine-test/kit/utils/page-logic';
import { getCurrentDocIdFromUrl } from '@affine-test/kit/utils/url';
import { expect } from '@playwright/test';
test('New a page , then delete it in all pages, restore it', async ({ page, workspace, }) => {
    ...
    await page.getByTestId('all-pages').click();
    const restoreCell = await getPageByTitle(page, 'this is a new page to restore');
    await expect(restoreCell).toBeVisible();
    const currentWorkspace = await workspace.current();
    expect(currentWorkspace.meta.flavour).toContain('local');
});
//# sourceMappingURL=local-first-restore-page.spec.js.map