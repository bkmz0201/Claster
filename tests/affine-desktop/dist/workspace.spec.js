...
import { getBlockSuiteEditorTitle, getPageByTitle, waitForEditorLoad, } from '@affine-test/kit/utils/page-logic';
import { clickNewPageButton, clickSideBarCurrentWorkspaceBanner, } from '@affine-test/kit/utils/sidebar';
import { createLocalWorkspace } from '@affine-test/kit/utils/workspace';
import { expect } from '@playwright/test';
import fs from 'fs-extra';
test('check workspace has a DB file', async ({ appInfo, workspace }) => {
    const w = await workspace.current();
    const dbPath = path.join(appInfo.sessionData, 'workspaces', 'local', w.meta.id, 'storage.db');
...
test('delete workspace and then restore it from backup', async ({ page }) => {
    ...
    const toast = page.locator('[data-sonner-toast]:has-text("Workspace enabled successfully")');
