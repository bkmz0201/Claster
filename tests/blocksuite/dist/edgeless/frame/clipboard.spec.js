...
import { autoFit, createFrame as _createFrame, createShapeElement, deleteAll, dragBetweenViewCoords, edgelessCommonSetup, getAllSortedIds, getFirstContainerId, getIds, Shape, shiftClickView, triggerComponentToolbarAction, zoomResetByKeyboard, } from '../../utils/actions/edgeless.js';
import { copyByKeyboard, pasteByKeyboard, pressBackspace, pressEscape, } from '../../utils/actions/keyboard.js';
...
test.describe('frame copy and paste', () => {
    test('copy of frame should keep relationship of child elements', async ({ page, }) => {
    ...
    test('copy of frame by alt/option dragging should keep relationship of child elements', async ({ page, }) => {
        await createFrame(page, [50, 50], [450, 450]);
        await createShapeElement(page, [200, 200], [300, 300], Shape.Square);
        ...
        const shapeIds = (await getIds(page)).filter(id => ![frameId, groupId].includes(id));
    ...
    test('copy of element by alt/option dragging in frame should belong to frame', async ({ page, }) => {
        await createFrame(page, [50, 50], [450, 450]);
        await createShapeElement(page, [100, 100], [200, 200], Shape.Square);
        await pressEscape(page);
    ...
    test('copy of element by alt/option dragging out of frame should not belong to frame', async ({ page, }) => {
        await createFrame(page, [50, 50], [450, 450]);
        await createShapeElement(page, [100, 100], [200, 200], Shape.Square);
        await pressEscape(page);
        await clickView(page, [150, 150]);
