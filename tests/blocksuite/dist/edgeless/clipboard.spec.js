...
import { createNote, createShapeElement, decreaseZoomLevel, deleteAll, getAllSortedIds, Shape, switchEditorMode, toViewCoord, triggerComponentToolbarAction, } from '../utils/actions/edgeless.js';
import { copyByKeyboard, cutByKeyboard, edgelessCommonSetup as commonSetup, enterPlaygroundRoom, expectConsoleMessage, focusTitle, getCurrentEditorDocId, initEmptyEdgelessState, mockParseDocUrlService, pasteByKeyboard, pasteContent, selectAllByKeyboard, type, waitNextFrame, } from '../utils/actions/index.js';
...
test.describe('pasting URLs', () => {
    test('pasting github pr url', async ({ page }) => {
        ...
        await expect(page.locator('affine-embed-edgeless-github-block')).toBeVisible();
    });
    test('pasting internal link', async ({ page }) => {
        ...
        await expect(page.locator('affine-embed-edgeless-linked-doc-block')).toBeVisible();
        await expect(page.locator('.affine-embed-linked-doc-content-title')).toHaveText('doc title');
    });
