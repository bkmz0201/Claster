...
import { addNote, locatorScalePanelButton, selectNoteInEdgeless, switchEditorMode, triggerComponentToolbarAction, zoomResetByKeyboard, } from '../../utils/actions/edgeless.js';
import { copyByKeyboard, pasteByKeyboard, selectAllByKeyboard, } from '../../utils/actions/keyboard.js';
...
async function setupAndAddNote(page) {
...
async function openScalePanel(page, noteId) {
...
async function checkNoteScale(page, noteId, expectedScale, expectedType = 'equal') {
...
test.describe('note scale', () => {
    ...
    test('Note scale can be changed by shift drag', async ({ page }) => {
        ...
        await page.mouse.move(noteRect.x + noteRect.width, noteRect.y + noteRect.height);
        await page.keyboard.down('Shift');
        await page.mouse.down();
        await page.mouse.move(noteRect.x + noteRect.width * 2, noteRect.y + noteRect.height * 2, {
            steps: 10,
        });
        await page.mouse.up();
