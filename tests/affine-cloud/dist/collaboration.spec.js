...
import { addUserToWorkspace, createRandomUser, enableCloudWorkspace, loginUser, } from '@affine-test/kit/utils/cloud';
...
test.skip('can collaborate with other user and name should display when editing', async ({ page, browser, }) => {
    await page.reload();
...
test('can sync collections between different browser', async ({ page, browser, }) => {
    await page.reload();
    await waitForEditorLoad(page);
    await createLocalWorkspace({
        name: 'test',
...
test('can sync svg between different browsers', async ({ page, browser }) => {
    ...
    const svg1 = await page.evaluate(src => fetch(src)
        .then(res => res.blob())
        .then(blob => blob.text()), src1);
    {
        const context = await browser.newContext();
        await skipOnboarding(context);
        const page2 = await context.newPage();
        ...
        const svg2 = await page2.evaluate(src => fetch(src)
            .then(res => res.blob())
            .then(blob => blob.text()), src2);
        expect(svg2).toEqual(svg1);
    }
});
//# sourceMappingURL=collaboration.spec.js.map