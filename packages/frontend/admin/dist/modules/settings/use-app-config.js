...
export const useAppConfig = () => {
    const { data: { appConfig }, mutate, } = useQuery({
    ...
    const [patchedAppConfig, setPatchedAppConfig] = useState(() => cloneDeep(appConfig));
    const save = useAsyncCallback(async () => {
        const updateInputs = Object.entries(updates).map(([key, value]) => {
    ...
    const update = useCallback((path, value) => {
        ...
        setPatchedAppConfig(prev => {
            return set(prev, `${module}.${field}${subField ? `.${subField}` : ''}`, value);
        });
    }, [appConfig]);
    return {
        appConfig: appConfig,
