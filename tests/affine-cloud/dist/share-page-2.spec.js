...
import { createRandomUser, enableCloudWorkspaceFromShareButton, enableShare, loginUser, } from '@affine-test/kit/utils/cloud';
...
test('The reference links in the shared page should be accessible normally and can go back and forward', async ({ page, browser, }) => {
    await page.reload();
    ...
    // check share page is accessible
    {
        ...
        await link.click();
        await waitForEditorLoad(page2);
        await expect(page2.locator('.doc-title-container:has-text("Test linked doc")')).toBeVisible();
        await expect(page2.locator('affine-paragraph').first()).toContainText('Test linked content');
        // go back and forward
        await page2.goBack();
        await waitForEditorLoad(page2);
        await expect(title).toContainText('Test Page');
...
test('Should show no permission page when the share page is not found', async ({ page, }) => {
    await page.goto('http://localhost:8080/workspace/abc/123');
    await expect(page.getByText('You do not have access or this content does not exist.')).toBeVisible();
});
test('Inline latex modal should be not shown in shared mode when clicking', async ({ page, browser, }) => {
    await page.reload();
    await waitForEditorLoad(page);
    await createLocalWorkspace({
        name: 'test',
...
test('share page should support copying content', async ({ page, browser }) => {
    ...
    // check share page is accessible and content can be copied
    {
        ...
        const copiedText = await page2.evaluate(() => navigator.clipboard.readText());
        expect(copiedText).toContain('Hello World');
    }
});
test('should enable opening peek view with pdf viewer in readonly and sharing modes', async ({ page, browser, }) => {
    await page.reload();
    await waitForEditorLoad(page);
    await createLocalWorkspace({
        name: 'test',
...
//# sourceMappingURL=share-page-2.spec.js.map