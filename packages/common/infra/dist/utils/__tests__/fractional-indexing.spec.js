...
function gen(a, b) {
    ...
    expect(a === null || b === null || (a < result && result < b), `${a} ${b} ${result}`).toBe(true);
    return result;
}
...
test('no postfix', () => {
    expect(generateFractionalIndexingKeyBetween('a0', null).startsWith('a1')).toBe(true);
    expect(generateFractionalIndexingKeyBetween('a01', null).startsWith('a1')).toBe(true);
    expect(generateFractionalIndexingKeyBetween('a0001', null).startsWith('a1')).toBe(true);
    expect(generateFractionalIndexingKeyBetween(null, 'a0').startsWith('Zz')).toBe(true);
    expect(generateFractionalIndexingKeyBetween('a0', 'a01').startsWith('a00V')).toBe(true);
});
test('edge cases', () => {
    for (let i = 0; i < 100; i++) {
...
//# sourceMappingURL=fractional-indexing.spec.js.map