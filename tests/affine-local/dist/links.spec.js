...
import { clickEdgelessModeButton, locateToolbar, } from '@affine-test/kit/utils/editor';
import { pasteByKeyboard, pressArrowUp, pressBackspace, pressEnter, selectAllByKeyboard, writeTextToClipboard, } from '@affine-test/kit/utils/keyboard';
import { coreUrl, openHomePage } from '@affine-test/kit/utils/load-page';
import { clickNewPageButton, createLinkedPage, createTodayPage, getBlockSuiteEditorTitle, type, waitForEmptyEditor, } from '@affine-test/kit/utils/page-logic';
import { confirmExperimentalPrompt, openEditorSetting, openExperimentalFeaturesPanel, } from '@affine-test/kit/utils/setting';
...
async function enableEmojiDocIcon(page) {
...
async function notClickable(locator) {
    await expect(locator).toHaveAttribute('disabled', '');
}
async function clickable(locator) {
    await expect(locator).not.toHaveAttribute('disabled', '');
}
test('not allowed to switch to embed view when linking to the same document', async ({ page, }) => {
    await page.keyboard.press('Enter');
    const url0 = new URL(page.url());
    await writeTextToClipboard(page, url0.toString());
    const { toolbar, switchViewBtn, inlineViewBtn, cardViewBtn, embedViewBtn } = toolbarButtons(page);
    // Inline
...
test('not allowed to switch to embed view when linking to block', async ({ page, }) => {
    await page.keyboard.press('Enter');
...
test('allow switching to embed view when linking to the other document without mode', async ({ page, }) => {
    await page.keyboard.press('Enter');
    await createLinkedPage(page, 'Test Page');
    const { switchViewBtn, inlineViewBtn, cardViewBtn, embedViewBtn } = toolbarButtons(page);
    // Inline
    const inlineLink = page.locator('affine-reference');
    await inlineLink.hover();
    await switchViewBtn.click();
    ...
    // Closes
    await switchViewBtn.click();
    await expect(page.locator('.affine-embed-synced-doc-container.page')).toBeVisible();
    await embedLink.click();
    await embedLink.click();
    await switchViewBtn.click();
    await clickable(inlineViewBtn);
...
test('allow switching to embed view when linking to the other document with mode', async ({ page, }) => {
    await page.keyboard.press('Enter');
    await createLinkedPage(page, 'Test Page');
    ...
    // Closes
    await switchViewBtn.click();
    await expect(page.locator('.affine-embed-synced-doc-container.edgeless')).toBeVisible();
    // refocus
    await embedLink.click();
    await switchViewBtn.click();
    await clickable(inlineViewBtn);
...
test('@ popover should show today menu item', async ({ page }) => {
    ...
    await expect(page.locator('affine-reference:has-text("' + date + '")')).toBeVisible();
});
test('@ popover with input=tmr', async ({ page }) => {
    ...
    // a affine-reference should be created with name date
    await expect(page.locator('affine-reference:has-text("' + textContent + '")')).toBeVisible();
});
test('@ popover with input=dec should create a reference with a December date', async ({ page, }) => {
    await page.keyboard.press('Enter');
    await waitForEmptyEditor(page);
    await page.keyboard.press('@');
    await page.keyboard.type('dc');
    const decemberMenuItem = page.locator('.linked-doc-popover icon-button:has-text("Dec")');
...
test('@ popover with click "select a specific date" should show a date picker', async ({ page, }) => {
    await page.keyboard.press('Enter');
    await waitForEmptyEditor(page);
    await page.keyboard.press('@');
    const todayMenuItem = page.locator('.linked-doc-popover').getByText('Today');
    ...
    await expect(page.locator('[data-is-date-cell][data-is-today=true]')).toBeVisible();
    await page.locator('[data-is-date-cell][data-is-today=true]').click();
    // a affine-reference should be created with name date
    await expect(page.locator('affine-reference:has-text("' + date + '")')).toBeVisible();
});
test('@ popover can auto focus on the "New Doc" item when query returns no items', async ({ page, }) => {
    await page.keyboard.press('Enter');
    await waitForEmptyEditor(page);
    await page.keyboard.press('@');
...
test('linked doc should show markdown preview in the backlink section', async ({ page, }) => {
    await waitForEmptyEditor(page);
    ...
    await expect(page.locator('text-renderer')).toContainText('some inline content');
    await expect(page.locator('text-renderer')).toContainText('Test Page');
});
test('the viewport should be fit when the linked document is with edgeless mode', async ({ page, }) => {
    await page.keyboard.press('Enter');
    ...
    expect(viewport.x + viewport.width).toBeGreaterThanOrEqual(edgelessText.x + edgelessText.width);
    expect(viewport.y + viewport.height).toBeGreaterThanOrEqual(edgelessText.y + edgelessText.height);
});
test('should show edgeless content when switching card view of linked mode doc in edgeless', async ({ page, }) => {
    await page.keyboard.press('Enter');
    await clickEdgelessModeButton(page);
    const note = page.locator('affine-edgeless-note');
    const noteBoundingBox = await note.boundingBox();
    ...
    const url = new URL(page.url());
    await clickNewPageButton(page);
    const closeAdButton = page.locator('[data-testid="local-demo-tips-close-button"]');
...
test.describe('Customize linked doc title and description', () => {
    ...
    test('should set a custom title for inline link', async ({ page }) => {
        ...
        const url1 = await (await page.evaluateHandle(() => navigator.clipboard.readText())).jsonValue();
    ...
    // Card View
    test('should set a custom title and description for card link', async ({ page, }) => {
        await page.keyboard.press('Enter');
        await createLinkedPage(page, 'Test Page');
        const { toolbar, switchViewBtn, inlineViewBtn, cardViewBtn } = toolbarButtons(page);
        const inlineLink = page.locator('affine-reference');
        await inlineLink.hover();
        // Copies link
        await toolbar.getByRole('button', { name: 'Copy link' }).click();
        const url0 = await (await page.evaluateHandle(() => navigator.clipboard.readText())).jsonValue();
        await page.waitForTimeout(200);
        await inlineLink.hover();
        // Edits title
        await toolbar.getByRole('button', { name: 'Edit' }).click();
        ...
        await cardViewBtn.click();
        const cardLink = page.locator('affine-embed-linked-doc-block');
        const cardTitle = cardLink.locator('.affine-embed-linked-doc-content-title-text');
        const cardDescription = cardLink.locator('.affine-embed-linked-doc-content-note.alias');
        await cardLink.click();
        await cardLink.click();
        // Copies link
        await toolbar.getByRole('button', { name: 'Copy link' }).click();
        ...
        // Switches to inline view
        {
            ...
            await page.mouse.click(bounds.x + bounds.width + 30, bounds.y + bounds.height / 2);
            await inlineLink.hover();
...
test('should save open doc mode of internal links', async ({ page }) => {
    ...
    const inlineLink = page.locator('affine-reference');
    await inlineLink.hover();
    const recentOpenModeBtn = toolbar.locator('editor-icon-button:has-text("Open")');
    await expect(recentOpenModeBtn).toHaveAttribute('aria-label', 'Open this doc');
    await expect(recentOpenModeBtn.locator('span.label:has-text("Open")')).toBeVisible();
    ...
    await inlineLink.hover();
    await expect(toolbar).toBeVisible();
    await expect(recentOpenModeBtn).toHaveAttribute('aria-label', 'Open in center peek');
    await switchViewBtn.click();
    await cardViewBtn.click();
    await expect(toolbar).toBeVisible();
    await expect(recentOpenModeBtn).toHaveAttribute('aria-label', 'Open in center peek');
...
test('should show full email address', async ({ page }) => {
    ...
    await expect(toolbar.locator('affine-link-preview')).toHaveText('dev@affine.pro');
});
test('should not show view toggle button when protocol of link is not http(s)', async ({ page, }) => {
    await page.keyboard.press('Enter');
...
test('should reach target block when clicking affine-link multiple times', async ({ page, }) => {
    await page.keyboard.press('Enter');
...
test('should display date as the original title of journal', async ({ page, }) => {
    await page.keyboard.press('Enter');
    await createTodayPage(page);
...
test('should open link in new tab when middle clicking on link', async ({ page, context, }) => {
