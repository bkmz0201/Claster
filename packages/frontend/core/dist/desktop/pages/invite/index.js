...
import { AcceptInvitePage, ExpiredPage, JoinFailedPage, RequestToJoinPage, SentRequestPage, } from '@affine/component/member-components';
...
const AcceptInvite = ({ inviteId: targetInviteId }) => {
    ...
    if (error || acceptError) {
        return (_jsx(JoinFailedPage, { inviteInfo: inviteInfo, error: error || acceptError }));
    }
    // for email invite
    if (accepted && inviteInfo?.status === WorkspaceMemberStatus.Accepted) {
        return (_jsx(AcceptInvitePage, { onOpenWorkspace: openWorkspace, inviteInfo: inviteInfo }));
    }
    if (inviteInfo?.status === WorkspaceMemberStatus.UnderReview ||
        ...
        return _jsx(SentRequestPage, { user: user, inviteInfo: inviteInfo });
    }
    return (_jsx(RequestToJoinPage, { user: user, inviteInfo: inviteInfo, requestToJoin: requestToJoin, onSignOut: onSignOut }));
};
/**
...
export const Component = () => {
    ...
    if (!params.inviteId) {
        return _jsx(Navigate, { to: "/expired" });
    }
    if (loginStatus === 'authenticated') {
        return _jsx(AcceptInvite, { inviteId: params.inviteId });
    }
    return null;
};
