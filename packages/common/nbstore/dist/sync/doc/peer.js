...
function createJobErrorCatcher(jobs) {
    return Object.fromEntries(Object.entries(jobs).map(([k, fn]) => {
        return [
            k,
            async (docId, ...args) => {
                try {
                    ...
                    if (err instanceof Error) {
                        throw new Error(`Error in job "${k}": ${err.stack || err.message}`);
                    }
                    else {
                        throw err;
                    }
 ...
 */
function docDiffUpdate(local, localSv, remoteDiff, remoteSv) {
...
export class DocSyncPeer {
    constructor(peerId, local, syncMetadata, remote, options = {}) {
        this.peerId = peerId;
        this.local = local;
         ...
         */
        this.uniqueId = `sync:${this.peerId}:${nanoid()}`;
        ...
        this.jobs = createJobErrorCatcher({
            connect: async (docId, signal) => {
                const pushedClock = (await this.syncMetadata.getPeerPushedClock(this.peerId, docId))
                    ?.timestamp ?? null;
                ...
                if (!this.remote.isReadonly &&
                    ...
                    // no need to push
                    const pulled = (await this.syncMetadata.getPeerPulledRemoteClock(this.peerId, docId))
                        ?.timestamp ?? null;
            ...
            push: async (docId, jobs, signal) => {
                if (this.status.connectedDocs.has(docId) && !this.remote.isReadonly) {
                    const maxClock = jobs.reduce((a, b) => (a.getTime() > b.clock.getTime() ? a : b.clock), new Date(0));
                    const merged = await this.mergeUpdates(jobs
            ...
            pullAndPush: async (docId, signal) => {
                const localDocRecord = await this.local.getDoc(docId);
                const stateVector = localDocRecord && !isEmptyUpdate(localDocRecord.bin)
                    ? encodeStateVectorFromUpdate(localDocRecord.bin)
                    : new Uint8Array();
                const remoteDocRecord = await this.remote.getDocDiff(docId, stateVector);
                if (remoteDocRecord) {
                    const { missing: newData, state: serverStateVector, timestamp: remoteClock, } = remoteDocRecord;
                    throwIfAborted(signal);
                    ...
                    const diff = localDocRecord && serverStateVector && serverStateVector.length > 0
                        ? docDiffUpdate(localDocRecord.bin, stateVector, newData, serverStateVector)
                        : localDocRecord?.bin;
            ...
            pull: async (docId, signal) => {
                const docRecord = await this.local.getDoc(docId);
                const stateVector = docRecord && !isEmptyUpdate(docRecord.bin)
            ...
            save: async (docId, jobs, signal) => {
                const remoteClock = jobs.reduce((a, b) => (a.getTime() > b.remoteClock.getTime() ? a : b.remoteClock), new Date(0));
                if (this.status.connectedDocs.has(docId)) {
                    const data = jobs
                        .map(j => j.update)
                        .filter((update) => update ? !isEmptyUpdate(update) : false);
                    const update = data.length > 0 ? await this.mergeUpdates(data) : new Uint8Array();
                    throwIfAborted(signal);
        ...
        this.actions = {
            updateRemoteClock: (docId, remoteClock) => {
    ...
    docState$(docId) {
        return new Observable(subscribe => {
            const next = () => {
                ...
                subscribe.next({
                    syncing: !this.status.connectedDocs.has(docId) ||
            ...
            return this.statusUpdatedSubject$.subscribe(updatedId => {
                if (updatedId === true || updatedId === docId)
                    next();
            });
        });
    }
    ...
    async retryLoop(signal) {
        ...
        try {
            ...
            for (const storage of [this.remote, this.local, this.syncMetadata]) {
                ...
                disposes.push(storage.connection.onStatusChanged((_status, error) => {
            ...
            disposes.push(this.local.subscribeDocUpdate((update, origin) => {
                if (origin === this.uniqueId ||
                    origin?.startsWith(`sync:${this.peerId}:`
            ...
            disposes.push(this.remote.subscribeDocUpdate(({ bin, docId, timestamp }, origin) => {
                if (origin === this.uniqueId) {
                    return;
                }
            ...
            const cachedClocks = await this.syncMetadata.getPeerRemoteClocks(this.peerId);
            ...
            while (true) {
                ...
                while (true) {
                    ...
                    if (push && push.length > 0) {
                        await this.jobs.push(docId, push, signal);
                        continue;
                    }
