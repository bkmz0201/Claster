...
import { clickNewPageButton, clickPageMoreActions, createLinkedPage, getBlockSuiteEditorTitle, getPageByTitle, waitForEditorLoad, waitForEmptyEditor, } from '@affine-test/kit/utils/page-logic';
...
test('Show favorite items in sidebar', async ({ page, workspace }) => {
    ...
    const favoriteListItemInSidebar = page.getByTestId('navigation-panel-doc-' + newPageId);
    expect(await favoriteListItemInSidebar.textContent()).toBe('this is a new page to favorite');
...
test('Show favorite reference in sidebar', async ({ page, workspace }) => {
    ...
    expect(await favoriteListItemInSidebar.textContent()).toBe('this is a new page to favorite');
    const collapseButton = favoriteListItemInSidebar.getByTestId('navigation-panel-collapsed-button');
    await expect(collapseButton).toBeVisible();
    await collapseButton.click();
    await expect(favoriteListItemInSidebar.locator('[data-testid^="navigation-panel-doc-"]:has-text("Another page")')).toBeVisible();
    const currentWorkspace = await workspace.current();
    expect(currentWorkspace.meta.flavour).toContain('local');
});
test("Deleted page's reference will not be shown in sidebar", async ({ page, }) => {
    await openHomePage(page);
    await waitForEditorLoad(page);
    await clickNewPageButton(page);
    await getBlockSuiteEditorTitle(page).click();
    ...
    await expect(page.locator('.doc-title-container:has-text("Another page")')).toBeVisible();
    const anotherPageId = getCurrentDocIdFromUrl(page);
    const favItemTestId = 'navigation-panel-doc-' + newPageId;
    await expect(page.getByTestId(favItemTestId)).toHaveText('this is a new page to favorite');
    await page
    ...
    await expect(page.getByTestId(favItemTestId).getByTestId(favItemAnotherPageTestId)).toBeVisible();
    // delete the page
    await clickPageMoreActions(page);
    ...
    await expect(page.getByTestId(favItemTestId).getByTestId(favItemAnotherPageTestId)).toBeHidden();
});
...
//# sourceMappingURL=local-first-favorites-items.spec.js.map