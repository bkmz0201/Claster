...
import { cutByKeyboard, dragBetweenIndices, enterPlaygroundRoom, focusRichText, focusRichTextEnd, getPageSnapshot, initEmptyParagraphState, pasteByKeyboard, pressEnter, pressShiftEnter, pressTab, selectAllByKeyboard, setSelection, SHORT_KEY, switchReadonly, type, waitNextFrame, } from './utils/actions/index.js';
import { assertKeyboardWorkInInput } from './utils/asserts.js';
import { test } from './utils/playwright.js';
const pressCreateLinkShortCut = async (page) => {
...
async function createLinkBlock(page, str, link) {
    const id = await page.evaluate(([str, link]) => {
        const { doc } = window;
        ...
        const id = doc.addBlock('affine:paragraph', { type: 'text', text: text }, noteId);
        return id;
    }, [str, link]);
    return id;
}
test('type character in link should not jump out link node', async ({ page, }, testInfo) => {
    await enterPlaygroundRoom(page);
...
test('type character after link should not extend the link attributes', async ({ page, }, testInfo) => {
    await enterPlaygroundRoom(page);
    await createLinkBlock(page, 'link text', 'http://example.com');
    await focusRichText(page, 0);
...
test('should keyboard work in link popover', async ({ page }) => {
    ...
    const editTextInput = editLinkPopover.locator('.affine-edit-area.text .affine-edit-input');
    await assertKeyboardWorkInInput(page, editTextInput);
    const editLinkInput = editLinkPopover.locator('.affine-edit-area.link .affine-edit-input');
    await assertKeyboardWorkInInput(page, editLinkInput);
});
test('link bar should not be appear when the range is collapsed', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await focusRichText(page);
    await type(page, 'aaa');
