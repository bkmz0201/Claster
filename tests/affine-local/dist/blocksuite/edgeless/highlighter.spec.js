...
import { clickEdgelessModeButton, clickView, dragView, locateEditorContainer, locateToolbar, setEdgelessTool, } from '@affine-test/kit/utils/editor';
...
test('should exit drawing tools menu when Escape is pressed', async ({ page, }) => {
    await setEdgelessTool(page, 'highlighter');
...
test('should enter highlighter tool when `Shift + P` is pressed', async ({ page, }) => {
    ...
    await expect(drawingToolButton).toHaveAttribute('data-drawing-tool', 'highlighter');
    await expect(drawingToolsMenu).toBeVisible();
    await page.keyboard.press('Escape');
    await expect(drawingToolsMenu).toBeHidden();
    await expect(drawingToolButton).toHaveAttribute('data-drawing-tool', 'highlighter');
