...
import { GfxControllerIdentifier, GfxPrimitiveElementModel, isGfxGroupCompatibleModel, } from '@blocksuite/affine/std/gfx';
...
function isOverlap(target, source) {
...
function isInside(target, source) {
    const { x, y, w, h } = source;
    const left = target.x;
    const top = target.y;
    const right = target.x + target.w;
...
function hideEdgelessElements(elements, std) {
    elements.forEach(ele => {
        if (ele instanceof GfxPrimitiveElementModel) {
            ele.lastOpacity = ele.opacity;
            ele.opacity = 0;
        }
        else {
...
function showEdgelessElements(elements, std) {
    elements.forEach(ele => {
        if (ele instanceof GfxPrimitiveElementModel) {
            ele.opacity = ele.lastOpacity;
            delete ele.lastOpacity;
        }
        else {
            const block = std.view.getBlock(ele.id);
            if (!block)
...
function withDescendantElements(elements) {
    const set = new Set();
...
const MARGIN = 20;
export function copyAsImage(std) {
    if (!apis) {
        notify.error({
    ...
    // fit to screen
    if (!isInside(gfx.viewport.viewportBounds, exBound) ||
        gfx.viewport.zoom < 1) {
    ...
    setTimeout(() => {
        ...
        try {
            ...
            const { zoom } = gfx.viewport;
            const isFrameSelected = selected.length === 1 &&
                selected[0].flavour === 'affine:frame';
...
export function createCopyAsPngMenuItem(framework) {
    return {
...
//# sourceMappingURL=copy-as-image.js.map