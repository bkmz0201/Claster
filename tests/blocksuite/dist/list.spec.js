...
import { dragBetweenIndices, enterPlaygroundRoom, enterPlaygroundWithList, focusRichText, getPageSnapshot, initEmptyEdgelessState, initEmptyParagraphState, initThreeLists, pressArrowLeft, pressArrowUp, pressBackspace, pressBackspaceWithShortKey, pressEnter, pressShiftEnter, pressShiftTab, pressSpace, pressTab, redoByClick, switchEditorMode, switchReadonly, type, undoByClick, undoByKeyboard, updateBlockType, waitNextFrame, } from './utils/actions/index.js';
import { assertBlockChildrenFlavours, assertBlockChildrenIds, assertBlockCount, assertBlockType, assertListPrefix, assertRichTextInlineRange, assertRichTexts, assertTextContent, } from './utils/asserts.js';
import { test } from './utils/playwright.js';
import { getFormatBar } from './utils/query.js';
async function isToggleIconVisible(toggleIcon) {
...
test('should indent todo block preserve todo status', async ({ page, }, testInfo) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await focusRichText(page);
    await type(page, 'text1');
...
test('enter list block with empty text', async ({ page }, testInfo) => {
     ...
     *   -
     */
    expect(await getPageSnapshot(page, true)).toMatchSnapshot(`${testInfo.title}_4.json`);
    await undoByClick(page);
    /**
     * -
     *   -
     ...
     *   -
     */
    expect(await getPageSnapshot(page, true)).toMatchSnapshot(`${testInfo.title}_5.json`);
    await undoByClick(page);
    /**
     * -
     *   -
...
test('add number prefix to a todo item should not forcefully change it into numbered list, vice versa', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await focusRichText(page, 0);
...
test('should not convert to a list when pressing space at the second line', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await focusRichText(page);
    await type(page, 'aaa');
...
test.describe('toggle list', () => {
    test('click toggle icon should collapsed list', async ({ page, }, testInfo) => {
        await enterPlaygroundRoom(page);
        await initEmptyParagraphState(page);
        ...
        expect(await getPageSnapshot(page, true)).toMatchSnapshot(`${testInfo.title}_toggle.json`);
...
test.describe('readonly', () => {
    test('can expand toggle in readonly mode', async ({ page }, testInfo) => {
        ...
        await toggleIcon.click();
        await expect(listChildren).not.toBeVisible();
        expect(await getPageSnapshot(page, true)).toMatchSnapshot(`${testInfo.title}_before_readonly.json`);
        await waitNextFrame(page, 200);
