...
import { addNote, changeNoteDisplayModeWithId, createNote, dragBlockToPoint, dragHandleFromBlockToBlockBottomById, enterPlaygroundRoom, focusRichText, initEmptyEdgelessState, initThreeParagraphs, pressEnter, pressEscape, setEdgelessTool, switchEditorMode, type, waitNextFrame, } from '../../utils/actions/index.js';
import { assertRectExist, assertRichTexts } from '../../utils/asserts.js';
import { NoteDisplayMode } from '../../utils/bs-alternative.js';
import { test } from '../../utils/playwright.js';
const CENTER_X = 450;
const CENTER_Y = 450;
test('drag handle should be shown when a note is activated in default mode or hidden in other modes', async ({ page, }) => {
...
test('drag handle can drag note into another note', async ({ page }) => {
    ...
    assertRectExist(secondNoteRect);
    {
        ...
        await page.mouse.move(handlerRect.x + handlerRect.width / 2, handlerRect.y + handlerRect.height / 2);
...
test('should keep relative order of new note when a block is dragged from note to canvas', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyEdgelessState(page);
    await focusRichText(page);
    await type(page, '3');
    ...
    await changeNoteDisplayModeWithId(page, note2, NoteDisplayMode.DocAndEdgeless);
    await pressEscape(page);
    const note3 = await createNote(page, [0, 200], '9');
    await pressEscape(page, 3);
    await changeNoteDisplayModeWithId(page, note3, NoteDisplayMode.DocAndEdgeless);
    await pressEscape(page);
...
test('drag handle should work when hover on the background of a selected edgeless note', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyEdgelessState(page);
    await focusRichText(page);
    await type(page, 'hello');
...
//# sourceMappingURL=drag-handle.spec.js.map