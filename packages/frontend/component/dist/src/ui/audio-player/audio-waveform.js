...
const getCSSVarValue = (element, varName) => {
...
const drawWaveform = ({ canvas, container, waveform, progress, mini, }) => {
    ...
    const playedColor = getCSSVarValue(container, 'block/recordBlock/timelineIndeicator');
...
export const AudioWaveform = ({ waveform, progress, onManualSeek, mini = false, // the bar will be 0.5px instead. by default, the bar is 1px
loading = false, }) => {
    ...
    return (_jsx("div", { ref: containerRef, className: styles.root, onPointerDown: handlePointerDown, onClick: stopPropagation, role: "slider", "aria-valuemin": 0, "aria-valuemax": 100, "aria-valuenow": progress * 100, draggable: "false", children: loading ? (_jsx(Skeleton, {})) : (_jsx("canvas", { ref: canvasRef, style: { width: '100%', height: '100%' } })) }));
};
