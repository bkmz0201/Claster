...
export async function waitForEmptyEditor(page) {
    await expect(page.locator('.doc-title-container-empty')).toBeVisible();
}
export function getBlockSuiteEditorTitle(page) {
    return page.locator('doc-title .inline-editor').nth(0);
}
export async function type(page, content, delay = 50) {
    await page.keyboard.type(content, { delay });
}
export const createLinkedPage = async (page, pageName) => {
...
export const createSyncedPageInEdgeless = async (page, pageName) => {
    await page.keyboard.type('@', { delay: 50 });
...
export const createTodayPage = async (page) => {
...
export const getPageOperationButton = (page, id) => {
    return getPageItem(page, id).getByTestId('doc-list-operation-button');
};
export const getPageItem = (page, id) => {
    return page.locator(`[data-doc-id="${id}"][data-testid="doc-list-item"]`);
};
export const getPageByTitle = (page, title) => {
    return page.getByTestId('doc-list-item').filter({
        has: page.locator(`[data-testid="doc-list-item-title"]:has-text("${title}")`),
    });
};
export const toPosition = (rect, location) => {
    switch (location) {
        case 'center':
            return {
                x: rect.width / 2,
...
export const dragTo = async (page, locator, target, location = 'center', willMoveOnDrag = false) => {
    await locator.hover();
    ...
    await page.mouse.move(locatorElement.x + locatorCenter.x, locatorElement.y + locatorCenter.y);
    await page.mouse.down();
    await page.waitForTimeout(100);
    await page.mouse.move(locatorElement.x + locatorCenter.x + 1, locatorElement.y + locatorCenter.y + 1);
    ...
    if (!willMoveOnDrag) {
        ...
        await page.mouse.move(targetElement.x + targetPosition.x, targetElement.y + targetPosition.y, {
            steps: 10,
        });
    }
    await page.waitForTimeout(100);
...
export const focusInlineEditor = async (page) => {
    await page
        .locator(`.affine-paragraph-rich-text-wrapper:has(.visible):has-text("Type '/' for commands")`)
        .locator('.inline-editor')
        .focus();
};
export const addDatabase = async (page, title) => {
    ...
    if (title) {
        ...
        await page
            .locator('affine-database-title textarea[data-block-is-database-title="true"]')
            .fill(title);
        await page
            .locator('affine-database-title textarea[data-block-is-database-title="true"]')
            .blur();
