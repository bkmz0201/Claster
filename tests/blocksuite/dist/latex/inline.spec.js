...
import { cutByKeyboard, pasteByKeyboard, pressArrowLeft, pressArrowRight, pressArrowUp, pressBackspace, pressBackspaceWithShortKey, pressEnter, pressShiftEnter, redoByKeyboard, selectAllByKeyboard, type, undoByKeyboard, } from '../utils/actions/keyboard.js';
import { enterPlaygroundRoom, focusRichText, initEmptyParagraphState, } from '../utils/actions/misc.js';
import { assertRichTextInlineDeltas, assertRichTextInlineRange, } from '../utils/asserts.js';
...
test('add inline latex at the start of line', async ({ page }, testInfo) => {
    ...
    const latexElement = page.locator('affine-paragraph rich-text affine-latex-node');
    ...
    expect(await latexElement.locator('.placeholder').innerText()).toBe('Equation');
...
test('latex editor', async ({ page }) => {
    ...
    expect(await latexEditorLine.innerText()).toBe('ababababababababababababababababababababababababab');
    // click outside to hide the editor
    expect(await latexEditorLine.isVisible()).toBeTruthy();
...
test('undo-redo when add inline latex using markdown shortcut', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await focusRichText(page);
...
test('auto focus after add inline latex using markdown shortcut', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await focusRichText(page);
