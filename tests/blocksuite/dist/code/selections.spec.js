...
import { pressArrowLeft, pressBackspace, pressEnter, pressEnterWithShortkey, redoByKeyboard, type, undoByKeyboard, } from '../utils/actions/keyboard.js';
import { enterPlaygroundRoom, focusRichText, getInlineSelectionIndex, getInlineSelectionText, initEmptyCodeBlockState, } from '../utils/actions/misc.js';
import { assertBlockCount, assertBlockSelections, assertRichTextInlineRange, assertRichTexts, } from '../utils/asserts.js';
...
test('drag select code block can delete it with backspace', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyCodeBlockState(page);
    await focusRichText(page);
    ...
    await dragBetweenCoords(page, { x: position.startX, y: position.startY }, { x: position.endX, y: position.endY }, { steps: 20 });
    await page.waitForTimeout(10);
    await page.keyboard.press('Backspace');
    const locator = page.locator('affine-code');
    await expect(locator).toBeHidden();
});
test('drag select code block can delete it with delete key', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyCodeBlockState(page);
    await focusRichText(page);
    const codeBlock = page.locator('affine-code');
...
test('press short key and enter at end of code block can jump out', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyCodeBlockState(page);
    await focusRichText(page);
...
test('press short key and enter at end of code block with content can jump out', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyCodeBlockState(page);
    await focusRichText(page);
...
test('press backspace after code block can select code block', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyCodeBlockState(page);
    await focusRichText(page);
...
test('press ArrowUp after code block can enter code block', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyCodeBlockState(page);
    await focusRichText(page);
    const code = 'const a = 1;';
...
//# sourceMappingURL=selections.spec.js.map