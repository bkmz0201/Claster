...
import { draggableGet, } from './types';
export const useDraggable = (getOptions = () => ({}), deps = []) => {
    const [dragging, setDragging] = useState(false);
    const [draggingPosition, setDraggingPosition] = useState({ offsetX: 0, offsetY: 0, clientX: 0, clientY: 0, outWindow: false });
    const [dropTarget, setDropTarget] = useState([]);
    const [customDragPreviewPortal, setCustomDragPreviewPortal] = useState(() => () => null);
    ...
    useEffect(() => {
        ...
        const windowEvent = {
            dragleave: () => {
                setDraggingPosition(state => state.outWindow === true ? state : { ...state, outWindow: true });
            },
            dragover: () => {
                setDraggingPosition(state => state.outWindow === true ? { ...state, outWindow: false } : state);
            },
        };
        ...
        const cleanupDraggable = draggable({
            ...
            onDrag: args => {
                if (enableDraggingPosition.current) {
                    setDraggingPosition(prev => ({
                        offsetX: args.location.current.input.clientX -
                            args.location.initial.input.clientX,
                        offsetY: args.location.current.input.clientY -
            ...
            onGenerateDragPreview({ nativeSetDragImage, source, location }) {
                ...
                let previewPosition = options.dragPreviewPosition ?? 'native';
                ...
                if (enableCustomDragPreview.current) {
                    setCustomNativeDragPreview({
                        ...
                        render({ container }) {
                            flushSync(() => {
                                setCustomDragPreviewPortal(() => ({ children, position, }) => {
