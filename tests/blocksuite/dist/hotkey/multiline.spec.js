...
import { assertBlockSelections, assertRichTextInlineRange, assertRichTexts, } from '../utils/asserts.js';
...
test('multi line rich-text inline code hotkey', async ({ page }, testInfo) => {
    ...
    expect(await getPageSnapshot(page, true)).toMatchSnapshot(`${testInfo.title}_redo.json`);
});
...
test('arrow up and down behavior on multiline text blocks when previous is non-text', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
...
test('should forwardDelete works when delete multi characters', async ({ page, }) => {
...
//# sourceMappingURL=multiline.spec.js.map