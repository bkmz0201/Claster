...
export const SidebarHeaderSwitcher = () => {
    ...
    const tabItems = tabs.map(tab => ({
        value: tab.id,
        label: (_jsx(ViewSidebarTabIconTarget, { className: styles.iconContainer, viewId: view.id, tabId: tab.id })),
        testId: `sidebar-tab-${tab.id}`,
        style: { padding: 0, fontSize: 20, width: 24 },
    }));
    const handleActiveTabChange = useCallback((tabId) => {
        view.activeSidebarTab(tabId);
    }, [view]);
    return tabItems.length ? (_jsx(RadioGroup, { iconMode: true, borderRadius: 8, itemHeight: 24, padding: 4, gap: 8, items: tabItems, value: activeTab?.id, onChange: handleActiveTabChange })) : null;
};
//# sourceMappingURL=sidebar-header-switcher.js.map