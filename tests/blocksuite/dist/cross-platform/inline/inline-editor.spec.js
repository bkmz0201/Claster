...
import { pressArrowLeft, pressBackspace, pressEnter, type, } from '../../utils/actions/keyboard.js';
...
async function enterInlineEditorPlayground(page) {
    const url = new URL('examples/inline/index.html', defaultPlaygroundURL);
    await page.goto(url.toString());
}
async function focusInlineRichText(page, index = 0) {
...
async function getDeltaFromInlineRichText(page, index = 0) {
...
async function setInlineRichTextRange(page, inlineRange, index = 0) {
...
async function getInlineRichTextLine(page, index, i = 0) {
...
async function getInlineRangeIndexRect(page, [richTextIndex, inlineIndex], coordOffSet = { x: 0, y: 0 }) {
...
async function assertSelection(page, richTextIndex, rangeIndex, rangeLength = 0) {
...
test('chinese input', async ({ page, browserName }) => {
    test.skip(browserName !== 'chromium', 'CDPSession is only supported in chromium');
    await enterInlineEditorPlayground(page);
    await focusInlineRichText(page);
    const editorA = page.locator('[data-v-root="true"]').nth(0);
    const editorB = page.locator('[data-v-root="true"]').nth(1);
...
test('caret should move correctly when inline elements are exist', async ({ page, }) => {
