...
async function closeImagePreviewModal(page) {
...
test('image able to zoom in and out with mouse scroll', async ({ page }) => {
    ...
    const naturalWidth = await locator.evaluate((img) => img.naturalWidth);
    await expect(locator).toBeVisible();
...
test('image able to copy to clipboard', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid=affine-toast]:has-text("Copied to clipboard.")')).toBeVisible();
});
test('image preview should be able to copy image to clipboard on copy event', async ({ page, }) => {
    await openHomePage(page);
    await waitForEditorLoad(page);
    await clickNewPageButton(page);
    let blobId;
...
test('image should only able to move when image is larger than viewport', async ({ page, }) => {
    await openHomePage(page);
    await waitForEditorLoad(page);
    await clickNewPageButton(page);
    let blobId;
...
test('image should able to delete and when delete, it will move to previous/next image', async ({ page, }) => {
    await openHomePage(page);
    await waitForEditorLoad(page);
    await clickNewPageButton(page);
    let blobId;
...
test('tooltips for all buttons should be visible when hovering', async ({ page, }) => {
    await openHomePage(page);
    await waitForEditorLoad(page);
    await clickNewPageButton(page);
    let blobId;
...
test('when mouse moves outside, the modal should be closed', async ({ page, }) => {
    await openHomePage(page);
    await waitForEditorLoad(page);
    await clickNewPageButton(page);
    const title = getBlockSuiteEditorTitle(page);
...
test('caption should be visible and different styles were applied if image zoomed larger than viewport', async ({ page, }) => {
...
//# sourceMappingURL=image-preview.spec.js.map