import { clickEdgelessModeButton, locateEditorContainer, locateToolbar, } from '@affine-test/kit/utils/editor';
import { openHomePage } from '@affine-test/kit/utils/load-page';
import { clickNewPageButton, type, waitForEmptyEditor, } from '@affine-test/kit/utils/page-logic';
import { expect, test } from '@playwright/test';
const TEST_SPOTIFY_URL = 'https://open.spotify.com/episode/7makk4oTQel546B0PZlDM5';
...
test.describe('embed iframe block', () => {
    async function addEmbedIframeBlock(page) {
    ...
    async function openToolbarAndSwitchView(page) {
    ...
    test('add embed iframe block using slash menu', async ({ page }) => {
        const { embedIframeBlock, embedIframeIdleCard, embedIframeLinkInputPopup, input, } = await addEmbedIframeBlock(page);
    ...
    test.describe('conversions', () => {
        async function setupEmbedIframe(page) {
    ...
    test.describe('edgeless mode', () => {
        ...
        async function addEmbedEdgelessIframeBlock(page) {
            ...
            await page
                .locator(`[data-testid="cmdk-label"][data-value="external-link:${TEST_SPOTIFY_URL}"]`)
                .click();
            // expect the embed iframe block count is 1
            const embedEdgelessIframeBlock = page.locator(EMBED_EDGELESS_IFRAME_BLOCK);
