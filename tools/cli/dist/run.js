...
export class RunCommand extends PackageCommand {
    constructor() {
        ...
        this.packageNameOrAlias = Option.String({
            required: true,
            validator: this.packageNameValidator,
        });
        this.args = Option.Proxy({ name: 'args', required: 1 });
    }
    static { this.paths = [[], ['run'], ['r']]; }
    static { this.usage = PackageCommand.Usage({
    ...
    async runScript(pkg, scriptName, args, opts = {}) {
        ...
        if (opts.includeDependencies) {
            const depsRun = Promise.all(pkg.deps.map(dep => {
                return this.runScript(pkg.workspace.getPackage(dep.name), scriptName, [], {
                    ...opts,
    ...
    async runCommand(pkg, args) {
        ...
        const isLoaderRequired = !ignoreLoaderScripts.some(ignore => new RegExp(ignore).test(bin)) ||
