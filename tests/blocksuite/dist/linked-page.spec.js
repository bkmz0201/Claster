...
import { addNewPage, getDebugMenu, switchToPage, } from './utils/actions/click.js';
...
import { copyByKeyboard, cutByKeyboard, pasteByKeyboard, pressArrowLeft, pressBackspace, pressEnter, pressEscape, selectAllByKeyboard, SHORT_KEY, type, } from './utils/actions/keyboard.js';
import { getLinkedDocPopover } from './utils/actions/linked-doc.js';
import { captureHistory, enterPlaygroundRoom, focusRichText, focusTitle, getPageSnapshot, initEmptyEdgelessState, initEmptyParagraphState, setInlineRangeInSelectedRichText, waitNextFrame, } from './utils/actions/misc.js';
import { assertParentBlockFlavour, assertRichTexts, assertTitle, } from './utils/asserts.js';
import { test } from './utils/playwright.js';
async function createAndConvertToEmbedLinkedDoc(page) {
    ...
    await page.mouse.move(lickedDocBox.x + lickedDocBox.width / 2, lickedDocBox.y + lickedDocBox.height / 2);
...
test.describe('reference node', () => {
    test('linked doc popover can show and hide correctly', async ({ page, }, testInfo) => {
        await enterPlaygroundRoom(page);
        await initEmptyParagraphState(page);
        await focusRichText(page);
        await type(page, '[[');
    ...
    test('linked doc popover should not show when the current content is @xx and pressing backspace', async ({ page, }) => {
        await enterPlaygroundRoom(page);
        await initEmptyParagraphState(page);
        await focusRichText(page);
        await type(page, '@');
    ...
    test('should reference node attributes correctly', async ({ page, }, testInfo) => {
        await enterPlaygroundRoom(page);
        await initEmptyParagraphState(page);
    ...
    test('text inserted in the between of reference nodes should not be extend attributes', async ({ page, }, testInfo) => {
        await enterPlaygroundRoom(page);
        await initEmptyParagraphState(page);
        await addNewPage(page);
        await focusRichText(page);
    ...
    test('text can be inserted as expected when reference node is in the start or end of line', async ({ page, }, testInfo) => {
        await enterPlaygroundRoom(page);
        await initEmptyParagraphState(page);
        await addNewPage(page);
        await focusRichText(page);
    ...
    test('should the cursor move correctly around reference node', async ({ page, }, testInfo) => {
        await enterPlaygroundRoom(page);
        await initEmptyParagraphState(page);
        await addNewPage(page);
        await focusRichText(page);
    ...
    test('should create reference node works', async ({ page }) => {
        ...
        await focusRichText(page);
        await type(page, '@');
        const { linkedDocPopover, refNode, assertExistRefText: assertReferenceText, } = getLinkedDocPopover(page);
        await expect(linkedDocPopover).toBeVisible();
        await pressEnter(page);
        await expect(linkedDocPopover).toBeHidden();
        await assertRichTexts(page, [' ']);
    ...
    test('should not merge consecutive identical reference nodes for rendering', async ({ page, }) => {
...
test.describe('linked page popover', () => {
    ...
    test('should fuzzy search works', async ({ page }) => {
        await enterPlaygroundRoom(page);
        await initEmptyParagraphState(page);
        const { linkedDocPopover, pageBtn, assertExistRefText, assertActivePageIdx, } = getLinkedDocPopover(page);
        await focusTitle(page);
        await type(page, 'page0');
...
test.describe('linked page with clipboard', () => {
    test('paste linked page should paste as linked page', async ({ page, }, testInfo) => {
        await enterPlaygroundRoom(page);
        await initEmptyParagraphState(page);
        await focusRichText(page);
        const { createLinkedDoc } = getLinkedDocPopover(page);
    ...
    test('duplicated linked page should paste as linked page', async ({ page, }, testInfo) => {
        await enterPlaygroundRoom(page);
        await initEmptyParagraphState(page);
        await focusRichText(page);
        const { createLinkedDoc } = getLinkedDocPopover(page);
...
test('should [[Selected text]] converted to linked page', async ({ page, }, testInfo) => {
    test.info().annotations.push({
        type: 'issue',
...
test('add reference node before the other reference node', async ({ page }) => {
    ...
    await type(page, '@bbb');
    await pressEnter(page);
    expect(await firstRefNode.textContent()).toEqual(expect.stringContaining('bbb'));
    expect(await firstRefNode.textContent()).not.toEqual(expect.stringContaining('ccc'));
    await pressArrowLeft(page, 3);
    await type(page, '@ccc');
    await pressEnter(page);
    expect(await firstRefNode.textContent()).not.toEqual(expect.stringContaining('bbb'));
    expect(await firstRefNode.textContent()).toEqual(expect.stringContaining('ccc'));
});
test('linked doc can be dragged from note to surface top level block', async ({ page, }) => {
    await enterPlaygroundRoom(page);
...
test.describe.skip('Customize linked doc title and description', () => {
    ...
    test('should set a custom title and description for card link', async ({ page, }) => {
        ...
        const linkedDocBlockTitle = linkedDocBlock.locator('.affine-embed-linked-doc-content-title-text');
        ...
        await waitNextFrame(page, 200);
        await expect(linkedDocBlockTitle).toHaveText('page0-title0');
        await expect(page.locator('.affine-embed-linked-doc-content-note.alias')).toHaveText('This is a new description');
    });
    // Embed View
    test('should automatically switch to card view and set a custom title and description', async ({ page, }) => {
        await enterPlaygroundRoom(page);
        await initEmptyParagraphState(page);
        await focusTitle(page);
        await type(page, 'title0');
        ...
        await waitNextFrame(page, 200);
        const syncedDocBlockTitle = syncedDocBlock.locator('.affine-embed-synced-doc-title');
        ...
        const linkedDocBlockTitle = linkedDocBlock.locator('.affine-embed-linked-doc-content-title-text');
        await expect(linkedDocBlockTitle).toHaveText('page0-title0');
        await expect(linkedDocBlock.locator('.affine-embed-linked-doc-content-note.alias')).toHaveText('This is a new description');
    });
    // Embed View
    test.fixme('should automatically switch to card view and set a custom title and description on edgeless', async ({ page, }) => {
        await enterPlaygroundRoom(page);
        await initEmptyEdgelessState(page);
        await focusRichText(page);
        await createAndConvertToEmbedLinkedDoc(page);
        ...
        await saveButton.click();
        await waitNextFrame(page);
        const syncedDocBlock = page.locator('affine-embed-edgeless-synced-doc-block');
        await expect(syncedDocBlock).toBeHidden();
        const linkedDocBlock = page.locator('affine-embed-edgeless-linked-doc-block');
        await expect(linkedDocBlock).toBeVisible();
        const linkedDocBlockTitle = linkedDocBlock.locator('.affine-embed-linked-doc-content-title-text');
        await expect(linkedDocBlockTitle).toHaveText('page0-title0');
        await expect(linkedDocBlock.locator('.affine-embed-linked-doc-content-note.alias')).toHaveText('This is a new description');
...
//# sourceMappingURL=linked-page.spec.js.map