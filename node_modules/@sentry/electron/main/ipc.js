...
function handleEnvelope(client, options, env, contents) {
    ...
    if (eventAndAttachments) {
        ...
        const profileChunk = envelope.profileChunkFromEnvelope(envelope$1);
        if (profileChunk) {
            const normalizedEnvelope = normalize.normalizeProfileChunkEnvelope(options, envelope$1, electron.app.getAppPath());
...
function handleAttributes(client, options, contents, maybeAttributes) {
    ...
    attributes['sentry.sdk.version'] = { value: version.SDK_VERSION, type: 'string' };
...
function handleMetricFromRenderer(client, options, metric, contents) {
    ...
    core._INTERNAL_captureSerializedMetric(client, metric);
}
...
function configureClassic(client, ipcUtil, options) {
    ...
    electron.ipcMain.on(ipcUtil.createKey('metric'), ({ sender }, metric) => handleMetricFromRenderer(client, options, metric, sender));
    const rendererStatusChanged = rendererAnr.createRendererEventLoopBlockStatusHandler(client);
