...
import { autoFit, captureHistory, cutByKeyboard, dblclickView, dragBetweenIndices, edgelessCommonSetup, enterPlaygroundRoom, getEdgelessSelectedRect, getIds, getInlineSelectionIndex, getPageSnapshot, initEmptyEdgelessState, pasteByKeyboard, pressArrowDown, pressArrowLeft, pressArrowRight, pressArrowUp, pressBackspace, pressEnter, pressEscape, pressShiftTab, pressTab, redoByKeyboard, selectAllByKeyboard, setEdgelessTool, switchEditorMode, toViewCoord, type, undoByKeyboard, waitNextFrame, } from '../utils/actions/index.js';
import { assertBlockChildrenIds, assertBlockFlavour, assertBlockTextContent, assertRichTextInlineDeltas, assertRichTextInlineRange, } from '../utils/asserts.js';
import { test } from '../utils/playwright.js';
import { getFormatBar } from '../utils/query.js';
async function assertEdgelessTextModelRect(page, id, bound) {
    const realXYWH = await page.evaluate(id => {
        const block = window.host.view.getBlock(id);
...
test.describe('edgeless text block', () => {
    ...
    test('edgeless text width fixed when drag moving', async ({ page }) => {
        ...
        const rect = await page.evaluate(() => {
            const container = document.querySelector('.edgeless-text-block-container');
            return container.getBoundingClientRect();
        });
        const modelXYWH = await page.evaluate(() => {
            const block = window.host.view.getBlock('4');
            return block.model.xywh;
        });
    ...
    test('When creating edgeless text, if the input is empty, it will be automatically deleted', async ({ page, }) => {
        await setEdgelessTool(page, 'default');
        await page.mouse.dblclick(130, 140, {
            delay: 100,
        });
    ...
    test('edgeless text should maintain selection when deleting across multiple lines', async ({ page, }) => {
    ...
    test('edgeless text should not blur after pressing backspace', async ({ page, }) => {
    ...
    test.fixme('edgeless text max width', async ({ page }) => {
        ...
        // from left to right
        await page.mouse.move(selectedRect.x + selectedRect.width, selectedRect.y + selectedRect.height / 2);
        await page.mouse.down();
        await page.mouse.move(selectedRect.x + selectedRect.width + 30, selectedRect.y + selectedRect.height / 2, {
            steps: 10,
        });
        await page.mouse.up();
        ...
        await page.mouse.move(selectedRect.x + selectedRect.width, selectedRect.y + selectedRect.height / 2);
        await page.mouse.down();
        await page.mouse.move(selectedRect.x + selectedRect.width - 45, selectedRect.y + selectedRect.height / 2, {
            steps: 10,
        });
        await page.mouse.up();
        // height changed
    ...
    test('min width limit for embed block', async ({ page }, testInfo) => {
        ...
        await pressEnter(page);
        await waitNextFrame(page, 200);
        expect(await getPageSnapshot(page, true)).toMatchSnapshot(`${testInfo.title}_add_linked_doc.json`);
        ...
        await autoFit(page);
        await waitNextFrame(page);
        expect(await getPageSnapshot(page, true)).toMatchSnapshot(`${testInfo.title}_link_to_card.json`);
        // blur
        await page.mouse.click(0, 0);
        // select text element
        await page.mouse.click(point[0] + 10, point[1] + 10);
        ...
        // from right to left
        await page.mouse.move(selectedRect0.x + selectedRect0.width, selectedRect0.y + selectedRect0.height / 2);
        await page.mouse.down();
        await page.mouse.move(selectedRect0.x, selectedRect0.y + selectedRect0.height / 2, {
            steps: 10,
        });
        await page.mouse.up();
        expect(await getPageSnapshot(page, true)).toMatchSnapshot(`${testInfo.title}_link_to_card_min_width.json`);
        const selectedRect1 = await getEdgelessSelectedRect(page);
        // from left to right
        await page.mouse.move(selectedRect1.x + selectedRect1.width, selectedRect1.y + selectedRect1.height / 2);
        await page.mouse.down();
        await page.mouse.move(selectedRect0.x + selectedRect0.width + 45, selectedRect1.y + selectedRect1.height / 2, {
            steps: 10,
        });
        await page.mouse.up();
...
test('press backspace at the start of first line when edgeless text exist', async ({ page, }, testInfo) => {
    await enterPlaygroundRoom(page);
    await page.evaluate(() => {
        const { doc } = window;
    ...
    await waitNextFrame(page);
    await switchEditorMode(page);
    expect(await getPageSnapshot(page, true)).toMatchSnapshot(`${testInfo.title}_note_empty.json`);
    await page.locator('.affine-page-root-block-container').click();
    expect(await getPageSnapshot(page, true)).toMatchSnapshot(`${testInfo.title}_note_not_empty.json`);
    await type(page, 'bbb');
    await pressArrowLeft(page, 3);
    await pressBackspace(page);
    await waitNextFrame(page);
