...
import { clickNewPageButton, createLinkedPage, dragTo, type, waitForEditorLoad, } from '@affine-test/kit/utils/page-logic';
import { clickSideBarAllPageButton } from '@affine-test/kit/utils/sidebar';
import { getCurrentCollectionIdFromUrl, getCurrentDocIdFromUrl, getDocIdFromUrl, } from '@affine-test/kit/utils/url';
import { expect } from '@playwright/test';
const dragToFavourites = async (page, dragItem, id, type = 'doc') => {
    const favourites = page.getByTestId('navigation-panel-favorite-category-divider');
...
const createCollection = async (page, name) => {
    ...
    const collection = page.getByTestId(`navigation-panel-collection-${newCollectionId}`);
...
const dragToCollection = async (page, dragItem) => {
    const collection = await createCollection(page, 'test collection');
    await clickSideBarAllPageButton(page);
    await dragTo(page, dragItem, collection);
    await page.waitForTimeout(500);
    const collectionPage = collection.locator('[data-testid^="navigation-panel-doc-"]');
    await expect(collectionPage).toBeVisible();
    return collectionPage;
};
const dragToTrash = async (page, title, dragItem) => {
    // drag to trash
    ...
    await expect(page.getByText(title), 'The deleted post is no longer on the All Page list').toHaveCount(0);
    await page.waitForTimeout(500);
    await page.getByTestId('trash-page').click();
    await expect(page.getByText(title), 'The deleted post exists in the Trash list').toHaveCount(1);
};
test.beforeEach(async ({ page }) => {
    await openHomePage(page);
    await waitForEditorLoad(page);
});
test('drag a page from "All pages" list to favourites, then drag to trash', async ({ page, }) => {
    ...
    await clickSideBarAllPageButton(page);
    await page.waitForTimeout(500);
    const favouritePage = await dragToFavourites(page, page.locator(`[data-testid="doc-list-item"]:has-text("${title}")`), pageId);
    await dragToTrash(page, title, favouritePage);
});
test('drag a page from "All pages" list to collections, then drag to trash', async ({ page, }) => {
    const title = 'this is a new page to drag';
    await waitForEditorLoad(page);
    await createPage(page, title);
    await clickSideBarAllPageButton(page);
    await page.waitForTimeout(500);
    const collectionPage = await dragToCollection(page, page.locator(`[data-testid="doc-list-item"]:has-text("${title}")`));
...
test('drag a page from "All pages" list to trash', async ({ page }) => {
    ...
    await clickSideBarAllPageButton(page);
    await page.waitForTimeout(500);
    await dragToTrash(page, title, page.locator(`[data-testid="doc-list-item"]:has-text("${title}")`));
});
...
test('items in favourites can be reordered by dragging', async ({ page }) => {
    ...
    await expect(page.getByTestId('navigation-panel-favorites').locator('[draggable]')).toHaveCount(3);
    await expect(page
        .getByTestId('navigation-panel-favorites')
        .locator('[draggable]')
        .first()).toHaveText('test collection');
    await expect(page.getByTestId('navigation-panel-favorites').locator('[draggable]').last()).toHaveText(title0);
    ...
    await expect(page
        .getByTestId('navigation-panel-favorites')
        .locator('[draggable]')
        .first()).toHaveText(title1);
    await expect(page.getByTestId('navigation-panel-favorites').locator('[draggable]').last()).toHaveText('test collection');
});
...
test.skip('drag a page link in editor to favourites', async ({ page }) => {
    ...
    const pageReference = page.locator('a').filter({
        has: page.locator('.affine-reference-title:has-text("hi from another page")'),
    });
    const pageLink = await pageReference.evaluate(el => el.href);
    expect(pageLink).toBeTruthy();
    if (!pageLink) {
        return;
    }
    const pageId = getDocIdFromUrl(pageLink);
    await dragToFavourites(page, page.locator('.affine-reference-title:has-text("hi from another page")'), pageId);
});
test('drag a page card block to another page', async ({ page }) => {
    ...
    await dragToFavourites(page, page.locator('.affine-drag-handle-container'), pageId);
});
test('drag a favourite page into note on page mode', async ({ page }) => {
    ...
    await dragTo(page, item, page.locator('.affine-paragraph-block-container').first());
    await expect(page.locator('affine-embed-linked-doc-block')).toContainText('hi from page');
});
...
test('drag a favourite page into note on edgeless mode', async ({ page }) => {
    ...
    await pressBackspace(page);
    await dragTo(page, item, edgelessNote);
    await expect(embedDocInNote, 'prefer view should only be available in canvas').toBeHidden();
    await expect(cardDocInNote).toBeVisible();
});
//# sourceMappingURL=drag-page.spec.js.map