...
import { createLocalWorkspace, openWorkspaceListModal, } from '@affine-test/kit/utils/workspace';
import { expect } from '@playwright/test';
test('just one item in the workspace list at first', async ({ page, workspace, }) => {
...
test('create one workspace in the workspace list', async ({ page, workspace, }) => {
    await openHomePage(page);
    await waitForEditorLoad(page);
...
test.skip('create multi workspace in the workspace list', async ({ page, workspace, }) => {
    await openHomePage(page);
    await waitForEditorLoad(page);
    ...
    await page.waitForFunction(() => {
        ...
        return (cards[1].textContent?.includes('New Workspace 3') &&
...
//# sourceMappingURL=local-first-workspace-list.spec.js.map