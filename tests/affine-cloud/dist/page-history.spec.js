...
import { createRandomUser, deleteUser, enableCloudWorkspace, loginUser, runPrisma, } from '@affine-test/kit/utils/cloud';
...
const pushCurrentPageUpdates = async (page) => {
    const [workspaceId, guid, updates, state] = await page.evaluate(() => {
        ...
        const updates = Y.encodeStateAsUpdate(spaceDoc);
        const state = Y.encodeStateVector(spaceDoc);
        return [workspaceId, spaceDoc.guid, [...updates], [...state]];
    });
    const toBuffer = (arr) => Buffer.from(arr);
...
//# sourceMappingURL=page-history.spec.js.map