...
describe('GraphQL fetcher', () => {
    beforeEach(() => {
        fetch = vi.fn(() => Promise.resolve(new Response(JSON.stringify({ data: { field: 1 } }), {
            headers: {
    ...
    it('should send POST request to given endpoint', async () => {
        ...
        const ctx = fetch.mock.lastCall?.[1];
    ...
    it('should send with correct graphql JSON body', async () => {
        ...
        expect(fetch.mock.lastCall?.[1]).toEqual(expect.objectContaining({
    ...
    it('should correctly ignore nil variables', async () => {
        ...
        expect(fetch.mock.lastCall?.[1].body).toMatchInlineSnapshot(`"{"query":"query { field }","variables":{"a":false,"b":null},"operationName":"query"}"`);
        await gql({
            query,
            // @ts-expect-error forgive the fake variables
            variables: { a: false, b: null, c: undefined },
        ...
        expect(fetch.mock.lastCall?.[1].body).toMatchInlineSnapshot(`"{"query":"query { field }","variables":{"a":false},"operationName":"query"}"`);
    });
    it('should correct handle graphql error', async () => {
        ...
        await expect(gql({ query, variables: void 0 })).rejects.toMatchInlineSnapshot(`[GraphQLError: error]`);
    });
});
