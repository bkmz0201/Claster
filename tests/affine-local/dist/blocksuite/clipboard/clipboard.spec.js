...
import { clickEdgelessModeButton, clickPageModeButton, clickView, getCodeBlockIds, getParagraphIds, locateEditorContainer, toViewCoord, } from '@affine-test/kit/utils/editor';
import { copyByKeyboard, cutByKeyboard, pasteByKeyboard, pressEnter, } from '@affine-test/kit/utils/keyboard';
import { openHomePage } from '@affine-test/kit/utils/load-page';
import { addCodeBlock, clickNewPageButton, type, waitForEditorLoad, } from '@affine-test/kit/utils/page-logic';
...
async function createParagraphBlocks(page, texts) {
...
async function verifyBlockContent(page, selector, index, expectedText) {
    expect(await page
        .locator(selector)
        .nth(index)
    ...
    }, expectedText)).toBeTruthy();
}
// Helper functions using the generic verifyBlockContent
async function verifyParagraphContent(page, index, expectedText) {
    await verifyBlockContent(page, paragraphLocator, index, expectedText);
}
async function verifyCodeBlockContent(page, index, expectedText) {
    await verifyBlockContent(page, codeBlockLocator, index, expectedText);
}
...
test.describe('surface-ref block', () => {
    async function setupSurfaceRefBlock(page) {
    ...
    test('paste surface-ref block to another doc as embed-linked-doc block', async ({ page, }) => {
        ...
        const embedLinkedDocBlockTitle = embedLinkedDocBlock.locator('.affine-embed-linked-doc-content-title-text');
        await expect(embedLinkedDocBlockTitle).toHaveText('Clipboard Test');
    });
    test('cut and paste surface-ref block to same doc should remain surface-ref block', async ({ page, }) => {
...
test.describe('paste to code block', () => {
    test('should replace the selected text when pasting plain text', async ({ page, }) => {
    ...
    test('should replace the selected text when pasting single snapshot', async ({ page, }) => {
        // Create initial test blocks
    ...
    test('should replace the selected text when pasting multiple snapshots', async ({ page, }) => {
        // Create initial test blocks
    ...
    test('should preserve indentation when pasting code with spaces into code block', async ({ page, }) => {
        await pressEnter(page);
        await addCodeBlock(page);
        ...
        const htmlCode = '<div><span>const</span><span> </span><span>fibonacci</span><span> </span><span>=</span><span> (n</span><span>:</span><span> </span><span>number</span><span>)</span><span>:</span><span> </span><span>number</span><span> </span><span>=></span><span> {</span></div><div><span>  </span><span>if</span><span> (n </span><span><=</span><span> </span><span>1</span><span>) </span><span>return</span><span> n;</span></div><div><span>  </span><span>return</span><span> </span><span>fibonacci</span><span>(n </span><span>-</span><span> </span><span>1</span><span>) </span><span>+</span><span> </span><span>fibonacci</span><span>(n </span><span>-</span><span> </span><span>2</span><span>);</span></div><div><span>}</span></div><div><span>function</span><span> </span><span>generateSequence</span><span>(length</span><span>:</span><span> </span><span>number</span><span>) {</span></div><div><span>  </span><span>const</span><span> </span><span>sequence</span><span> </span><span>=</span><span> [];</span></div><div><span>  </span><span>for</span><span> (</span><span>let</span><span> i </span><span>=</span><span> </span><span>0</span><span>; i </span><span><</span><span> length; i</span><span>++</span><span>) {</span></div><div><span>    sequence.</span><span>push</span><span>(</span><span>fibonacci</span><span>(i));</span></div><div><span>  }</span></div><div><span>  </span><span>return</span><span> sequence;</span></div><div><span>}</span></div></div>';
    ...
    test('html tag should be treated as plain text when pasting', async ({ page, }) => {
        await pressEnter(page);
        await addCodeBlock(page);
        const textWithHtmlTags = '<div><span>const</span><span> </span><span>fibonacci</span><span> </span><span>=</span><span> (n</span><span>:</span><span> </span><span>number</span><span>)</span><span>:</span><span> </span><span>number</span><span> </span><span>=></span><span> {</span></div><div><span>  </span><span>if</span><span> (n </span><span><=</span><span> </span><span>1</span><span>) </span><span>return</span><span> n;</span></div><div><span>  </span><span>return</span><span> </span><span>fibonacci</span><span>(n </span><span>-</span><span> </span><span>1</span><span>) </span><span>+</span><span> </span><span>fibonacci</span><span>(n </span><span>-</span><span> </span><span>2</span><span>);</span></div><div><span>}</span></div><div><span>function</span><span> </span><span>generateSequence</span><span>(length</span><span>:</span><span> </span><span>number</span><span>) {</span></div><div><span>  </span><span>const</span><span> </span><span>sequence</span><span> </span><span>=</span><span> [];</span></div><div><span>  </span><span>for</span><span> (</span><span>let</span><span> i </span><span>=</span><span> </span><span>0</span><span>; i </span><span><</span><span> length; i</span><span>++</span><span>) {</span></div><div><span>    sequence.</span><span>push</span><span>(</span><span>fibonacci</span><span>(i));</span></div><div><span>  }</span></div><div><span>  </span><span>return</span><span> sequence;</span></div><div><span>}</span></div></div>';
        await pasteContent(page, { 'text/plain': textWithHtmlTags });
        await page.waitForTimeout(100);
        // Verify the pasted code maintains indentation
        await verifyCodeBlockContent(page, 0, textWithHtmlTags);
    });
    test('should not wrap line in brackets when pasting code', async ({ page, }) => {
        await pressEnter(page);
        await addCodeBlock(page);
        const plainTextCode = [
...
test.describe('paste in readonly mode', () => {
    test('should not paste content when document is in readonly mode', async ({ page, }) => {
...
test.describe('cross document clipboard regression', () => {
    ...
    test('copied content remains available to external clipboard consumers', async ({ page, }) => {
        const container = locateEditorContainer(page);
        await container.click();
        const textForExternal = 'External clipboard visibility check';
        await type(page, textForExternal);
        const { blockIds } = await getParagraphIds(page);
        await setSelection(page, blockIds[0], 0, blockIds[0], textForExternal.length);
        await copyByKeyboard(page);
        const plainText = await page.evaluate(() => navigator.clipboard.readText());
        expect(plainText).toBe(textForExternal);
    });
    test('copy and paste within a single document still duplicates content', async ({ page, }) => {
        const container = locateEditorContainer(page);
        await container.click();
...
test('should copy single image from edgeless and paste to page', async ({ page, }) => {
    await clickEdgelessModeButton(page);
...
//# sourceMappingURL=clipboard.spec.js.map