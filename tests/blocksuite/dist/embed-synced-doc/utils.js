 ...
 */
export async function initEmbedSyncedDocState(page, data, option) {
    if (data.length === 0) {
        throw new Error('Data is empty');
    }
    return await page.evaluate(async ({ data, option }) => {
        const createDoc = async (docId, title, content) => {
            const { collection, $blocksuite } = window;
            const Text = $blocksuite.store.Text;
            const store = (collection.getDoc(docId) ?? collection.createDoc(docId)).getStore();
            store.load();
        ...
        const getVisibleNote = (store) => {
            const note = store
                .getModelsByFlavour('affine:note')
                .find((note) => {
                return (note instanceof NoteBlockModel &&
                    note.props.displayMode === NoteDisplayMode.DocAndEdgeless);
            });
            return note ?? null;
        };
        const docIds = await Promise.all(data.map(async ({ title, content }, index) => {
            const id = index === 0 ? window.doc.id : `embed-doc-${index}`;
            await createDoc(id, title, content);
            return id;
        }));
        const { NoteBlockModel, NoteDisplayMode } = window.$blocksuite.affineModel;
