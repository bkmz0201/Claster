...
test.describe('AIInsertion/AddToEdgelessAsNote', () => {
    ...
    test('should only show option in edgeless mode', async ({ loggedInPage: page, utils, }) => {
        ...
        await expect(page.getByTestId('action-add-to-edgeless-as-note')).not.toBeVisible();
        await utils.editor.switchToEdgelessMode(page);
        await expect(page.getByTestId('action-add-to-edgeless-as-note')).toBeVisible();
    });
    test('should add to edgeless as note  in edgeless mode', async ({ loggedInPage: page, utils, }) => {
        await utils.editor.switchToEdgelessMode(page);
        // Delete default note
...
//# sourceMappingURL=add-to-edgeless-as-note.spec.js.map