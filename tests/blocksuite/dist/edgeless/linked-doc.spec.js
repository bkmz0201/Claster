...
import { activeNoteInEdgeless, createConnectorElement, createNote, createShapeElement, edgelessCommonSetup, getConnectorPath, selectNoteInEdgeless, Shape, triggerComponentToolbarAction, } from '../utils/actions/edgeless.js';
import { addBasicBrushElement, pressEnter, selectAllByKeyboard, type, waitNextFrame, } from '../utils/actions/index.js';
...
test.describe('note to linked doc', () => {
    test('select a note and turn it into a linked doc', async ({ page }) => {
        ...
        const embedSyncedBlock = page.locator('affine-embed-edgeless-synced-doc-block');
        await expect(embedSyncedBlock).toBeVisible();
        const paragraph = embedSyncedBlock.locator('affine-paragraph [data-v-text="true"]');
    ...
    test('select a group, turn into a linked doc', async ({ page }) => {
        ...
        const groups = await page.evaluate(() => {
            ...
            return container.service.crud.getElementsByType('group').map(s => ({
                ...
                children: s.childElements.map((c) => c.type || c.flavour),
            }));
        });
...
test.describe('multiple edgeless elements to linked doc', () => {
    test('multi-select note, frame, shape, connector, brush and group, turn it into a linked doc', async ({ page, }) => {
        await edgelessCommonSetup(page);
    ...
    test('multi-select with embed doc card inside, turn it into a linked doc', async ({ page, }) => {
        await edgelessCommonSetup(page);
    ...
    test('multi-select with mindmap, turn it into a linked doc', async ({ page, }) => {
        await edgelessCommonSetup(page);
...
//# sourceMappingURL=linked-doc.spec.js.map