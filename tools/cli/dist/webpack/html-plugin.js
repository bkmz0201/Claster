...
function getHTMLPluginOptions(BUILD_CONFIG) {
...
const AssetsManifestPlugin = {
    apply(compiler) {
        compiler.hooks.compilation.tap('assets-manifest-plugin', compilation => {
            HTMLPlugin.getHooks(compilation).beforeAssetTagGeneration.tap('assets-manifest-plugin', arg => {
                if (!compilation.getAsset('assets-manifest.json')) {
                    compilation.emitAsset(`assets-manifest.json`, new webpack.sources.RawSource(JSON.stringify({
                        ...arg.assets,
                        js: arg.assets.js.map(file => file.substring(arg.assets.publicPath.length)),
                        css: arg.assets.css.map(file => file.substring(arg.assets.publicPath.length)),
...
const GlobalErrorHandlerPlugin = {
    apply(compiler) {
        ...
        compiler.hooks.compilation.tap('global-error-handler-plugin', compilation => {
            HTMLPlugin.getHooks(compilation).beforeAssetTagGeneration.tap('global-error-handler-plugin', arg => {
                if (!compilation.getAsset(globalErrorHandler[0])) {
                    compilation.emitAsset(globalErrorHandler[0], new webpack.sources.RawSource(globalErrorHandler[1]));
                    arg.assets.js.unshift(arg.assets.publicPath + globalErrorHandler[0]);
                }
                return arg;
            });
        });
...
const CorsPlugin = {
    apply(compiler) {
        compiler.hooks.compilation.tap('html-js-cors-plugin', compilation => {
            HTMLPlugin.getHooks(compilation).alterAssetTags.tap('html-js-cors-plugin', options => {
...
export function createHTMLPlugins(BUILD_CONFIG, config) {
    const publicPath = getPublicPath(BUILD_CONFIG);
