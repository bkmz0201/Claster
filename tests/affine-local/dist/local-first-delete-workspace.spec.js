...
import { openSettingModal, openWorkspaceSettingPanel, } from '@affine-test/kit/utils/setting';
...
test('Create new workspace, then delete it', async ({ page, workspace }) => {
    ...
    expect(await page.getByTestId('workspace-name').textContent()).toBe('Test Workspace');
    await openSettingModal(page);
    await openWorkspaceSettingPanel(page);
    await page.getByTestId('delete-workspace-button').click();
    await expect(page.locator('.affine-notification-center').first()).not.toBeVisible();
    const workspaceNameDom = page.getByTestId('workspace-name');
    const currentWorkspaceName = (await workspaceNameDom.evaluate(node => node.textContent));
    ...
    expect(await page.getByTestId('workspace-name').textContent()).toBe('Demo Workspace');
...
//# sourceMappingURL=local-first-delete-workspace.spec.js.map