...
export const MemberOptions = ({ member, isOwner, isAdmin, openAssignModal, goToTeamBilling, }) => {
    const t = useI18n();
    const membersService = useService(WorkspaceMembersService);
    const permission = useService(WorkspacePermissionService).permission;
    const isTeam = useLiveData(permission.isTeam$);
    const { openConfirmModal } = useConfirmModal();
    const openRemoveConfirmModal = useCallback((successNotify) => {
        openConfirmModal({
            title: t['com.affine.payment.member.team.remove.confirm.title'](),
            description: t['com.affine.payment.member.team.remove.confirm.description'](),
            confirmText: t['com.affine.payment.member.team.remove.confirm.confirm-button'](),
            cancelText: t['com.affine.payment.member.team.remove.confirm.cancel'](),
            confirmButtonOptions: {
                variant: 'error',
            },
            onConfirm: () => membersService
    ...
    const handleChangeToCollaborator = useCallback(() => {
        membersService
            ...
            if (result) {
                notify.success({
                    title: t['com.affine.payment.member.team.change.notify.title'](),
                    message: t['com.affine.payment.member.team.change.collaborator.notify.message']({
                        name: member.name || member.email || member.id,
                    }),
                });
                membersService.members.revalidate();
    ...
    const handleRetryPayment = useCallback(() => {
        openConfirmModal({
            ...
            description: t[`com.affine.payment.member.team.retry-payment.${isOwner ? 'owner' : 'admin'}.description`](),
    ...
    return (_jsx(_Fragment, { children: operationButtonInfo.map(item => item.show ? (_jsx(MenuItem, { onSelect: item.onClick, children: item.label }, item.label)) : null) }));
};
//# sourceMappingURL=member-option.js.map