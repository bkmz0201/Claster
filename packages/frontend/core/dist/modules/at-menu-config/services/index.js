...
import { DocRole, ErrorNames, } from '@affine/graphql';
import { I18n, i18nTime } from '@affine/i18n';
import track from '@affine/track';
import { DocModeProvider } from '@blocksuite/affine/shared/services';
import { BLOCK_ID_ATTR, } from '@blocksuite/affine/std';
import { LinkedWidgetUtils, } from '@blocksuite/affine/widgets/linked-doc';
import { DateTimeIcon, NewXxxEdgelessIcon, NewXxxPageIcon, UserIcon, } from '@blocksuite/icons/lit';
...
export class AtMenuConfigService extends Service {
    constructor(journalService, docDisplayMetaService, dialogService, docsService, searchMenuService, workspaceServerService, memberSearchService, guardService, docGrantedUsersService) {
        super();
        ...
        this.docGrantedUsersService = docGrantedUsersService;
        this.autoFocusedItemKey = (menus, query) => {
    ...
    insertDoc(inlineEditor, id) {
        LinkedWidgetUtils.insertLinkedNode({
            inlineEditor,
            docId: id,
        });
    }
    newDocMenuGroup(query, close, editorHost, inlineEditor) {
        const originalNewDocMenuGroup = LinkedWidgetUtils.createNewDocMenuGroup(query, close, editorHost, inlineEditor);
    ...
    journalGroup(query, close, inlineEditor) {
    ...
    linkToDocGroup(query, close, inlineEditor, abortSignal) {
        const action = (meta) => {
            close();
            track.doc.editor.atMenu.linkDoc();
            this.insertDoc(inlineEditor, meta.id);
        };
        const result = this.searchMenuService.getDocMenuGroup(query, action, abortSignal);
    ...
    highlightFuseTitle(matches, title, key) {
        if (!matches) {
            return title;
        }
    ...
    memberGroup(query, close, inlineEditor, _) {
        const getMenuItem = (id, name, avatar, sendNotification = true) => {
            ...
            return {
                ...
                action: () => {
                    ...
                    if (!block)
                        return;
                    const notificationService = this.workspaceServerService.server?.scope.get(NotificationService);
                    ...
                    if (!sendNotification)
                        return;
                    const relativePosition = createRelativePositionFromTypeIndex(inlineEditor.yText, inlineRange.index + 1);
                    notificationService
                        ...
                        if (!doc)
                            return;
                        const absolutePosition = createAbsolutePositionFromRelativePosition(relativePosition, doc);
                        if (!absolutePosition)
                            return;
                        ...
                        if (err.is(ErrorNames.MENTION_USER_DOC_ACCESS_DENIED)) {
                            ...
                            const canUserManage = this.guardService.can$('Doc_Users_Manage', docId).signal.value;
                            if (canUserManage) {
                                const username = name ?? 'Unknown';
                                notify.error({
                                    title: I18n.t('com.affine.editor.at-menu.access-needed'),
                                    message: I18n['com.affine.editor.at-menu.access-needed-message']({
                                        username,
                                    }),
                                    actions: [
                                        {
                                            ...
                                            onClick: async () => {
                                                ...
                                                try {
                                                    await this.docGrantedUsersService.updateUserRole(id, DocRole.Reader);
                                                    await notificationService.mentionUser(id, workspaceId, {
                                                        id: docId,
                                                        title: this.docDisplayMetaService.title$(docId)
                                                            .value,
                                                        blockId: block.blockId,
                                                        mode: mode,
                                                    });
                                                    notify.success({
                                                        title: I18n.t('com.affine.editor.at-menu.invited-and-notified'),
                                                    });
                                                }
                                                catch (error) {
                                                    const err = UserFriendlyError.fromAny(error);
                                ...
                                notify.error({
                                    title: I18n.t('com.affine.editor.at-menu.member-not-notified'),
                                    message: I18n['com.affine.editor.at-menu.member-not-notified-message'](),
                                });
                            }
                            return;
                        }
        ...
        const items = computed(() => {
            ...
            const currentUser = this.workspaceServerService.server?.scope.get(AuthService).session
            ...
            if (query.length === 0) {
                return [
                    ...(currentUser
                        ? [
                            getMenuItem(currentUser.id, currentUser.info?.name, currentUser.info?.avatarUrl, false),
                        ]
                        : []),
                    ...members
                        .slice(0, 2)
            ...
            return [
                ...searchResults.map(result => {
                    ...
                    const displayName = this.highlightFuseTitle(result.matches, member.name ?? 'Unknown', 'name');
                    return {
                        ...getMenuItem(member.id, member.name, member.avatarUrl, member.id !== currentUser?.id),
                        name: html `${unsafeHTML(displayName)}`,
                    };
                }),
                ...(canUserManage ? [inviteItem] : []),
