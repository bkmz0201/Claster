...
import { pressArrowDown, pressArrowLeft, pressArrowRight, pressArrowUp, pressBackspace, pressEnter, pressEscape, pressShiftEnter, pressShiftTab, pressTab, redoByKeyboard, SHORT_KEY, type, undoByKeyboard, } from './utils/actions/keyboard.js';
import { captureHistory, enterPlaygroundRoom, focusRichText, getInlineSelectionText, getPageSnapshot, getSelectionRect, initEmptyEdgelessState, initEmptyParagraphState, insertThreeLevelLists, waitNextFrame, } from './utils/actions/misc.js';
import { assertAlmostEqual, assertBlockCount, assertRichTexts, } from './utils/asserts.js';
import { test } from './utils/playwright.js';
function formatWithTimezone(date, timeZone = 'Asia/Tokyo') {
...
test.describe('slash menu should show and hide correctly', () => {
    ...
    test('slash menu should hide after click away', async ({ page, }, testInfo) => {
        await initEmptyParagraphState(page);
        const slashMenu = page.locator(`.slash-menu`);
        await focusRichText(page);
        await type(page, '/');
    ...
    test('delete the slash symbol should close the slash menu', async ({ page, }, testInfo) => {
        await initEmptyParagraphState(page);
        const slashMenu = page.locator(`.slash-menu`);
        await focusRichText(page);
        await type(page, '/');
    ...
    test('typing something that does not match should close the slash menu', async ({ page, }) => {
        await initEmptyParagraphState(page);
        const slashMenu = page.locator(`.slash-menu`);
        await focusRichText(page);
        await type(page, '/');
    ...
    test('pressing the slash key again should close the old slash menu and open new one', async ({ page, }) => {
        await initEmptyParagraphState(page);
        const slashMenu = page.locator(`.slash-menu`);
        await focusRichText(page);
        await type(page, '/');
    ...
    test('should move up down with ctrl/cmd+n and ctrl/cmd+p', async ({ page, }) => {
        await initEmptyParagraphState(page);
        const slashMenu = page.locator(`.slash-menu`);
        await focusRichText(page);
        await type(page, '/');
    ...
    test('navigate menu with left right arrow and Enter', async ({ page }) => {
        ...
        await expect(slashMenu, 'root menu should be visible when press arrow left').toBeVisible();
        await pressEscape(page);
        // Test sub menu case
        const slashItems = slashMenu.locator('icon-button');
        await type(page, '/');
...
test('should clean slash string after soft enter', async ({ page, }, testInfo) => {
...
test('should focus on code blocks created by the slash menu', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await focusRichText(page);
    await type(page, '000');
...
test('delete block by slash menu should remove children', async ({ page, }, testInfo) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
...
test('should slash menu can trigger linked doc popover', async ({ page }) => {
    ...
    await expect(page.locator('affine-reference .affine-reference-title')).toHaveText('doc');
});
//# sourceMappingURL=slash-menu.spec.js.map