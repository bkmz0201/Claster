...
import { activeNoteInEdgeless, click, copyByKeyboard, countBlock, dragBetweenCoords, enterPlaygroundRoom, fillLine, focusRichText, getNoteRect, initEmptyEdgelessState, initSixParagraphs, pasteByKeyboard, pressEscape, redoByClick, redoByKeyboard, selectNoteInEdgeless, switchEditorMode, triggerComponentToolbarAction, type, undoByClick, undoByKeyboard, waitNextFrame, zoomResetByKeyboard, } from '../../utils/actions/index.js';
...
test('continuous undo and redo (note block add operation) should work', async ({ page, }) => {
    await enterPlaygroundRoom(page);
...
test('undo/redo should work when change note custom background', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    const { noteId } = await initEmptyEdgelessState(page);
    await switchEditorMode(page);
    await selectNoteInEdgeless(page, noteId);
...
//# sourceMappingURL=undo-redo.spec.js.map