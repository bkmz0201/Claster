...
import { captureHistory, copyByKeyboard, dragOverTitle, enterPlaygroundRoom, focusRichText, focusTitle, getClipboardHTML, getClipboardSnapshot, getClipboardText, getCurrentEditorDocId, getEditorLocator, getPageSnapshot, initEmptyParagraphState, mockParseDocUrlService, pasteByKeyboard, pasteContent, pressArrowRight, pressEnter, pressShiftTab, pressTab, resetHistory, selectAllByKeyboard, setInlineRangeInSelectedRichText, setSelection, SHORT_KEY, type, undoByClick, undoByKeyboard, waitNextFrame, } from '../utils/actions/index.js';
import { assertBlockTypes, assertClipItems, assertRichTexts, assertText, assertTitle, } from '../utils/asserts.js';
...
test('should keep first line format when pasted into a new line', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await focusRichText(page);
    const clipData = `
...
test(scoped `pasting internal url with params`, async ({ page }) => {
    ...
    await pasteContent(page, {
        'text/plain': 'http://workspace/doc-id?mode=page&blockIds=rL2_GXbtLU2SsJVfCSmh_',
    });
    await expect(page.locator('affine-reference')).toContainText('test page');
});
test(scoped `pasting an external URL from clipboard to automatically creating a link from selection`, async ({ page }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await focusTitle(page);
    await type(page, 'test page');
...
test(scoped `pasting an internal URL from clipboard to automatically creating a link from selection`, async ({ page }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await focusTitle(page);
    await type(page, 'test page');
...
test(scoped `clipboard copy nested items`, async ({ page }, testInfo) => {
    ...
    expect(JSON.stringify(snapshot2.snapshot.content, null, 2)).toMatchSnapshot(`${testInfo.title}-clipboard2.json`);
