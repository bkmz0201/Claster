export function universalId({ peer, type, id, }) {
    return `@peer(${peer});@type(${type});@id(${id});`;
}
export function isValidSpaceType(type) {
    return type === 'workspace' || type === 'userspace';
}
export function isValidUniversalId(opts) {
...
export function parseUniversalId(id) {
    const result = {};
    let key = '';
    let value = '';
    let isInValue = false;
    ...
    if (!isValidUniversalId(result)) {
        throw new Error(`Invalid universal storage id: ${id}. It should be in format of @peer(\${peer});@type(\${type});@id(\${id});`);
    }
    return result;
}
//# sourceMappingURL=universal-id.js.map