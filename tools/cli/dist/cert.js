...
export class CertCommand extends Command {
    constructor() {
        super(...arguments);
        this.install = Option.Boolean('--install', {
            description: 'Install the CA',
        });
        this.domain = Option.String('--domain', {
            description: 'Generate certificates for given domain. e.g. "affine.localhost"',
        });
        this.uninstall = Option.Boolean('--uninstall', {
            description: 'Uninstall the CA',
        });
    }
    static { this.paths = [['cert']]; }
    async execute() {
        if (this.install) {
            this.installCa();
        }
    ...
    createCert(domain) {
        if (!this.checkInstalled(CA_PEM_PATH)) {
            this.logger.error('CA not installed. Please run `yarn affine cert --install` first.');
            process.exit(1);
        }
        ...
        this.exec(`openssl req -new -key ${keyPath} -out ${csrPath} -config ${confPath} -subj "/C=/ST=/O=/localityName=/commonName=${domain}/organizationalUnitName=/emailAddress=${domain}@affine.pro/"`);
        this.exec(`openssl x509 -req -days 1024 -in ${csrPath} -CA ${CA_PEM_PATH} -CAkey ${CA_KEY_PATH} -CAcreateserial -out ${crtPath} -extensions v3_req -extfile ${confPath}`);
    ...
    installCa() {
        ...
        CA_KEY_PATH.rm();
        CA_DIR.mkdir();
        this.exec(`openssl req -new -newkey rsa:2048 -days 1024 -nodes -x509 -subj "/C=/ST=/O=${CA_ORG}/localityName=/commonName=${CA_NAME}/organizationalUnitName=Developers/emailAddress=dev@affine.pro/" -keyout ${CA_KEY_PATH} -out ${CA_PEM_PATH}`);
        this.trustCa(CA_PEM_PATH);
    }
    trustCa(pem) {
        this.logger.info(`Trusting AFFiNE Dev Self Signed CA`);
        this.exec(`sudo security add-trusted-cert -d -r trustRoot -k /Library/Keychains/System.keychain ${pem}`);
    }
    uninstallCa(pem) {
        ...
        this.exec(`sudo security delete-certificate -c ${CA_NAME} /Library/Keychains/System.keychain`);
    }
    checkInstalled(pem) {
        if (!pem.exists()) {
            return false;
