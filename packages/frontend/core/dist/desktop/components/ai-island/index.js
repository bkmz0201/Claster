...
export const AIIsland = () => {
    ...
    const haveChatTab = useLiveData(activeView.sidebarTabs$.map(tabs => tabs.some(t => t.id === 'chat')));
    ...
    useEffect(() => {
        ...
        if (haveChatTab) {
            ...
            hide = hideChat.some(item => typeof item === 'string' ? path === item : item(path));
        }
        setHide(hide);
    ...
    return (_jsx(IslandContainer, { className: clsx(toolStyle, { hide }), children: _jsx("div", { className: aiIslandWrapper, "data-hide": hide, children: _jsx("button", { className: aiIslandBtn, "data-testid": "ai-island", onClick: onOpenChat, children: _jsx(AIIcon, {}) }) }) }));
};
