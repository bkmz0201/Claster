...
import { clickEdgelessModeButton, clickView, createEdgelessNoteBlock, fitViewportToContent, focusDocTitle, getSelectedXYWH, locateEditorContainer, resizeElementByHandle, scaleElementByHandle, } from '@affine-test/kit/utils/editor';
import { pressEnter } from '@affine-test/kit/utils/keyboard';
import { openHomePage } from '@affine-test/kit/utils/load-page';
import { clickNewPageButton, createLinkedPage, waitForEditorLoad, } from '@affine-test/kit/utils/page-logic';
...
test('should not show hidden note in embed view page mode', async ({ page, }) => {
...
test.describe('edgeless', () => {
    ...
    test.describe('header of edgeless embed synced doc', () => {
        test('should fold button works', async ({ page }) => {
            ...
            const foldButton = embedBlock.getByTestId('edgeless-embed-synced-doc-fold-button');
        ...
        test('should show title in header', async ({ page }) => {
            const embedBlock = page.locator('affine-embed-edgeless-synced-doc-block');
            const headerTitle = embedBlock.getByTestId('edgeless-embed-synced-doc-title');
    ...
    test.describe('size adjustment of embed synced doc', () => {
        ...
        test('should fold embed synced doc when adjust height to smallest', async ({ page, }) => {
            ...
            expect(h2, 'should recover height when unfold embed synced doc which was resized to smallest height directly').toEqual(h);
        });
        test('should be able to adjust height the folded embed synced doc', async ({ page, }) => {
            const embedBlock = page.locator('affine-embed-edgeless-synced-doc-block');
            const content = embedBlock.locator('editor-host');
            const foldButton = embedBlock.getByTestId('edgeless-embed-synced-doc-fold-button');
            await foldButton.click();
            ...
            await expect(content, 'should unfold the embed synced doc when adjust height to greater').toBeInViewport();
            await expect(foldButton).toHaveAttribute('data-folded', 'false');
            await expect(content).toBeInViewport();
        });
    });
});
//# sourceMappingURL=synced.spec.js.map