...
export const AICancel = (btnProps) => {
    const t = useI18n();
    ...
    const cancel = useAsyncCallback(async () => {
        ...
        openConfirmModal({
            ...
            description: t['com.affine.payment.ai.action.cancel.confirm.description'](),
            reverseFooter: true,
            confirmText: t['com.affine.payment.ai.action.cancel.confirm.confirm-text'](),
            confirmButtonOptions: {
                variant: 'secondary',
            },
            cancelText: t['com.affine.payment.ai.action.cancel.confirm.cancel-text'](),
            cancelButtonOptions: {
                variant: 'primary',
            },
            onConfirm: async () => {
                try {
                    setMutating(true);
                    await subscription.cancelSubscription(idempotencyKey, SubscriptionPlan.AI);
    ...
    return (_jsx(Button, { onClick: cancel, loading: isMutating, variant: "secondary", ...btnProps, children: t['com.affine.payment.ai.action.cancel.button-label']() }));
};
