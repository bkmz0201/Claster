...
import { clickEdgelessModeButton, dragView, locateToolbar, setEdgelessTool, } from '@affine-test/kit/utils/editor';
import { importImage } from '@affine-test/kit/utils/image';
import { selectAllByKeyboard, writeTextToClipboard, } from '@affine-test/kit/utils/keyboard';
import { openHomePage } from '@affine-test/kit/utils/load-page';
import { clickNewPageButton, waitForEmptyEditor, } from '@affine-test/kit/utils/page-logic';
...
test('should not show toolbar when releasing spacebar and elements have been deleted', async ({ page, }) => {
...
test('should not show inner toolbar of surface-ref in note under edgeless', async ({ page, }) => {
    await page.keyboard.press('Enter');
    ...
    await expect(edgelessSurfaceRef.locator('.affine-edgeless-surface-ref-container')).toHaveClass(/focused$/);
    await expect(toolbar).toBeHidden();
});
test('should show toolbar when inline link is preceded by image or surface-ref', async ({ page, }) => {
    await page.keyboard.press('Enter');
...
test('should focus on input of popover on toolbar', async ({ page }) => {
    ...
    const cornersInput = stylePanelButton.locator('.edgeless-note-corner-radius-panel input');
...
test('Dropdown menus should be closed automatically when toolbar is displayed', async ({ page, }) => {
    await page.keyboard.press('Enter');
    await page.keyboard.type('/frame');
    await page.keyboard.press('Enter');
    const toolbar = locateToolbar(page);
