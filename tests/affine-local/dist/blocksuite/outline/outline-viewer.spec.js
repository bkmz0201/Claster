...
import { clickEdgelessModeButton, clickPageModeButton, createEdgelessNoteBlock, } from '@affine-test/kit/utils/editor';
import { pressBackspace, pressEnter, selectAllByKeyboard, } from '@affine-test/kit/utils/keyboard';
import { openHomePage } from '@affine-test/kit/utils/load-page';
import { clickNewPageButton, createLinkedPage, type, waitForEditorLoad, } from '@affine-test/kit/utils/page-logic';
...
test('should display indicators when non-empty headings exists', async ({ page, }) => {
...
test('should update indicator when clear title or headings', async ({ page, }) => {
    const indicators = getIndicators(page);
...
test('should display simple outline panel when hovering over indicators', async ({ page, }) => {
    const indicators = getIndicators(page);
...
test('should highlight indicator when click item in outline panel', async ({ page, }) => {
    const viewer = page.locator('affine-outline-viewer');
    const indicators = getIndicators(page);
    const headings = await createHeadings(page, 10);
    await indicators.first().hover({ force: true });
    const headingsInPanel = Array.from({ length: 6 }, (_, i) => viewer.getByTestId(`outline-block-preview-h${i + 1}`));
...
test('outline viewer should update after change heading in edgeless mode', async ({ page, }) => {
    await createTitle(page);
    await pressEnter(page);
    await type(page, '# ');
    await type(page, 'Heading 1');
...
test('outline viewer should be useable in doc peek preview', async ({ page, }) => {
    await pressEnter(page);
    ...
    {
        const headingButtons = peekView.locator('affine-outline-viewer .outline-viewer-item:not(.outline-viewer-header)');
    ...
    {
        await indicators.first().hover({ force: true });
        const toggleButton = peekView.locator('.outline-viewer-header edgeless-tool-icon-button');
...
//# sourceMappingURL=outline-viewer.spec.js.map