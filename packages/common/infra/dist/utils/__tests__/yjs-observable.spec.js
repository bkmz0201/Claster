...
describe('yjs observable', () => {
    test('basic', async () => {
        ...
        yjsGetPath(ydoc.getMap('foo'), 'key.subkey').subscribe(v => (currentValue = v));
        expect(currentValue).toBe(undefined);
        ydoc.getMap('foo').set('key', new YMap([['subkey', 'xxxzzz']]));
        expect(currentValue).toBe('xxxzzz');
        ydoc.getMap('foo').get('key').set('subkey', 'yyy');
        expect(currentValue).toBe('yyy');
        ydoc.getMap('foo').get('key').delete('subkey');
        expect(currentValue).toBe(undefined);
        ydoc.getMap('foo').get('key').set('subkey', 'yyy');
    ...
    test('observe with path', async () => {
        ...
        yjsObservePath(ydoc.getMap('metas'), 'pages.*.tags').subscribe(v => {
            ...
            currentValue = v
                .toJSON()
                .pages?.map((page) => ({ id: page.id, tags: page.tags ?? [] }));
        });
        expect(callbackCount).toBe(1);
        ydoc.getMap('metas').set('pages', new YArray());
        expect(callbackCount).toBe(2);
        expect(currentValue).toStrictEqual([]);
        const pages = ydoc.getMap('metas').get('pages');
        pages.push([
            new YMap([
                ['id', '1'],
                ['title', 'page 1'],
...
//# sourceMappingURL=yjs-observable.spec.js.map