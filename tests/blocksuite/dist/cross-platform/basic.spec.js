...
test(scoped `basic input`, async ({ page }) => {
    ...
    expect(await getPageSnapshot(page, true)).toMatchSnapshot(`${BASIC_DEFAULT_SNAPSHOT}.json`);
...
test(scoped `basic init with external text`, async ({ page }) => {
    ...
    await page.evaluate(() => {
        ...
        doc.addBlock('affine:paragraph', {
            text: new window.$blocksuite.store.Text(delta),
        }, note);
    });
...
test(scoped `A open and edit, then joins B`, async ({ context, page: pageA }) => {
    const room = await enterPlaygroundRoom(pageA);
    await initEmptyParagraphState(pageA);
    await waitNextFrame(pageA);
    ...
    await Promise.all([
        ...
        expect(await getPageSnapshot(pageA, true)).toMatchSnapshot(`${BASIC_DEFAULT_SNAPSHOT}.json`),
        expect(await getPageSnapshot(pageB, true)).toMatchSnapshot(`${BASIC_DEFAULT_SNAPSHOT}.json`),
...
test(scoped `does not sync when disconnected`, async ({ browser, page: pageA }) => {
    test.fail();
    const room = await enterPlaygroundRoom(pageA);
...
test(scoped `should be able to delete an emoji completely by pressing backspace once`, async ({ page }) => {
...
test(scoped `ZERO_WIDTH_FOR_EMPTY_LINE should be counted by one cursor position`, async ({ page }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await focusRichText(page);
...
test('when no note block, click editing area auto add a new note block', async ({ page, }) => {
    await enterPlaygroundRoom(page);
