 ...
 *
 */
export const QuotaCheck = ({ workspaceMeta, }) => {
    ...
    const onConfirm = useCallback(() => {
        ...
        if (dialog.some(d => (d.type === 'setting' && d.props.activeTab === 'plans') ||
    ...
    useEffect(() => {
        ...
        const message = getSyncPausedMessage(!!isOwner, memberOverflow, !!storageOverflow);
        if (memberOverflow || storageOverflow) {
            openConfirmModal({
                title: _jsx(Title, { title: t.t(message.title) }),
                ...
                children: _jsx(Tips, { tips: message.tips }),
...
function getSyncPausedMessage(isOwner, isMemberOverflow, isStorageOverflow) {
    const userType = isOwner ? 'owner' : 'member';
    const condition = isStorageOverflow && isMemberOverflow
        ? 'both'
        : isStorageOverflow
            ? 'storage'
            : isMemberOverflow
...
const Tips = ({ tips }) => {
    ...
    return (_jsx("div", { className: styles.tipsStyle, children: tips.map(tip => (_jsxs("div", { className: styles.tipStyle, children: [_jsx("div", { className: styles.bullet }), t.t(tip)] }, tip.toString()))) }));
};
const Title = ({ title }) => {
    return (_jsxs("div", { className: styles.modalTitle, children: [_jsx(InformationFillDuotoneIcon, { className: styles.errorIcon }), title] }));
};
