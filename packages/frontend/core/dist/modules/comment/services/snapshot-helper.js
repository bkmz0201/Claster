...
import { customImageProxyMiddleware, MarkdownAdapter, } from '@blocksuite/affine/shared/adapters';
import { nanoid, Text, Transformer, } from '@blocksuite/affine/store';
...
export class SnapshotHelper extends Service {
    ...
    getTempWorkspace() {
        const collection = new WorkspaceImpl({
            ...
            blobSource: {
                ...
                readonly: true,
                get: async (key) => {
                    const record = await this.workspaceService.workspace.engine.blob.get(key);
    ...
    getTransformer() {
        ...
        const imageProxyUrl = new URL(BUILD_CONFIG.imageProxyUrl, this.serverService.baseUrl).toString();
