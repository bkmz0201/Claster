...
import { clickEdgelessModeButton, dragView, locateToolbar, } from '@affine-test/kit/utils/editor';
...
test('should keep color on custom color button', async ({ page }) => {
    ...
    const importSnapshot = importDialog.getByTestId('editor-option-menu-import-snapshot');
    ...
    // frame
    {
        ...
        const customColorButton = colorPicker.locator('edgeless-color-custom-button');
        const keptColor = await customColorButton.evaluate(e => e.style.getPropertyValue('--c'));
    ...
    // shape
    {
        ...
        const strokeCustomColorButton = colorPicker
            .locator('edgeless-color-custom-button')
            .last();
        const fillKeptColor = await fillCustomColorButton.evaluate(e => e.style.getPropertyValue('--c'));
        const strokeKeptColor = await strokeCustomColorButton.evaluate(e => e.style.getPropertyValue('--c'));
