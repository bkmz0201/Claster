...
import { expectActiveTab, expectTabTitle, } from '@affine-test/kit/utils/app-tabs';
import { clickNewPageButton, createLinkedPage, dragTo, getPageByTitle, waitForAllPagesLoad, } from '@affine-test/kit/utils/page-logic';
...
test('open split view', async ({ page }) => {
    ...
    await expectActiveTab(page, 0, 0);
    await expect(page.getByTestId('split-view-indicator').nth(0)).toHaveAttribute('data-active', 'true');
    const firstDragHandle = page
        .getByTestId('split-view-panel')
        .first()
        .getByTestId('split-view-indicator');
    await dragTo(page, firstDragHandle, page.getByTestId('split-view-panel').last(), 'center', true);
...
test('open split view in all docs (operations button)', async ({ page }) => {
    ...
    await expect(targetPage.locator('.doc-title-container')).toContainText(testTitle);
});
test('open split view in all docs (drag to resize handle)', async ({ page, }) => {
    const testTitle = 'test-page';
    await clickNewPageButton(page, testTitle);
    await clickSideBarAllPageButton(page);
    await waitForAllPagesLoad(page);
...
test('creating split view by dragging sidebar journals', async ({ page }) => {
    ...
    await expect(page
        ...
        .getByTestId('date-today-label')).toBeVisible();
});
//# sourceMappingURL=split-view.spec.js.map