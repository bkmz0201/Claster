...
import { assertBlockTypes, assertEdgelessNoteBackground, assertEdgelessSelectedModelRect, assertRichTextModelType, assertRichTexts, assertText, } from '../utils/asserts.js';
...
test('copy a nested list by clicking button, the clipboard data should be complete', async ({ page, }, testInfo) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await focusRichText(page);
    const clipData = {
...
`,
    ...
    await dragBetweenCoords(page, { x: rootListBound.x + 1, y: rootListBound.y - 1 }, { x: rootListBound.x + 1, y: rootListBound.y + rootListBound.height - 1 });
    ...
    expect(JSON.stringify(snapshot.snapshot.content, null, 2)).toMatchSnapshot(`${testInfo.title}-clipboard.json`);
...
test(scoped `pasting into empty list should not convert the list into paragraph`, async ({ page }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await focusRichText(page);
...
test('cut will delete all content, and copy will reappear content', async ({ page, }, testInfo) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await focusRichText(page);
    await type(page, '-');
    ...
    await cutByKeyboard(page);
    await waitNextFrame(page);
    expect(await getPageSnapshot(page, true)).toMatchSnapshot(`${testInfo.title}_after-cut.json`);
    await waitNextFrame(page);
    await focusRichText(page);
    await pasteByKeyboard(page);
    await waitNextFrame(page);
    expect(await getPageSnapshot(page, true)).toMatchSnapshot(`${testInfo.title}_after-paste.json`);
});
...
test(scoped `paste note block with background`, async ({ page }) => {
    ...
    await assertEdgelessNoteBackground(page, ids.noteId, lightThemeV2['edgeless/note/white']);
    await copyByKeyboard(page);
...
test(scoped `should keep paragraph block's type when pasting at the start of empty paragraph block except type text`, async ({ page }, testInfo) => {
    test.info().annotations.push({
        type: 'issue',
    ...
    await pasteByKeyboard(page);
    await waitNextFrame(page);
    expect(await getPageSnapshot(page, true)).toMatchSnapshot(`${testInfo.title}_after-paste-1.json`);
    await pressEnter(page);
    await waitNextFrame(page);
    await pressEnter(page);
    await waitNextFrame(page);
    await pasteByKeyboard(page, false);
    await waitNextFrame(page);
    expect(await getPageSnapshot(page, true)).toMatchSnapshot(`${testInfo.title}_after-paste-2.json`);
});
