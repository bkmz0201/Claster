...
import { pressEnter, pressShiftTab, pressTab, selectAllByKeyboard, undoByKeyboard, } from '@affine-test/kit/utils/keyboard';
...
test('heading icon should be updated after change heading level', async ({ page, }) => {
...
test('add new heading when press enter at the end of collapsed heading', async ({ page, }) => {
    await pressEnter(page);
    await type(page, '# aaa');
    ...
    expect(await paragraph
        .nth(1)
        .evaluate((block) => block.model.props.text.toString() === 'bbb')).toBeTruthy();
    await pressEnter(page);
    expect(await paragraph
        .nth(1)
        .evaluate((block) => block.model.props.type === 'text' &&
        block.model.props.text.toString() === '')).toBeTruthy();
    expect(await paragraph
        .nth(2)
        .evaluate((block) => block.model.props.text.toString() === 'bbb')).toBeTruthy();
    await undoByKeyboard(page);
    ...
    expect(await paragraph
        .nth(2)
        .evaluate((block) => block.model.props.type === 'h1' &&
        block.model.props.text.toString() === '')).toBeTruthy();
});
test('unfold collapsed heading when some block indented to be its child', async ({ page, }) => {
    await pressEnter(page);
    await type(page, '# aaa\nbbb\n# ccc');
    const paragraph = page.locator('affine-note affine-paragraph');
    await paragraph
...
test('unfold collapsed heading when its siblings changed to text type from heading type', async ({ page, }) => {
    await pressEnter(page);
    await type(page, '# aaa\nbbb\n# ccc');
    const paragraph = page.locator('affine-note affine-paragraph');
    await paragraph
    ...
    expect(await paragraph
        .nth(2)
        .evaluate((block) => block.model.props.type === 'h1' &&
        block.model.props.text.toString() === 'ccc')).toBeTruthy();
...
test('also move children when dedent collapsed heading', async ({ page }) => {
    ...
    expect(await subParagraph
        .nth(0)
        .evaluate((block) => block.model.props.type === 'h1' &&
        block.model.props.text.toString() === 'bbb')).toBeTruthy();
    expect(await subParagraph
        .nth(1)
        .evaluate((block) => block.model.props.type === 'text' &&
        block.model.props.text.toString() === 'ccc')).toBeTruthy();
...
test('also move collapsed siblings when indent collapsed heading', async ({ page, }) => {
    await pressEnter(page);
    await type(page, 'aaa');
    await page.keyboard.press('ArrowDown');
    await type(page, '# bbb\nccc');
...
test('unfold collapsed heading when its other blocks indented to be its sibling', async ({ page, }) => {
    await pressEnter(page);
    await type(page, 'aaa');
    await page.keyboard.press('ArrowDown');
    await type(page, '# bbb\nddd');
