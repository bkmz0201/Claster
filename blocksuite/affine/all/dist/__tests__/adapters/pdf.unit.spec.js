...
function createBaseSnapshot(children) {
    return {
        type: 'block',
        id: 'block:test',
        flavour: 'affine:page',
...
describe('snapshot to pdf', () => {
    test('paragraph', async () => {
        ...
        const definition = await pdfAdapter.getDocDefinition([blockSnapshot], undefined);
        ...
        const paragraphContent = content.find((item) => item.text === 'Hello World' ||
    ...
    test('code block', async () => {
        ...
        const codeBlock = content.find((item) => item.table && item.table.body);
    ...
    test('list items', async () => {
        ...
        const listItems = content.filter((item) => item.table && item.table.widths && item.table.widths.length === 2);
    ...
    test('header', async () => {
        ...
        const content = definition.content;
        // Find header content
        const header = content.find((item) => item.style === 'header1' ||
            (item.text &&
                (item.text === 'Heading 1' ||
    ...
    test('document with title', async () => {
        ...
        const pdfAdapter = new PdfAdapter(createJob(), provider);
        const definition = await pdfAdapter.getDocDefinition([docSnapshot.blocks], docSnapshot.meta?.title);
        expect(definition.content).toBeDefined();
        const content = definition.content;
    ...
    describe('inline text styling', () => {
        test('bold text', async () => {
            ...
            if (Array.isArray(textContent.text)) {
                const styledText = textContent.text.find((t) => typeof t === 'object' && t.bold === true);
        ...
        test('italic text', async () => {
            ...
            expect(textContent).toBeDefined();
            if (Array.isArray(textContent.text)) {
                const styledText = textContent.text.find((t) => typeof t === 'object' && t.italics === true);
                expect(styledText).toBeDefined();
        ...
        test('underline text', async () => {
            ...
            expect(textContent).toBeDefined();
            if (Array.isArray(textContent.text)) {
                const styledText = textContent.text.find((t) => typeof t === 'object' &&
        ...
        test('inline code', async () => {
            ...
            expect(textContent).toBeDefined();
            if (Array.isArray(textContent.text)) {
                const codeText = textContent.text.find((t) => typeof t === 'object' &&
                    t.font === 'Inter' &&
                    t.background === '#f5f5f5');
        ...
        test('combined styles', async () => {
            ...
            expect(textContent).toBeDefined();
            if (Array.isArray(textContent.text)) {
                const styledText = textContent.text.find((t) => typeof t === 'object' && t.bold === true && t.italics === true);
                expect(styledText).toBeDefined();
            }
        });
    });
    describe('links and references', () => {
        test('link attribute', async () => {
            ...
            expect(textContent).toBeDefined();
            if (Array.isArray(textContent.text)) {
                const linkText = textContent.text.find((t) => typeof t === 'object' && t.link === 'https://example.com');
        ...
        test('linked page reference - found', async () => {
            ...
            expect(textContent).toBeDefined();
            if (Array.isArray(textContent.text)) {
                const refText = textContent.text.find((t) => typeof t === 'object' &&
        ...
        test('linked page reference - not found', async () => {
            ...
            expect(textContent).toBeDefined();
            if (Array.isArray(textContent.text)) {
                const refText = textContent.text.find((t) => typeof t === 'object' && t.text === 'Page not found');
                expect(refText).toBeDefined();
            }
        });
    });
    describe('quote blocks', () => {
        test('quote block with text', async () => {
            ...
            const definition = await pdfAdapter.getDocDefinition([blockSnapshot], undefined);
            const content = definition.content;
            const quoteBlock = content.find((item) => item.table &&
                item.table.widths &&
        ...
        test('quote block with children', async () => {
            ...
            const definition = await pdfAdapter.getDocDefinition([blockSnapshot], undefined);
            const content = definition.content;
            const quoteBlock = content.find((item) => item.table && item.table.widths && item.table.widths.length === 2);
            expect(quoteBlock).toBeDefined();
    ...
    describe('callout blocks', () => {
        test('callout with default background', async () => {
            ...
            const definition = await pdfAdapter.getDocDefinition([blockSnapshot], undefined);
            const content = definition.content;
            const calloutBlock = content.find((item) => item.table && item.table.widths && item.table.widths.length === 1);
    ...
    describe('image handling', () => {
        test('image without sourceId', async () => {
            ...
            const definition = await pdfAdapter.getDocDefinition([blockSnapshot], undefined);
            const content = definition.content;
            const imageContent = content.find((item) => item.text &&
        ...
        test('SVG image', async () => {
            ...
            const pdfAdapter = new PdfAdapter(createJob(), provider);
            const definition = await pdfAdapter.getDocDefinition([blockSnapshot], undefined, assets);
            const content = definition.content;
            const svgContent = content.find((item) => item.svg);
        ...
        test('image with missing asset', async () => {
            ...
            const definition = await pdfAdapter.getDocDefinition([blockSnapshot], undefined);
            const content = definition.content;
            const fallbackContent = content.find((item) => item.text &&
    ...
    describe('table rendering', () => {
        test('table with cells', async () => {
            ...
            const definition = await pdfAdapter.getDocDefinition([blockSnapshot], undefined);
            const content = definition.content;
            const table = content.find((item) => item.table && item.table.body && Array.isArray(item.table.body));
        ...
        test('empty table', async () => {
            ...
            const definition = await pdfAdapter.getDocDefinition([blockSnapshot], undefined);
            const content = definition.content;
            const table = content.find((item) => item.table && item.table.body);
    ...
    describe('edge cases', () => {
        ...
        test('text alignment', async () => {
            ...
            const definition = await pdfAdapter.getDocDefinition([blockSnapshot], undefined);
            const content = definition.content;
            const paragraph = content.find((item) => item.text &&
                (item.text === 'Centered text' ||
                    (Array.isArray(item.text) &&
                        item.text.some((t) => typeof t === 'string'
                            ? t === 'Centered text'
        ...
        test('numbered list with order', async () => {
            ...
            const definition = await pdfAdapter.getDocDefinition([blockSnapshot], undefined);
            const content = definition.content;
            const listItem = content.find((item) => item.table &&
                item.table.body &&
                item.table.body[0] &&
                item.table.body[0][0] &&
        ...
        test('todo list checked', async () => {
            ...
            const definition = await pdfAdapter.getDocDefinition([blockSnapshot], undefined);
            const content = definition.content;
            const todoItem = content.find((item) => item.table &&
                item.table.body &&
                item.table.body[0] &&
                item.table.body[0][0] &&
...
//# sourceMappingURL=pdf.unit.spec.js.map