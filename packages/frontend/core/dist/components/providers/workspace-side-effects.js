...
import { AIProvider, CopilotClient, setupAIProvider, } from '@affine/core/blocksuite/ai';
...
import { AuthService, EventSourceService, FetchService, GraphQLService, } from '@affine/core/modules/cloud';
...
import { effect, fromPromise, onStart, throwIfAborted, useService, useServices, } from '@toeverything/infra';
...
export const WorkspaceSideEffects = () => {
    ...
    useEffect(() => {
        const insertTemplate = effect(switchMap(({ template, mode }) => {
            return fromPromise(async (abort) => {
                ...
                const [doc] = await ZipTransformer.importDocs(currentWorkspace.docCollection, getAFFiNEWorkspaceSchema(), templateBlob);
                if (doc) {
                    doc.resetHistory();
                }
                return { doc, mode };
            }).pipe(timeout(10000 /* 10s */), tap(({ mode, doc }) => {
                if (doc) {
                    docsList.setPrimaryMode(doc.id, mode);
        ...
        const disposable = AIProvider.slots.requestInsertTemplate.subscribe(({ template, mode }) => {
    ...
    useEffect(() => {
        const dispose = setupAIProvider(new CopilotClient(graphqlService.gql, fetchService.fetch, eventSourceService.eventSource), globalDialogService, authService);
        return () => {
            dispose();
        };
    }, [
    ...
    return (_jsxs(_Fragment, { children: [_jsx(QuickSearchContainer, {}), _jsx(OverCapacityNotification, {})] }));
};
//# sourceMappingURL=workspace-side-effects.js.map