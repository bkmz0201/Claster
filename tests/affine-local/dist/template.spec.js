...
import { confirmCreateJournal, openHomePage, } from '@affine-test/kit/utils/load-page';
import { waitForEditorLoad } from '@affine-test/kit/utils/page-logic';
import { expect } from '@playwright/test';
function getTemplateRow(page) {
    return page.locator('[data-testid="doc-property-row"][data-info-id="template"]');
}
async function toggleTemplate(row, value) {
...
const createDocAndMarkAsTemplate = async (page, title, onCreated) => {
...
const enableAskMeEveryTime = async (page) => {
...
test('set default template for journal', async ({ page }) => {
    ...
    await waitForEditorLoad(page);
    await expect(page.getByText('This is a journal template doc')).not.toBeVisible();
...
test('create template doc from sidebar template entrance', async ({ page }) => {
    ...
    const templateDocId = await getDocId(page);
    await page.getByTestId('sidebar-template-doc-entrance').click();
    await expect(page.getByTestId(`template-doc-item-${templateDocId}`)).toBeVisible();
});
