export const calcPX = (px, totalWidth) => typeof px === 'number' ? px : px(totalWidth);
export const calcColumns = (totalWidth, itemWidth, itemWidthMin, gapX, _paddingX, columns) => {
    ...
    if (itemWidth === 'stretch') {
        ...
        const finalWidth = (availableWidth - (finalColumns - 1) * gapX) / finalColumns;
        return {
...
export const calcLayout = (groups, options) => {
    const { totalWidth, columns, width, gapX, gapY, paddingX: _paddingX, paddingY, groupsGap, groupHeaderGapWithItems, collapsedGroups, } = options;
    const paddingX = calcPX(_paddingX, totalWidth);
    ...
    groups.forEach((group, index) => {
        ...
        group.items.forEach(item => {
            ...
            const aroundGapXValue = columns > 1
...
export const calcActive = (options) => {
    ...
    layoutMap.forEach((layout, id) => {
        const { y, h } = layout;
        const isInView = y + h + preloadHeight > scrollY &&
