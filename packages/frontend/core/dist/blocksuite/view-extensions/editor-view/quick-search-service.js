...
import { BookmarkSlashMenuConfigIdentifier, insertLinkByQuickSearchCommand, } from '@blocksuite/affine/blocks/bookmark';
...
export function patchQuickSearchService(framework) {
    const QuickSearch = QuickSearchExtension({
        async openQuickSearch() {
            ...
            searchResult = await new Promise((resolve, reject) => framework.get(QuickSearchService).quickSearch.show([
    ...
    const SlashMenuQuickSearchExtension = {
        setup: di => {
            const overrideFn = (identifier) => {
                ...
                di.override(identifier, provider => {
                    ...
                    if (typeof prevConfig.items === 'function') {
                        ...
                        prevConfig.items = ctx => prevConfigItemGenerator(ctx).map(modifyFn);
                    }
                    else {
            ...
            const modifyFn = (item) => {
                if ('action' in item && item.name === 'Link') {
                    item.action = ({ std }) => {
                        const [success, { insertedLinkType }] = std.command.exec(insertLinkByQuickSearchCommand);
                        if (!success)
                            return;
                        insertedLinkType
                            ?.then(type => {
