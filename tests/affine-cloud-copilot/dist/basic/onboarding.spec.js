...
test.describe('AIBasic/Onboarding', () => {
    ...
    test('should show AI onboarding', async ({ loggedInPage: page }) => {
        ...
        // Show options
        await expect(page.getByTestId('read-foreign-language-article-with-ai')).toBeVisible();
        await expect(page.getByTestId('tidy-an-article-with-ai-mindmap-action')).toBeVisible();
        await expect(page.getByTestId('add-illustrations-to-the-article')).toBeVisible();
    ...
    test('read a foreign language article with AI', async ({ loggedInPage: page, utils, }) => {
    ...
    test('tidy an article with AI MindMap Action', async ({ loggedInPage: page, utils, }) => {
        await page.getByTestId('tidy-an-article-with-ai-mindmap-action').click();
        await utils.editor.isEdgelessMode(page);
        const docTitle = await utils.editor.getDocTitle(page);
        await expect(docTitle).toContain('Tidy');
    });
    test('add illustrations to the article', async ({ loggedInPage: page, utils, }) => {
...
//# sourceMappingURL=onboarding.spec.js.map