...
const equalComparator = (a, b) => {
    return isEqual(a, b);
};
export class DocDatabaseBacklinksService extends Service {
    constructor(docsService, docsSearchService) {
        super();
    ...
    adaptRowCells(dbModel, rowId) {
        ...
        const hydratedRows$ = combineLatest([
            ...
            return propertyIds
                ...
                return {
                    id,
                    value$: LiveData.fromSignal(dataSource.cellValueGet$(rowId, id)).distinctUntilChanged(equalComparator),
                    property: {
                        id,
        ...
        return [hydratedRows$, dataSource];
    }
    // for each db doc backlink,
    watchDatabaseRow$(backlink) {
        return new Observable(subscriber => {
            ...
            const run = async () => {
                ...
                const maybeDatabaseBlock = docRef.doc.blockSuiteDoc.getBlock(backlink.databaseBlockId);
                if (maybeDatabaseBlock?.flavour === 'affine:database') {
                    const dbModel = maybeDatabaseBlock.model;
                    const [cells$, dataSource] = this.adaptRowCells(dbModel, backlink.rowId);
    ...
    watchDbBacklinkRows$(docId, defaultItems) {
        return this.docsSearchService.watchDatabasesTo(docId).pipe(distinctUntilChanged(equalComparator), map(rows => rows.toSorted((a, b) => a.databaseName?.localeCompare(b.databaseName ?? '') ?? 0)), map(backlinks => {
            ...
            backlinks.forEach(backlink => {
                ...
                if (merged.some(item => item.databaseBlockId === backlink.databaseBlockId &&
...
//# sourceMappingURL=doc-database-backlinks.js.map