...
import { activeNoteInEdgeless, addNote, assertEdgelessTool, changeEdgelessNoteBackground, changeNoteDisplayMode, dragBetweenViewCoords, getSelectedBound, getSelectedBoundCount, getViewportCenter, locatorComponentToolbar, locatorEdgelessZoomToolButton, resizeElementByHandle, selectNoteInEdgeless, setEdgelessTool, setViewportCenter, switchEditorMode, triggerComponentToolbarAction, zoomOutByKeyboard, zoomResetByKeyboard, } from '../../utils/actions/edgeless.js';
import { click, clickBlockById, clickView, dragBetweenCoords, dragBetweenIndices, enterPlaygroundRoom, focusRichText, focusRichTextEnd, initEmptyEdgelessState, initThreeParagraphs, pressArrowDown, pressArrowUp, pressBackspace, pressEnter, pressEscape, pressTab, selectAllByKeyboard, type, undoByKeyboard, waitForInlineEditorStateUpdated, waitNextFrame, } from '../../utils/actions/index.js';
import { assertBlockChildrenIds, assertBlockCount, assertEdgelessNonSelectedRect, assertEdgelessNoteBackground, assertEdgelessSelectedRect, assertNoteSequence, assertNoteXYWH, assertRichTextInlineRange, assertRichTexts, assertTextSelection, } from '../../utils/asserts.js';
import { DEFAULT_NOTE_HEIGHT, DEFAULT_NOTE_WIDTH, NoteDisplayMode, } from '../../utils/bs-alternative.js';
...
test('can drag selected non-active note', async ({ page }) => {
    ...
    await dragBetweenCoords(page, { x: CENTER_X, y: CENTER_Y }, { x: CENTER_X, y: CENTER_Y + 100 });
...
test('dragging un-selected note', async ({ page }) => {
    ...
    await dragBetweenCoords(page, { x: noteBox.x + 10, y: noteBox.y + 15 }, { x: noteBox.x + 10, y: noteBox.y + 35 }, { steps: 10 });
    await assertEdgelessSelectedRect(page, [
        noteBox.x,
        noteBox.y + 20,
        noteBox.width,
...
test('format quick bar should show up when double-clicking on text', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyEdgelessState(page);
...
test('when editing text in edgeless, should hide component toolbar', async ({ page, }) => {
    await enterPlaygroundRoom(page);
...
test('double click toolbar zoom button, should not add text', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyEdgelessState(page);
    await switchEditorMode(page);
    const zoomOutButton = await locatorEdgelessZoomToolButton(page, 'zoomOut', false);
...
test('change note color', async ({ page }) => {
    ...
    await initThreeParagraphs(page);
    await switchEditorMode(page);
    await assertEdgelessNoteBackground(page, noteId, lightThemeV2['edgeless/note/white']);
    await selectNoteInEdgeless(page, noteId);
    await triggerComponentToolbarAction(page, 'changeNoteStyle');
    await changeEdgelessNoteBackground(page, 'Green');
    await assertEdgelessNoteBackground(page, noteId, lightThemeV2['edgeless/note/green']);
});
...
test('when no visible note block, clicking in page mode will auto add a new note block', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyEdgelessState(page);
    await switchEditorMode(page);
...
test.fixme('Click at empty note should add a paragraph block', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyEdgelessState(page);
    await focusRichText(page);
    await type(page, '123');
    ...
    await waitNextFrame(page);
    await page.mouse.move(paragraphBlock.x + paragraphBlock.width / 2, paragraphBlock.y + paragraphBlock.height / 2);
    await waitNextFrame(page);
    ...
    await page.mouse.click(emptyNote.x + emptyNote.width / 2, emptyNote.y + emptyNote.height / 2);
...
test('Should focus at closest text block when note collapse', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyEdgelessState(page);
    ...
    await page.mouse.move(selectedBox.x + selectedBox.width / 2, selectedBox.y + selectedBox.height);
    await page.mouse.down();
    await page.mouse.move(selectedBox.x + selectedBox.width / 2, selectedBox.y + selectedBox.height + 200, { steps: 10 });
    await page.mouse.up();
    await expect(selectedRect).toBeVisible();
    // Click at the bottom of note to focus at the closest text block
    await page.mouse.click(selectedBox.x + selectedBox.width / 2, selectedBox.y + selectedBox.height - 20);
    await waitNextFrame(page, 200);
...
test('should not select doc only note', async ({ page }) => {
    ...
    expect(await getSelectedBoundCount(page)).toBe(0);
    await dragBetweenViewCoords(page, [noteBound[0] - 10, noteBound[1] - 10], [noteBound[0] + noteBound[2] + 10, noteBound[1] + noteBound[3] + 10]);
    expect(await getSelectedBoundCount(page)).toBe(0);
});
test.describe('visibility of hidden content of edgeless note', () => {
    ...
    test('should hide content when note is not selected or hovered when selected', async ({ page, }) => {
        const note = page.locator('affine-edgeless-note');
        ...
        await page.mouse.move(noteBound.x + noteBound.width + 10, noteBound.y + noteBound.height - 10);
        await expect(lastParagraph).not.toBeInViewport();
    });
    test('should show hidden content when hover on selected note', async ({ page, }) => {
        const note = page.locator('affine-edgeless-note');
        const lastParagraph = page.locator('affine-paragraph').last();
        const noteBound = await note.boundingBox();
        if (!noteBound) {
        ...
        await page.mouse.move(noteBound.x + noteBound.width - 10, noteBound.y + noteBound.height + 100);
        await expect(lastParagraph).toBeInViewport();
    });
    test('should show hidden content when the note is being edited', async ({ page, }) => {
        const note = page.locator('affine-edgeless-note');
        const lastParagraph = page.locator('affine-paragraph').last();
        await note.click({ clickCount: 3 });
        ...
        await expect(lastParagraph, 'editing note but out of viewport should also show hidden content').toBeInViewport();
    });
});
