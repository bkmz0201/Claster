...
import { pressArrowRight, pressEnter, pressEscape, pressSpace, pressTab, selectAllBlocksByKeyboard, SHORT_KEY, type, } from './keyboard.js';
export const defaultPlaygroundURL = new URL(`http://localhost:${process.env.CI ? 4173 : 5173}/`);
...
export const getSelectionRect = async (page) => {
...
export const getEditorLocator = (page) => {
    return page.locator('affine-editor-container').nth(currentEditorIndex);
};
export const getEditorHostLocator = (page) => {
    return page.locator('editor-host').nth(currentEditorIndex);
};
function ignoredLog(message) {
    message.__ignore = true;
}
function isIgnoredLog(message) {
 ...
 * ```
 */
export function expectConsoleMessage(page, logPrefixOrRegex, type = 'error') {
    page.on('console', (message) => {
        const sameType = message.type() === type;
        const textMatch = logPrefixOrRegex instanceof RegExp
...
export async function enterPlaygroundRoom(page, ops) {
    ...
    page.on('console', message => {
        ...
        if (!ignore) {
            expect
                ...
                .toBe('Please remove the "console.log" or declare `expectConsoleMessage` before `enterPlaygroundRoom`. It is advised not to output logs in a production environment.');
        }
    ...
    await page.evaluate(async () => {
        const dom = document.querySelector('affine-editor-container');
        if (dom) {
...
export async function waitDefaultPageLoaded(page) {
    await page.waitForSelector('affine-page-root[data-block-id="0"]');
}
export async function waitEmbedLoaded(page) {
    await page.waitForSelector('.resizable-img');
}
export async function waitNextFrame(page, frameTimeout = NEXT_FRAME_TIMEOUT) {
    await page.waitForTimeout(frameTimeout);
}
export async function clearLog(page) {
    await page.evaluate(() => console.clear());
}
export async function captureHistory(page) {
    await page.evaluate(() => {
        window.doc.captureSync();
    });
}
export async function resetHistory(page) {
    await page.evaluate(() => {
...
export async function enterPlaygroundWithList(page, contents = ['', '', ''], type = 'bulleted') {
    const room = generateRandomRoomId();
    await page.goto(`${DEFAULT_PLAYGROUND}?room=${room}`);
    await page.evaluate(({ contents, type }) => {
        const { doc } = window;
        ...
        for (let i = 0; i < contents.length; i++) {
            doc.addBlock('affine:list', contents.length > 0
...
export async function initEmptyParagraphState(page, rootId) {
...
export async function initMultipleNoteWithParagraphState(page, rootId, count = 2) {
    const ids = await page.evaluate(({ rootId, count }) => {
        const { doc } = window;
        doc.captureSync();
        if (!rootId) {
            rootId = doc.addBlock('affine:page', {
...
export async function initEmptyEdgelessState(page) {
...
export async function initEmptyDatabaseState(page, rootId) {
    const ids = await page.evaluate(async (rootId) => {
        ...
        const model = doc.getModelById(databaseId);
        const datasource = new window.$blocksuite.blocks.database.DatabaseBlockDataSource(model);
...
export async function initKanbanViewState(page, config, rootId) {
    const ids = await page.evaluate(async ({ rootId, config }) => {
        const { doc } = window;
        doc.captureSync();
        if (!rootId) {
            rootId = doc.addBlock('affine:page', {
        ...
        const rowIds = config.rows.map(rowText => {
            const rowId = doc.addBlock('affine:paragraph', { type: 'text', text: new window.$blocksuite.store.Text(rowText) }, databaseId);
            return rowId;
        });
        config.columns.forEach(column => {
            ...
            rowIds.forEach((rowId, index) => {
                ...
                if (value !== undefined) {
                    datasource.cellValueChange(rowId, columnId, column.type === 'rich-text'
                        ? new window.$blocksuite.store.Text(value)
                        : value);
                }
            });
        });
...
export async function initEmptyDatabaseWithParagraphState(page, rootId) {
    const ids = await page.evaluate(async (rootId) => {
        const { doc } = window;
        doc.captureSync();
        if (!rootId) {
...
export async function focusDatabaseTitle(page) {
    const dbTitle = page.locator('[data-block-is-database-title="true"]');
    await dbTitle.click();
    await page.evaluate(() => {
        const dbTitle = document.querySelector('affine-database-title textarea');
        if (!dbTitle) {
...
export async function assertDatabaseColumnOrder(page, order) {
    const columns = await page
        .locator('affine-database-column-header')
        .locator('affine-database-header-column')
        .all();
    expect(await Promise.all(columns.slice(1).map(v => v.innerText()))).toEqual(order);
}
export async function initEmptyCodeBlockState(page, codeBlockProps = {}) {
...
export async function focusRichText(page, i = 0, options) {
...
export async function focusRichTextEnd(page, i = 0) {
    await page.evaluate(([i, currentEditorIndex]) => {
        const editorHost = document.querySelectorAll('editor-host')[currentEditorIndex];
        const richTexts = Array.from(editorHost.querySelectorAll('rich-text'));
        richTexts[i].inlineEditor?.focusEnd();
    }, [i, currentEditorIndex]);
    await waitNextFrame(page);
}
export async function initThreeParagraphs(page) {
...
export async function initSixParagraphs(page) {
    await focusRichText(page);
    await type(page, '1');
    await pressEnter(page);
    await type(page, '2');
...
export async function initThreeLists(page) {
    await focusRichText(page);
    await type(page, '-');
...
export async function insertThreeLevelLists(page, i = 0) {
...
export async function initThreeDividers(page) {
    await focusRichText(page);
    await type(page, '123');
    await pressEnter(page);
    await type(page, '---');
...
export async function initParagraphsByCount(page, count) {
    await focusRichText(page);
...
export async function getInlineSelectionIndex(page) {
...
export async function getInlineSelectionText(page) {
    return page.evaluate(() => {
        const selection = window.getSelection();
        const range = selection.getRangeAt(0);
        const component = range.startContainer.parentElement?.closest('rich-text');
...
export async function getSelectedTextByInlineEditor(page) {
    return page.evaluate(() => {
        const selection = window.getSelection();
        const range = selection.getRangeAt(0);
        const component = range.startContainer.parentElement?.closest('rich-text');
        ...
        return (component?.inlineEditor?.yText.toString().slice(index, index + length) ||
            '');
    });
}
export async function getSelectedText(page) {
    return page.evaluate(() => {
        let content = '';
        const selection = window.getSelection();
        if (selection.rangeCount === 0)
            return content;
        const range = selection.getRangeAt(0);
        const components = range.commonAncestorContainer.parentElement?.querySelectorAll('rich-text') || [];
...
export async function setInlineRangeInSelectedRichText(page, index, length) {
...
export async function setInlineRangeInInlineEditor(page, inlineRange, i = 0) {
    await page.evaluate(({ i, inlineRange }) => {
        const inlineEditor = document.querySelectorAll('[data-v-root="true"]')[i]?.inlineEditor;
        if (!inlineEditor) {
...
export async function pasteTestImage(page) {
    await page.evaluate(async () => {
...
export async function getClipboardHTML(page) {
    const dataInClipboard = await page.evaluate(async () => {
        ...
        const container = html.querySelector('[data-blocksuite-snapshot]');
        if (!container) {
            return '';
        }
...
export async function getClipboardCustomData(page, type) {
    const dataInClipboard = await page.evaluate(async () => {
        const clipItems = await navigator.clipboard.read();
        const item = clipItems.find(item => item.types.includes('text/html'));
        const data = await item?.getType('text/html');
...
export async function getClipboardSnapshot(page) {
    const dataInClipboard = await getClipboardCustomData(page, 'BLOCKSUITE/SNAPSHOT');
    if (!dataInClipboard) {
        throw new Error('dataInClipboard is not found');
    }
    const json = JSON.parse(dataInClipboard);
    return json;
}
export async function getPageSnapshot(page, toJSON) {
...
export async function setSelection(page, anchorBlockId, anchorOffset, focusBlockId, focusOffset) {
    await page.evaluate(({ anchorBlockId, anchorOffset, focusBlockId, focusOffset, currentEditorIndex, }) => {
        const editorHost = document.querySelectorAll('editor-host')[currentEditorIndex];
...
export async function readClipboardText(page, type = 'input') {
...
export const getCenterPositionByLocator = async (_page, locator) => {
...
export async function getBoundingBox(locator) {
    const box = await locator.boundingBox();
    if (!box)
        throw new Error('Missing column box');
    return box;
}
export async function getBlockModel(page, blockId) {
...
export async function getIndexCoordinate(page, [richTextIndex, vIndex], coordOffSet = { x: 0, y: 0 }) {
    const coord = await page.evaluate(({ richTextIndex, vIndex, coordOffSet, currentEditorIndex }) => {
        const editorHost = document.querySelectorAll('editor-host')[currentEditorIndex];
...
export async function focusTitle(page) {
 ...
 */
export async function shamefullyBlurActiveElement(page) {
    await page.evaluate(() => {
 ...
 *
 */
export async function waitForInlineEditorStateUpdated(page) {
...
export async function initImageState(page, prependParagraph = false) {
    await page.evaluate(async (prepend) => {
        ...
        if (!pageRoot)
            throw new Error('Cannot find doc page');
        const imageBlob = await fetch(`${location.origin}/test-card-1.png`).then(response => response.blob());
        const storage = pageRoot.store.blobSync;
        const sourceId = await storage.set(imageBlob);
        if (prepend) {
            doc.addBlock('affine:paragraph', {}, noteId);
        }
        const imageId = doc.addBlock('affine:image', {
            sourceId,
        }, noteId);
        doc.resetHistory();
...
export async function getCurrentEditorDocId(page) {
    return page.evaluate(index => {
        const editor = document.querySelectorAll('affine-editor-container')[index];
        if (!editor)
            throw new Error("Can't find affine-editor-container");
        const docId = editor.doc.id;
        return docId;
    }, currentEditorIndex);
}
export async function getCurrentHTMLTheme(page) {
    const root = page.locator('html');
    return root.getAttribute('data-theme');
}
export async function getCurrentEditorTheme(page) {
...
export async function getCurrentThemeCSSPropertyValue(page, property) {
    const value = await page
        .locator('affine-editor-container')
        .evaluate((_, property) => window
        .getComputedStyle(document.documentElement)
...
export async function scrollToBottom(page) {
    ...
    await page
        ...
        .evaluate(node => node.scrollTo({ left: 0, top: 1000, behavior: 'smooth' }));
    ...
    await page.waitForFunction(() => {
        ...
        Math.abs(scrollContainer.scrollHeight -
            scrollContainer.scrollTop -
            scrollContainer.clientHeight) < 10);
    });
}
export async function mockParseDocUrlService(page, mapping) {
    await page.evaluate(mapping => {
        const parseDocUrlService = window.host.std.get(window.$blocksuite.services.ParseDocUrlProvider);
        parseDocUrlService.parseDocUrl = (url) => {
