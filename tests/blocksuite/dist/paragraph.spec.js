...
import { captureHistory, dragBetweenIndices, dragOverTitle, enterPlaygroundRoom, focusRichText, focusTitle, getBlockIds, getIndexCoordinate, getInlineSelectionIndex, getPageSnapshot, initEmptyEdgelessState, initEmptyParagraphState, initThreeDividers, initThreeParagraphs, pressArrowDown, pressArrowLeft, pressArrowRight, pressArrowUp, pressBackspace, pressBackspaceWithShortKey, pressEnter, pressForwardDelete, pressShiftEnter, pressShiftTab, pressSpace, pressTab, redoByClick, redoByKeyboard, resetHistory, setSelection, SHORT_KEY, switchReadonly, type, undoByClick, undoByKeyboard, updateBlockType, waitDefaultPageLoaded, waitNextFrame, } from './utils/actions/index.js';
import { assertBlockChildrenFlavours, assertBlockChildrenIds, assertBlockCount, assertBlockSelections, assertBlockTextContent, assertBlockType, assertClassName, assertDivider, assertDocTitleFocus, assertRichTextInlineRange, assertRichTexts, assertTitle, } from './utils/asserts.js';
...
for (const { initState, desc } of [
    ...
    test(`backspace on line start of the first block (${desc})`, async ({ page, }) => {
        await enterPlaygroundRoom(page);
    ...
    test(`backspace on line start of the first empty block (${desc})`, async ({ page, }) => {
        await enterPlaygroundRoom(page);
        await initState(page);
        await focusTitle(page);
        await pressArrowDown(page);
...
test('should indent and unindent works with children', async ({ page, }, testInfo) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    ...
    // 012
    // 345
    expect(await getPageSnapshot(page, true)).toMatchSnapshot(`${testInfo.title}_indent.json`);
    // Focus 012
    ...
    //   012|
    // 345
    expect(await getPageSnapshot(page, true)).toMatchSnapshot(`${testInfo.title}_indent_2.json`);
    // Focus 456
    await focusRichText(page, 1);
    await pressTab(page);
    // 123
    ...
    //     012
    // 345
    expect(await getPageSnapshot(page, true)).toMatchSnapshot(`${testInfo.title}_indent_3.json`);
    // Focus 345
    ...
    //     012
    //       345|
    expect(await getPageSnapshot(page, true)).toMatchSnapshot(`${testInfo.title}_indent_4.json`);
    //-- test unindent --//
    // Focus 456
    await focusRichText(page, 1);
    await pressShiftTab(page);
    ...
    //   012
    //     345
    expect(await getPageSnapshot(page, true)).toMatchSnapshot(`${testInfo.title}_unindent_1.json`);
    // Focus 012
    await focusRichText(page, 3);
    await pressShiftTab(page);
    // 123
    ...
    // 012|
    //   345
    expect(await getPageSnapshot(page, true)).toMatchSnapshot(`${testInfo.title}_unindent_2.json`);
    // Focus 789
    await focusRichText(page, 2);
    await pressShiftTab(page);
    // 123
    ...
    // 012
    // 345|
    expect(await getPageSnapshot(page, true)).toMatchSnapshot(`${testInfo.title}_unindent_3.json`);
});
test('paragraph with child block should work at enter', async ({ page, }, testInfo) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await focusRichText(page);
    await type(page, '123');
    ...
    await pressEnter(page);
    await type(page, '789');
    expect(await getPageSnapshot(page, true)).toMatchSnapshot(`${testInfo.title}_final.json`);
});
test('should delete paragraph block child can hold cursor in correct position', async ({ page, }, testInfo) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await focusRichText(page);
    await type(page, '123');
...
test('delete at start of paragraph immediately following list', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await focusRichText(page);
    await type(page, 'hello');
...
test('handling keyup when cursor located in first paragraph', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await focusTitle(page);
    await type(page, 'hello');
...
test('after deleting a text row, cursor should jump to the end of previous list row', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await focusRichText(page);
    await type(page, 'hello');
...
test('press left in first paragraph start should not change cursor position', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await focusRichText(page);
    await type(page, '1');
...
test('press arrow down should move caret to the start of line', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await page.evaluate(() => {
        const { doc } = window;
...
test('press arrow up in the second line should move caret to the first line', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await page.evaluate(() => {
        const { doc } = window;
        const rootId = doc.addBlock('affine:page', {
...
test('press arrow down in indent line should not move caret to the start of line', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await page.evaluate(() => {
        const { doc } = window;
        const rootId = doc.addBlock('affine:page', {
...
test('should placeholder not show when multiple blocks are selected', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await focusRichText(page);
    await pressEnter(page);
...
test.describe('press ArrowDown when cursor is at the last line of a block', () => {
    test.beforeEach(async ({ page }) => {
        ...
        await page.evaluate(() => {
            ...
            const note = doc.addBlock('affine:note', {}, rootId);
            doc.addBlock('affine:paragraph', {
                text: new window.$blocksuite.store.Text('This is the 2nd last block.'),
            }, note);
            doc.addBlock('affine:paragraph', {
    ...
    test('move cursor to next block if this block is _not_ the last block in the page', async ({ page, }) => {
    ...
    test('move cursor to the end of line if the block is the last block in the page', async ({ page, }) => {
...
test('delete empty text paragraph block should keep children blocks when following custom blocks', async ({ page, }, testInfo) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await focusRichText(page);
    await type(page, '--- ');
...
test('delete first paragraph with children should keep children blocks', async ({ page, }) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await focusRichText(page);
    await type(page, '123');
...
test('paragraph indent and delete in line start', async ({ page, }, testInfo) => {
    await enterPlaygroundRoom(page);
    await initEmptyParagraphState(page);
    await focusRichText(page);
    await type(page, 'abc');
    ...
    //   k|lm
    //   nop
    expect(await getPageSnapshot(page, true)).toMatchSnapshot(`${testInfo.title}_after_press_backspace.json`);
    await pressBackspace(page, 2);
    await setSelection(page, 6, 1, 6, 1);
    // abcfg
    //   hijlm
    //   n|op
    expect(await getPageSnapshot(page, true)).toMatchSnapshot(`${testInfo.title}_after_press_backspace_2.json`);
    await pressBackspace(page, 2);
    // abcfg
    //   hijlm
    // |op
    expect(await getPageSnapshot(page, true)).toMatchSnapshot(`${testInfo.title}_after_press_backspace_3.json`);
});
...
test('arrow up/down navigation within and across paragraphs containing different types of text', async ({ page, }) => {
    test.info().annotations.push({
        type: 'issue',
...
test('select divider using delete keyboard from prev/next paragraph', async ({ page, }) => {
    test.info().annotations.push({
        type: 'issue',
...
test.describe('readonly mode', () => {
    ...
    test('should readonly mode not be able to modify text', async ({ page, }, testInfo) => {
        await enterPlaygroundRoom(page);
        await initEmptyParagraphState(page);
        await focusRichText(page);
        await type(page, 'hello');
        ...
        expect(await getPageSnapshot(page, true)).toMatchSnapshot(`${testInfo.title}_1.json`);
        await undoByKeyboard(page);
        expect(await getPageSnapshot(page, true)).toMatchSnapshot(`${testInfo.title}_2.json`);
    });
});
