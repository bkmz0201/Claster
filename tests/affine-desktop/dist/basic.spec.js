...
const historyShortcut = async (page, command) => {
    await page.keyboard.press(command === 'goBack' ? 'ControlOrMeta+[' : 'ControlOrMeta+]');
};
...
test('clientBorder value should disable by default on window', async ({ page, }) => {
    await clickSideBarSettingButton(page);
    await page.waitForTimeout(1000);
    const settingItem = page.locator('[data-testid="client-border-style-trigger"]');
    expect(await settingItem.locator('input').inputValue()).toEqual(process.platform === 'win32' ? 'off' : 'on');
});
...
test('delete workspace', async ({ page }) => {
    ...
    await expect(page.getByTestId('workspace-name-input')).toHaveValue('Delete Me');
    await page.getByTestId('delete-workspace-button').click();
    await page.getByTestId('delete-workspace-input').fill('Delete Me');
    await page.getByTestId('delete-workspace-confirm-button').click();
    await expect(page.getByTestId('workspace-name')).toContainText('Demo Workspace');
});
