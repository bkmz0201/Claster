...
import { createYProxy, Schema, Transformer, } from '@blocksuite/affine/store';
...
function generateMarkdownPreviewBuilder(workspaceId, blocks, yRootDoc) {
    function yblockToDraftModal(yblock) {
        ...
        const props = Object.fromEntries(keys.map(key => [key, createYProxy(yblock.get(`prop:${key}`))]));
        return {
            props,
            ...
            version: yblock.get('sys:version') ?? blockSchema.version,
            keys: Array.from(yblock.keys())
                .filter(key => key.startsWith('prop:'))
                .map(key => key.substring(5)),
        };
    }
    const titleMiddleware = ({ adapterConfigs }) => {
        const pages = yRootDoc?.getMap('meta').get('pages');
        if (!(pages instanceof YArray)) {
            return;
        }
        for (const meta of pages.toArray()) {
            adapterConfigs.set('title:' + meta.get('id'), meta.get('title')?.toString() ?? 'Untitled');
        }
    };
    const baseUrl = `/workspace/${workspaceId}`;
    function getDocLink(docId, blockId) {
    ...
    const markdownAdapter = new MarkdownAdapter(new Transformer({
    ...
    function trimCodeBlock(markdown) {
    ...
    function trimParagraph(markdown) {
        const lines = markdown.split('\n').filter(line => line.trim() !== '');
        if (lines.length > 3) {
    ...
    function getListDepth(block) {
        ...
        do {
            currentBlock = blocks.find(b => b.blockId === currentBlock?.parentBlockId);
    ...
    const generateDatabaseMarkdownPreview = (block) => {
        const isDatabaseBlock = (block) => {
    ...
    const generateImageMarkdownPreview = (block) => {
    ...
    const generateEmbedMarkdownPreview = (block) => {
        const isEmbedModel = (model) => {
            return (model?.flavour === 'affine:embed-linked-doc' ||
    ...
    const generateLatexMarkdownPreview = (block) => {
    ...
    const generateBookmarkMarkdownPreview = (block) => {
        const isBookmarkModel = (model) => {
    ...
    const generateAttachmentMarkdownPreview = (block) => {
        const isAttachmentModel = (model) => {
    ...
    const generateTableMarkdownPreview = (block) => {
        const isTableModel = (model) => {
    ...
    const generateMarkdownPreview = async (block) => {
...
//   - list item 2
// ```
function unindentMarkdown(markdown) {
    const lines = markdown.split('\n');
    const res = [];
...
export async function readAllBlocksFromDoc({ ydoc, rootYDoc, spaceId, maxSummaryLength, }) {
    let docTitle = '';
    let summary = '';
    maxSummaryLength ??= 1000;
    const blockDocuments = [];
    const generateMarkdownPreview = generateMarkdownPreviewBuilder(spaceId, blockDocuments, rootYDoc);
    ...
    const nearest = (blockId, predicate) => {
        let current = blockId;
        while (current) {
    ...
    const nearestByFlavour = (blockId, flavour) => nearest(blockId, block => block.get('sys:flavour') === flavour);
    ...
    const queue = [{ id: rootBlockId }];
    const visited = new Set(); // avoid loop
    const pushChildren = (id, block) => {
        const children = block.get('sys:children');
        if (children instanceof YArray && children.length) {
    ...
    while (queue.length) {
        ...
        if (flavour === 'affine:page') {
            ...
            const refs = uniq(deltas
                ...
                .filter(ref => !!ref));
            const databaseName = flavour === 'affine:paragraph' && parentFlavour === 'affine:database' // if block is a database row
                ? parentBlock?.get('prop:title')?.toString()
                : undefined;
            blockDocuments.push({
                ...commonBlockProps,
                content: text.toString(),
                ...refs.reduce((prev, curr) => {
            ...
            if (!(elementsObj instanceof YMap &&
                elementsObj.get('type') === '$blocksuite:internal:native$')) {
                continue;
            }
            const elements = elementsObj.get('value');
            ...
            const contents = Array.from(block.keys())
                .map(key => {
    ...
    for (const block of blockDocuments) {
        if (block.ref?.length) {
            ...
            const sameNoteBlocks = noteBlock
                ? blockDocuments.filter(candidate => nearestByFlavour(candidate.blockId, 'affine:note') === noteBlock)
                : [];
            // only generate markdown preview for reference blocks
            let previewText = (await generateMarkdownPreview(target)) ?? '';
            let previousBlock = 0;
            let followBlock = 0;
            let previousIndex = sameNoteBlocks.findIndex(block => block.blockId === target.blockId);
            let followIndex = previousIndex;
            while (!((previewText.length > TARGET_PREVIEW_CHARACTER || // stop if preview text reaches the limit
                ((previousBlock >= TARGET_PREVIOUS_BLOCK || previousIndex < 0) &&
                    (followBlock >= TARGET_FOLLOW_BLOCK ||
                        followIndex >= sameNoteBlocks.length))) // stop if no more blocks, or preview block reaches the limit
            )) {
                if (previousBlock < TARGET_PREVIOUS_BLOCK) {
                    previousIndex--;
                    const block = previousIndex >= 0 ? sameNoteBlocks.at(previousIndex) : null;
                    const markdown = block ? await generateMarkdownPreview(block) : null;
                    if (markdown &&
                        !previewText.startsWith(markdown) /* A small hack to skip blocks with the same content */) {
                ...
                if (followBlock < TARGET_FOLLOW_BLOCK) {
                    ...
                    const markdown = block ? await generateMarkdownPreview(block) : null;
                    if (markdown &&
                        !previewText.endsWith(markdown) /* A small hack to skip blocks with the same content */) {
 ...
 */
export function readAllDocsFromRootDoc(rootDoc, options) {
    const docs = rootDoc.getMap('meta').get('pages');
...
export function readAllDocIdsFromRootDoc(rootDoc, options) {
    const docs = rootDoc.getMap('meta').get('pages');
