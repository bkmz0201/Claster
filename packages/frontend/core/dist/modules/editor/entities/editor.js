...
import { DocModeProvider, findCommentedBlocks, findCommentedElements, } from '@blocksuite/affine/shared/services';
...
export class Editor extends Entity {
    ...
    setDefaultOpenProperty(defaultOpenProperty) {
    ...
    bindWorkbenchView(view) {
        ...
        const viewParams$ = view
            .queryString$(paramsParseOptions)
            .map(preprocessParams)
            .map(params => defaults(params, {
    ...
    handleFocusAt(focusAt) {
        ...
        if (commentId && !id && editorContainer.host?.std) {
            ...
            const inlineCommentedSelections = findCommentedTexts(std.store, commentId);
    ...
    bindEditorContainer(editorContainer, docTitle, scrollViewport) {
        ...
        else if (this.mode$.value === 'edgeless' &&
        ...
        if (gfx) {
            const subscription = gfx.viewport.viewportUpdated.subscribe(saveScrollPosition);
        ...
        if (!editorMode || editorMode !== 'edgeless' || !gfx) {
            ...
            this.isPresenting$.next(gfx.tool.currentToolName$.peek() === 'frameNavigator');
            const disposable = effect(() => {
                this.isPresenting$.next(gfx.tool.currentToolName$.value === 'frameNavigator');
            });
    ...
    constructor(docService, workspaceService) {
        ...
        this.scope = this.framework.createScope(EditorScope, {
            editor: this,
        });
        this.mode$ = new LiveData('page');
        this.selector$ = new LiveData(undefined);
        this.doc = this.docService.doc;
        this.isSharedMode = this.workspaceService.workspace.openOptions.isSharedMode;
        this.editorContainer$ = new LiveData(null);
        this.defaultOpenProperty$ = new LiveData(undefined);
        this.workbenchView = null;
        this.scrollPosition = {
            page: null,
            edgeless: null,
        };
        this.focusAt$ = LiveData.computed(get => {
        ...
        this.isPresenting$ = new LiveData(false);
    }
}
