...
import { clickNewPageButton, getBlockSuiteEditorTitle, getPageByTitle, waitForEditorLoad, } from '@affine-test/kit/utils/page-logic';
import { clickSideBarAllPageButton, clickSideBarCurrentWorkspaceBanner, } from '@affine-test/kit/utils/sidebar';
import { createLocalWorkspace } from '@affine-test/kit/utils/workspace';
import { expect } from '@playwright/test';
const removeOnboardingPages = async (page) => {
...
const createAndPinCollection = async (page, options) => {
    await page.getByTestId('all-pages').click();
...
test('Show collections items in sidebar', async ({ page }) => {
    ...
    await page.getByTestId('confirm-modal-confirm').click();
    await expect(first.locator('[data-testid^="navigation-panel-doc-"]')).toHaveCount(0);
...
test('add collection from sidebar', async ({ page }) => {
    ...
    await page
        ...
        .click();
    const nullCollection = page.getByTestId('slider-bar-collection-empty-message');
...
//# sourceMappingURL=local-first-collections-items.spec.js.map