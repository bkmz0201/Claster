...
import { AuthProvider, AuthService, DefaultServerService, ServerScope, ServerService, ServersService, SubscriptionService, ValidatorProvider, } from '@affine/core/modules/cloud';
...
import { configureLocalStorageStateStorageImpls, NbstoreProvider, } from '@affine/core/modules/storage';
import { PopupWindowProvider } from '@affine/core/modules/url';
import { ClientSchemeProvider } from '@affine/core/modules/url/providers/client-schema';
import { configureBrowserWorkbenchModule, WorkbenchService, } from '@affine/core/modules/workbench';
import { getAFFiNEWorkspaceSchema, WorkspacesService, } from '@affine/core/modules/workspace';
import { configureBrowserWorkspaceFlavours } from '@affine/core/modules/workspace-engine';
import { getWorkerUrl } from '@affine/env/worker';
import { refreshSubscriptionMutation, requestApplySubscriptionMutation, } from '@affine/graphql';
import { I18n } from '@affine/i18n';
import { StoreManagerClient } from '@affine/nbstore/worker/client';
import { Container } from '@blocksuite/affine/global/di';
import { docLinkBaseURLMiddleware, MarkdownAdapter, titleMiddleware, } from '@blocksuite/affine/shared/adapters';
...
framework.impl(VirtualKeyboardProvider, {
    ...
    onChange: callback => {
        ...
        Promise.all([
            ...
            .then(handlers => {
            disposeRef.dispose = () => {
                Promise.all(handlers.map(handler => handler.remove())).catch(console.error);
            };
        })
            .catch(console.error);
        return () => {
...
framework.impl(HapticProvider, {
    ...
    vibrate: options => Haptics.vibrate(options),
...
window.getCurrentServerBaseUrl = () => {
    ...
    const currentServer = (currentServerId ? serversService.server$(currentServerId).value : null) ??
        defaultServerService.server;
    return currentServer.baseUrl;
};
window.getCurrentI18nLocale = () => {
    return I18n.language;
};
window.getAiButtonFeatureFlag = () => {
...
window.getCurrentDocContentInMarkdown = async () => {
    const globalContextService = frameworkProvider.get(GlobalContextService);
    const currentWorkspaceId = globalContextService.globalContext.workspaceId.get();
...
window.createNewDocByMarkdownInCurrentWorkspace = async (markdown, title) => {
    const globalContextService = frameworkProvider.get(GlobalContextService);
    const currentWorkspaceId = globalContextService.globalContext.workspaceId.get();
    const workspacesService = frameworkProvider.get(WorkspacesService);
    const workspaceRef = currentWorkspaceId
...
window.updateSubscriptionState = async () => {
    const globalContextService = frameworkProvider.get(GlobalContextService);
    const currentServerId = globalContextService.globalContext.serverId.get();
    const serversService = frameworkProvider.get(ServersService);
    const defaultServerService = frameworkProvider.get(DefaultServerService);
...
window.requestApplySubscription = async (transactionId) => {
    const globalContextService = frameworkProvider.get(GlobalContextService);
    const currentServerId = globalContextService.globalContext.serverId.get();
    const serversService = frameworkProvider.get(ServersService);
    const defaultServerService = frameworkProvider.get(DefaultServerService);
...
export function App() {
    return (_jsx(Suspense, { children: _jsx(FrameworkRoot, { framework: frameworkProvider, children: _jsx(I18nProvider, { children: _jsxs(AffineContext, { store: getCurrentStore(), children: [_jsx(KeyboardThemeProvider, {}), _jsx(ModalConfigProvider, { children: _jsx(BlocksuiteMenuConfigProvider, { children: _jsx(RouterProvider, { fallbackElement: _jsx(AppFallback, {}), router: router, future: future }) }) })] }) }) }) }));
}
function createStoreManagerClient() {
    const worker = new Worker(getWorkerUrl('nbstore'));
    const { port1: nativeDBApiChannelServer, port2: nativeDBApiChannelClient } = new MessageChannel();
