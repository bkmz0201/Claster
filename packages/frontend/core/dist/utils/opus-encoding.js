 ...
 */
function extractAudioData(audioBuffer, startSample = 0, endSample) {
    const numberOfChannels = audioBuffer.numberOfChannels;
    const sampleCount = endSample !== undefined
 ...
 */
export function createOpusEncoder(config) {
    const encodedChunks = [];
 ...
 */
async function encodeAudioFrames({ audioData, numberOfChannels, sampleRate, encoder, }) {
 ...
 */
export function muxToMp4(encodedChunks, config) {
 ...
 */
async function encodeAudioBufferToOpus(audioBuffer, targetBitrate = DEFAULT_BITRATE) {
    const config = {
 ...
 */
export async function encodeRawBufferToOpus({ filepath, sampleRate, numberOfChannels, }) {
 ...
 */
export async function encodeAudioBlobToOpus(blob, targetBitrate = DEFAULT_BITRATE) {
    const audioContext = new AudioContext();
    logger.debug('Encoding audio blob to Opus');
    try {
        const arrayBuffer = await blobToArrayBuffer(blob);
        const audioBuffer = await audioContext.decodeAudioData(arrayBuffer);
        const { encodedChunks, config } = await encodeAudioBufferToOpus(audioBuffer, targetBitrate);
 ...
 */
function findSlicePoint(audioBuffer, startSample, endSample, minSliceSamples) {
...
export async function encodeAudioBlobToOpusSlices(blob, targetBitrate = DEFAULT_BITRATE) {
    const audioContext = new AudioContext();
    try {
        const arrayBuffer = await blobToArrayBuffer(blob);
        const audioBuffer = await audioContext.decodeAudioData(arrayBuffer);
        ...
        while (startSample < totalSamples) {
            ...
            endSample = findSlicePoint(audioBuffer, startSample, endSample, minSliceSamples);
...
export const createStreamEncoder = (recordingId, codecs) => {
    ...
    const toAudioData = (buffer) => {
        ...
        return new AudioData({
            ...
            sampleRate: codecs.sampleRate,
            numberOfChannels: codecs.numberOfChannels,
            numberOfFrames: buffer.length / BYTES_PER_SAMPLE / codecs.numberOfChannels,
            timestamp: 0,
            data: buffer,
        });
    };
    ...
    const next = async () => {
        ...
        const { buffer, nextCursor } = await apis.recording.getRawAudioBuffers(recordingId, cursor);
...
//# sourceMappingURL=opus-encoding.js.map