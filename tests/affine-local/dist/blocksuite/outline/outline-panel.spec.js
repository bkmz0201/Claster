...
import { clickEdgelessModeButton, clickPageModeButton, clickView, createEdgelessNoteBlock, focusDocTitle, getEdgelessSelectedIds, getViewportCenter, locateToolbar, setViewportCenter, } from '@affine-test/kit/utils/editor';
import { pressBackspace, pressEnter, pressEscape, selectAllByKeyboard, } from '@affine-test/kit/utils/keyboard';
import { openHomePage } from '@affine-test/kit/utils/load-page';
import { clickNewPageButton, type, waitForEditorLoad, } from '@affine-test/kit/utils/page-logic';
import { closeSettingModal, confirmExperimentalPrompt, openExperimentalFeaturesPanel, openSettingModal, } from '@affine-test/kit/utils/setting';
import { openRightSideBar } from '@affine-test/kit/utils/sidebar';
import { expect } from '@playwright/test';
import { createHeadings, createTitle, getVerticalCenterFromLocator, } from './utils';
async function openTocPanel(page) {
...
function getTocHeading(panel, level) {
...
test.describe('TOC display', () => {
    test('should display title and headings when there are non-empty headings in editor', async ({ page, }) => {
    ...
    test('should not display headings when there are only empty headings', async ({ page, }) => {
        await createTitle(page);
    ...
    test('should update panel when modify or clear title or headings', async ({ page, }) => {
    ...
    test('visibility sorting should be enabled in edgeless mode and disabled in page mode by default, and can be changed', async ({ page, }) => {
        await pressEnter(page);
        await type(page, '# Heading 1');
        const toc = await openTocPanel(page);
    ...
    test('should notify user when there are some page only notes and sorting is disabled', async ({ page, }) => {
        await clickEdgelessModeButton(page);
        const toc = await openTocPanel(page);
...
test.describe('TOC and editor scroll', () => {
    test('should highlight heading when scroll to area before viewport center', async ({ page, }) => {
        const title = await createTitle(page);
        for (let i = 0; i < 3; i++) {
            await pressEnter(page);
        }
        ...
        await title.scrollIntoViewIfNeeded();
        const toc = await openTocPanel(page);
        const viewportCenter = await getVerticalCenterFromLocator(page.locator('body'));
        const activeHeadingContainer = toc.locator('affine-outline-panel-body .active');
        await title.click();
    ...
    test('should scroll to heading and highlight heading when click item in outline panel', async ({ page, }) => {
        const headings = await createHeadings(page, 10);
        const toc = await openTocPanel(page);
        const activeHeadingContainer = toc.locator('affine-outline-panel-body .active');
        const headingsInPanel = Array.from({ length: 6 }, (_, i) => getTocHeading(toc, i + 1));
    ...
    test('should scroll to title when click title in outline panel', async ({ page, }) => {
        const title = await createTitle(page);
        await pressEnter(page);
...
test.describe('TOC and edgeless selection', () => {
    test('should select note blocks when selecting cards in TOC', async ({ page, }) => {
        const toc = await openTocPanel(page);
        await clickEdgelessModeButton(page);
        ...
        expect(await getEdgelessSelectedIds(page), 'should not select doc only note').toHaveLength(0);
    ...
    test('should select note cards when select note blocks in canvas', async ({ page, }) => {
        await clickEdgelessModeButton(page);
...
test.describe('drag and drop note in outline panel', () => {
    const dragCard = async (from, to, position = 'before') => {
        ...
        await from.dragTo(to, {
            ...
            targetPosition: position === 'before'
    ...
    test('should reorder notes when drag and drop a note in outline panel', async ({ page, }) => {
        const toc = await openTocPanel(page);
        const cards = locateCards(toc);
    ...
    test('multiple selected note cards can be dragged and dropped at same time', async ({ page, }) => {
        const toc = await openTocPanel(page);
        const cards = locateCards(toc);
        await cards.nth(2).click({ modifiers: ['Shift'] });
...
test.describe('advanced visibility control', () => {
    ...
    test('should update notes when change note display mode from note toolbar', async ({ page, }) => {
        await clickEdgelessModeButton(page);
        await setViewportCenter(page, [0, 0]);
        await createEdgelessNoteBlock(page, [100, 100]);
    ...
    test('should update notes after slicing note', async ({ page }) => {
        ...
        await expect(bothCard.nth(1), 'the sliced note card should keep the order in its parent').toContainText('hello');
        await expect(bothCard.nth(2), 'the sliced note card should keep the order in its parent').toContainText('world');
    });
});
//# sourceMappingURL=outline-panel.spec.js.map