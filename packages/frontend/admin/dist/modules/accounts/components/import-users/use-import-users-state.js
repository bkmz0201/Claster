...
import { exportImportResults, getValidUsersToImport, ImportStatus, processCSVFile, validateUsers, } from '../../utils/csv-utils';
import { useImportUsers, } from '../use-user-management';
export function useImportUsersState({ passwordLimits, onClose, }) {
    ...
    const importUsersCallback = useCallback((result) => {
        const successfulUsers = result.filter((user) => user.__typename === 'UserType');
        const failedUsers = result.filter((user) => user.__typename === 'UserImportFailedType');
        const successCount = successfulUsers.length;
        const failedCount = parsedUsers.length - successCount;
        if (failedCount > 0) {
            toast.info(`Successfully imported ${successCount} users, ${failedCount} failed`);
        }
        else {
            toast.success(`Successfully imported ${successCount} users`);
        }
        const successfulUserEmails = new Set(successfulUsers.map(user => user.email));
        const failedUserErrorMap = new Map(failedUsers.map(user => [user.email, user.error]));
        setParsedUsers(prev => {
    ...
    const handleFileSelected = useCallback(async (file) => {
        setIsImporting(true);
        try {
            await processCSVFile(file, usersFromCsv => {
    ...
    const isImported = !!parsedUsers.some(user => user.importStatus && user.importStatus !== ImportStatus.Processing);
...
//# sourceMappingURL=use-import-users-state.js.map